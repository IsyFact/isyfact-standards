...
NOTE: Details hierzu können in der xref:glossary:literaturextern:inhalt.adoc#litextern-Spring-Boot-Properties-Conversion[Spring Boot Dokumentation zu „Properties Conversion"] nachgelesen werden.
...
Für weitere Details siehe xref:blaupausen:detailkonzept-komponente-datenzugriff/master.adoc#_detailkonzept_datenzugriff_einleitung[Detailkonzept Komponente Datenzugriff].
...
Die Konfiguration dieses Tasks ist im Dokument xref:isy-task:nutzungsvorgaben/master.adoc#einleitung[Nutzungsvorgaben Task-Schedulung] beschrieben.
