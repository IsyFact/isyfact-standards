...
Das Konzept JPA/Hibernate konkretisiert die xref:blaupausen:detailkonzept-komponente-datenzugriff/einordnung-ziele.adoc[Vorgaben aus der Referenzarchitektur] bezüglich des Datenzugriffs und bietet eine architektonische Lösung auf Basis des O/R-Mappers Hibernate.
...
Das größte Thema behandelt die xref:konzept/vorgaben-konventionen.adoc[] bei der Umsetzung des Datenzugriffs mit JPA und Hibernate.
Die Vorgaben und Konventionen betreffen die folgenden Bereiche:

* die generelle Verwendung von JPA und Hibernate zum Datenzugriff,
