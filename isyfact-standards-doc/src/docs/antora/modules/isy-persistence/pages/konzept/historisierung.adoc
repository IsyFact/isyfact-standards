...
****
Theoretisch wäre es auch möglich, die Historisierung auf der Ebene des Datenzugriffs durchzuführen.
Dazu würde der Datenzugriff, statt Entitäten zu aktualisieren, neue Entitäten mit aktuellen Zeitstempeln anlegen.
Beim Auslesen würde der Datenzugriff immer der aktuellste Datensatz liefern.
