...
Liquibase nutzt JDBC für die Datenbankkommunikation in Java- bzw. Spring-basierten Anwendungen.
...
Vor der Einführung von Liquibase in die IsyFact gab es eine eigens entwickelte xref:konzept/versionierung.adoc[Versionierung], die den aktuellen Zustand der Datenbank dokumentiert hat.
...
Liquibase verwaltet zudem eine interne Historie aller Änderungen, sodass zukünftige Anpassungen in einer konsistenten Reihenfolge ausgeführt werden.
