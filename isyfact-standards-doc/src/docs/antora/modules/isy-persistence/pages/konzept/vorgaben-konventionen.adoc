...
Die Vorgaben und Konventionen ergänzen die allgemeinen xref:referenzarchitektur:software-technisch/backend/persistenzschicht.adoc#persistenz-klassenmodell-vorgaben[Vorgaben zur Erstellung eines Persistenz-Klassenmodells].
...
Eine 1-zu-n-Assoziation (siehe https://docs.jboss.org/hibernate/orm/5.6/userguide/html_single/Hibernate_User_Guide.html#collections[Collection Mapping]) ist in der Regel als unsortierte Menge (`Set`) zu definieren, da in dieser keine Reihenfolge definiert ist.
...
Hinweise zu deren Verwendung finden sich im xref:isy-datetime-doc:ROOT:konzept/master.adoc[Baustein Datum & Zeit].
