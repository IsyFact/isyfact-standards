...
Bidirektionale Assoziation beschreibt die Beziehung zwischen zwei Entitäten, wobei jede Entität einen Verweis auf die andere Entität hat. Sie ermöglicht es Ihnen, von einer Entität zu einer anderen Entität zu navigieren, die mit ihr verbunden ist, und umgekehrt.
...
Wenn eine bidirektionale Assoziation gebildet wird, muss der Anwendungsentwickler sicherstellen, dass beide Seiten zu jeder Zeit synchron sind.

===== Vermeiden von to-many-Assoziationen
Bei einer to-many Assoziation lädt Hibernate alle zugehörigen Entitäten, wenn es die Assoziation initialisiert. Das kann mehrere Sekunden oder sogar Minuten dauern, wenn Hibernate mehrere tausend Entitäten abrufen muss.

Stattdessen sollte für die Traversierung in Gegenrichtung (many-to-one) eine Query verwendet werden.

===== Best Practices
Generell sollten bei der Verwendung von Assoziationen die Best Practices beachtet werden
