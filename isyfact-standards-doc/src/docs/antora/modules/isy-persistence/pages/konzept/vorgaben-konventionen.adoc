...
Außer der dafür vorgesehenen @Filter Annotation gibt es auch die @Where Annotation, die jedoch immer aktiv ist und eine statische Filterung durchführt.
Deshalb wird diese Möglichkeit nicht bzw. nur im Ausnahmefall empfohlen und hier nicht näher betrachtet, siehe aber <<beispiel-where-annotation, Beispiel @Where Annotation>>.
...
Sie können an Entitäten und Collections (Assoziationen) definiert werden.
****
...
==== Beispiele für die Verwendung von Hibernate Filtern

Zur Veranschaulichung wird ein Beispiel für die Verwendung von Hibernate Filtern aufgeführt.
Es gibt eine Entität "User" und eine Entität "Item" sowie eine 1:n-Assoziation zwischen User und Item.
...
.Hibernate Filter auf Klassen-Ebene und Collection-Ebene
[source,java]
----
@Entity
...
//Hibernate Filter ist deaktiviert per Default
List<Item> alleItems = itemRepository.findAll();
assertEquals(3, alleItems.size());

User user = userRepository.findById(1).orElse(null);
assertEquals(3, user.getItems());
...
assertEquals(2, aktuelleItems.size());

assertEquals(2, user.getItems());
----

CAUTION: Suchen per Identifier (itemRepository.findById(1)) filtert nicht, siehe xref:glossary:literaturextern:inhalt.adoc#litextern-hibernate-filter[Filtering entities and associations].

[[beispiel-where-annotation]]
Das Beispiel lässt sich auch mit der @Where Annotation umsetzen.
Die Anwendung kann dann jedoch keine gelöschten Items abfragen.
