...
CAUTION: Das Suchen per Identifier (z.B. mittels `itemRepository.findById(1)`) wendet keine Filter an, siehe xref:literaturextern:inhalt.adoc#litextern-hibernate-filter[filtering entities and associations].
...
NOTE: siehe xref:literaturextern:inhalt.adoc#litextern-multitable-bulk-operations[Multi-table Bulk Operations]
...
Eine 1-zu-n-Assoziation (siehe xref:literaturextern:inhalt.adoc#litextern-collection-mapping[Collection Mapping]) ist in der Regel als unsortierte Menge (`Set`) zu definieren, da in dieser keine Reihenfolge definiert ist.
...
* xref:literaturextern:inhalt.adoc#litextern-jakarta-persistence[Jakarta Persistence],
* xref:literaturextern:inhalt.adoc#litextern-hibernate-inheritance[Hibernate User Guide - Inheritance],
* xref:literaturextern:inhalt.adoc#litextern-jpa-hibernate[Java Persistence with Hibernate].
====
