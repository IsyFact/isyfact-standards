...
[[vererbungshierarchien-regeln]]
=== Vererbungshierarchien: Architekturregeln

Die Jakarta Persistence API Specification beschreibt mehrere Strategien des O/R-Mappings von Vererbungshierarchien, zu beachten ist auch xref:blaupausen:detailkonzept-komponente-datenzugriff/persistenzlogik.adoc#vererbung-im-persistenz-klassenmodell[Vererbung im Persistenz-Klassenmodell].
...
=== Vererbungshierarchien: Beispiele, Vor- und Nachteile
...
Vgl. auch <<table-per-concrete-class-rules>>.
...
Vgl. auch <<mapped-superclass-rules>>.
