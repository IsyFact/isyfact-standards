...
Dies gewährleistet eine nachvollziehbare Historie der Schemaänderungen.
...
[source,yaml]
----
liquibase --url=$DB_URL --username=$DB_USER --password=$DB_PASSWORD rollback --tag=$RELEASE_VERSION
liquibase --url=$DB_URL --username=$DB_USER --password=$DB_PASSWORD history
----
