...
Die Repositories werden im Package der Fachkomponente abgelegt, welche die Datenhoheit über die Tabelle(n) des Repositories besitzt (zum Thema Datenhoheit siehe xref:referenzarchitektur:software-technisch/backend.adoc#datenzugriff[Referenzarchitektur IT-System]).
Die Repositories werden nur von Klassen der Fachkomponente mit Datenhoheit aufgerufen.

Gemäß der xref:referenzarchitektur:software-technisch/backend.adoc#datenzugriff[Referenzarchitektur] dürfen Entitäten nur von der Fachkomponente im Anwendungskern verändert werden, welche zur Fachkomponente im Datenzugriff gehört.
