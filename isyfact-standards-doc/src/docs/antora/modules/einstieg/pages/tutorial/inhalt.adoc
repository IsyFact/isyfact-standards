...
Alle Konzepte und Vorgaben zum Thema Persistenz und Datenzugriff stehen im <<DetailkonzeptKomponenteDatenzugriff>>.
...
* DAOs und Entitätsklassen werden in einer bestimmten Packetstrucktur abgelegt (siehe <<DetailkonzeptKomponenteDatenzugriff>>).
...
Dieser Abschnitt beschreibt die Realisierung von HTTP Invoker Schnittstellen (siehe <<Spring>>).
...
:desc-image-KDServiceSS: Klassendesign für HTTP Invoker Service-Schnittstellen
...
Eine Definition und Kriterien zur Typisierung können in <<KonzeptKonfiguration>> nachgelesen werden.
...
Details zur Implementierung siehe <<KonzeptKonfiguration>>.
...
NOTE: Detaillierte Informationen zur Überwachung sind im <<KonzeptUeberwachung>> und den
<<NutzungsvorgabenUeberwachung>> enthalten.
