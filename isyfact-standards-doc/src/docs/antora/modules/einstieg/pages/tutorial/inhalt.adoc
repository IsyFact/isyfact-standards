...
In diesem Kapitel wird die Realisierung von verschiedenen, technischen Zugangswegen zum Anwendungskern beschrieben.
...
Dieser Abschnitt beschreibt die Realisierung von HTTP Invoker Schnittstellen (siehe https://docs.spring.io/spring-framework/docs/5.3.x/reference/html/[Spring Framework]).
...
Der vom AufrufKontextVerwalter verwaltete AufrufKontext wird beim Aufruf der Anwendung in der Service-Schnittstelle gesetzt und steht fortan, während der Verarbeitung des Requests, in der gesamten Anwendung zur Verfügung.
...
zu programmieren.
* Eine erste Berechtigungsprüfung erfolgt in der Service-Schnittstelle.
Es wird geprüft, ob der Aufrufer den Service überhaupt verwenden darf.
