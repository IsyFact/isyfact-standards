...
* Die Authentifizierung von Anfragen wird im Servicegateway und im Portal mithilfe des IAM-Systems durchgeführt.
* Prozesse, die innerhalb des Anwendungssystems oder der Anwendungslandschaft starten (z.B. Timertasks, Batches) verwenden einen technischen Benutzer und authentifizieren diesen selbständig gegen das IAM-System.
