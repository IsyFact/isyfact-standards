...
Alle Konzepte und Vorgaben zum Thema Persistenz und Datenzugriff stehen im Dokument xref:blaupausen:detailkonzept-komponente-datenzugriff/master.adoc[Detailkonzept Komponente Datenzugriff].
...
* DAOs und Entitätsklassen werden in einer bestimmten Paketstruktur abgelegt (siehe xref:blaupausen:detailkonzept-komponente-datenzugriff/master.adoc[Detailkonzept Komponente Datenzugriff]).
...
Dazu werden die benötigten DDL-Anweisungen, wie im xref:blaupausen:detailkonzept-komponente-datenzugriff/master.adoc[Detailkonzept Komponente Datenzugriff] beschrieben, in einem Verzeichnis abgelegt.
