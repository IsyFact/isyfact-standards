...
Alle Bibliotheken der IsyFact stehen über <<TODO Linkziel: MavenCentral>> zum Download bereit.
...
Alle Konzepte und Vorgaben zum Thema Persistenz und Datenzugriff stehen im <<TODO anderes Dokument: DetailkonzeptKomponenteDatenzugriff>>.
...
* DAOs und Entitätsklassen werden in einer bestimmten Packetstrucktur abgelegt (siehe <<TODO anderes Dokument: DetailkonzeptKomponenteDatenzugriff>>).
...
Entitäten, die von mehreren xref:glossary:glossary:master.adoc#glossar-Anwendungskomponente[Anwendungskomponenten (Glossar)] verwendet werden, bietet die Komponente Basisdaten an.
...
Weitere Details können <<TODO anderes Dokument: IsyFactReferenzarchitektur>> entnommen werden.
...
Dazu werden die benötigten DDL-Anweisungen, wie im <<TODO anderes Dokument: DetailkonzeptKomponenteDatenzugriff>> beschrieben, in einem Verzeichnis abgelegt.
Die <<TODO anderes Dokument: Vorlageanwendung>> enthält diese Anweisungen im Verzeichnis `src/main/skripte/sql/terminfindung`.
...
Eine passende Beispielkonfiguration findet sich in der Maven-Konfiguration der <<TODO anderes Dokument: Vorlageanwendung>>.
...
Für fachliche Komponenten wird eine Basisimplementierung in <<TODO anderes Dokument: IsyFactReferenzarchitektur>> beschrieben.
...
Dieser Abschnitt beschreibt die Realisierung von HTTP Invoker Schnittstellen (siehe <<TODO Linkprüfung: Spring>>).
...
Details sind in <<TODO anderes Dokument: KonzeptFehlerbehandlung>> nachzulesen.
...
Die Implementierung der Fehlerbehandlung wird im Detail in <<TODO anderes Dokument: KonzeptFehlerbehandlung>> beschrieben. +
...
Ein Beispiel dafür findet sich in <<TODO anderes Dokument: Vorlageanwendung>>.
...
An den Methoden der Service-Implementierung werden die Annotationen `@StelltAufrufKontextBereit` und `@Gesichert` gemäß <<TODO anderes Dokument: NutzungsvorgabenSicherheit>> verwendet, um den Zugriff auf die Service-Methode zu autorisieren.

Beispiele dafür finden sich in <<TODO anderes Dokument: Vorlageanwendung>>.
...
Eine Beschreibung der Parameter ist in <<TODO anderes Dokument: DetailkonzeptKomponenteBatch>> enthalten.
...
Ein Beispiel ist in <<TODO anderes Dokument: Vorlageanwendung>> enthalten.
...
* Es wird ein Debug-, Info- und ein Error-Log geführt.
  Die Zuordnung der Log-Levels auf diese Log-Arten wird in <<TODO anderes Dokument: KonzeptLogging>> definiert.
...
Eine Definition und Kriterien zur Typisierung können in <<TODO anderes Dokument: KonzeptKonfiguration>> nachgelesen werden.
...
Details zur Implementierung siehe<<TODO anderes Dokument: KonzeptKonfiguration>>.
...
Weitere Hinweise für die richtige Behandlung von Fehlern sind in <<TODO anderes Dokument: KonzeptFehlerbehandlung>> enthalten.
...
Ein Beispiel für die Verwendung ist in der <<TODO anderes Dokument: Vorlageanwendung>> enthalten.
...
Ein Beispiel hierfür ist wieder in <<TODO anderes Dokument: Vorlageanwendung>> enthalten.
...
.Einbindung der Bibliotheken zur Nutzung der Protokollierung
...
Ein Klassendiagram ist in <<TODO anderes Dokument: ProtokollierungKonzept>> enthalten.
...
Die Konfiguration ist in <<TODO anderes Dokument: NutzungsvorgabenSicherheit>> beschrieben.
...
Dies erfolgt über Annotationen oder im Webflow (siehe <<TODO anderes Dokument: NutzungsvorgabenSicherheit>>).
...
NOTE: Detaillierte Informationen zur Überwachung sind im <<TODO anderes Dokument: KonzeptUeberwachung>> und den
<<TODO anderes Dokument: NutzungsvorgabenUeberwachung>> enthalten.

[[designvorgaben-11]]
==== Designvorgaben

* Die Erreichbarkeit der Systems wird über einen HealthCheck von Spring Boot Actuator realisiert.
...
Konfiguration besteht aus aus zwei Teilen:
