...
Die IsyFact basiert auf einer Reihe von etablierten Software-Produkten, die die unterschiedlichen funktionalen Anforderungen eines Anwendungssystems realisieren.
...
== Bedienkonzepte und die Gestaltung von Benutzeroberflächen
...
Die Dokumente der IsyFact verweisen an verschiedenen Stellen auf das Bedienkonzept und dort zu definierende Regeln.
Es wird jedoch vorausgesetzt, dass jeder Anwender der IsyFact einen eigenes Bedienkonzept erstellt, der spezifisch auf seinen Anwendungskontext abgestimmt ist.
