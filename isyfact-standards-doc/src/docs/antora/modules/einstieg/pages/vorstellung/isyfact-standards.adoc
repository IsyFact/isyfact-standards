...
Die Vorgaben dazu befinden sich mitsamt Vorgehen und Beispielen in der Vorlage für die xref:methodik:spezifikation/systemspezifikation.adoc[Systemspezifikation].
...
Für die Erfassung von Anforderungslisten gibt es ebenfalls eine xref:methodik:spezifikation/anforderungsliste.adoc[Vorlage Anforderungsliste].
...
Im Dokument xref:methodik:spezifikation/datenflussdiagramme.adoc[Anleitung zu Datenflussdiagrammen] wird die Erstellung von Datenflussdiagrammen beschrieben und eine Leseanleitung dazu geliefert.
