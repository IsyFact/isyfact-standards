...
Das Resultat des Tailoring ist eine spezifische Factory für einen Anwendungskontext, wie sie z.B. beim Bundesverwaltungsamt in Form der xref:literaturextern:inhalt.adoc#litextern-register-factory[Register Factory] für den Kontext der Register Anwendungen erstellt wurde.
