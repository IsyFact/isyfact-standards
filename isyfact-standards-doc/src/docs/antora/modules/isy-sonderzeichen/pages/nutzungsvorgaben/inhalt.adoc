...
die Encodierung für jeden Benutzer individuell gesetzt werden. Weitere Locale-Parameter und deren Verwendung sind bei xref:literaturextern:inhalt.adoc#litextern-linux-locale-parameters[gnu.org] nachzulesen.
...
Oracle unterstützt ab Version 10g Release 2 Unicode v4.0. Oracle empfiehlt xref:literaturextern:inhalt.adoc#litextern-oracle-globalization-support[Oracle, Globalization Support Guide 10g Release 2 (10.2)], neue Datenbanken als Unicode-Datenbanken anzulegen.
...
Damit die Zeichen in der Spalte „Glyph“ korrekt dargestellt werden, muss ein Font installiert sein, der alle Zeichen unterstützt. (z.B. Code2000, erhältlich unter xref:literaturextern:inhalt.adoc#litextern-code2000[Code2000]).
