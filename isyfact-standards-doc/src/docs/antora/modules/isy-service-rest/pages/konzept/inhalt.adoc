...
Die Fehlerbehandlung erfolgt nicht in der Service-Fassade, sondern an zentraler Stelle in einem separaten `RestExceptionHandler`, 
welcher die Exception Fassade der Service-Komponente darstellt (vgl. <<image-fehlerbehandlung>>). Dort werden alle Exceptions aus Methoden des 
Anwendungskerns oder Methoden der Service-Endpoints gefangen, gelogged, in Exception-Transport-Objekte umgewandelt und Response zurückgegeben.
