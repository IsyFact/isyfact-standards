...
NOTE: Für eine Liste der unterstützten Monitoring-Systeme siehe https://micrometer.io/docs/concepts#_supported_monitoring_systems[unterstützte Monitoring-Systeme]
...
Availability (Verfügbarkeit) eines Systems oder einer individuellen Systemkomponente ist definiert als der Prozentsatz eines Zeitraums, in welcher es ordnungsgemäß nach gesetzten Performance-Kriterien läuft (vgl. https://docs.oracle.com/cd/A97335_02/ias.102/a95201/avail.htm[Oracle: Definition von Availability]).
 +
 +
...
_Liveness_ und _Readiness_ wird stattdessen unter Availability zusammengefasst (vgl. https://docs.spring.io/spring-boot/docs/2.7.x/api/org/springframework/boot/availability/AvailabilityState.html[Spring Boot AvailabilityState]).
...
Availability wird zur quantitativen Messung der Resilienz eines Systems und der Feststellung des Erreichungsgrads eines angestrebten Resilienz-Ziels genutzt (vgl. https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/availability.html[aws: Berechnung von Availability]).
