...
==== Besonderheit bei einem Nachbarsystem-HealthIndicator
Bei und nur bei einer Nachbarsystemprüfung muss eine Korrelations-ID erzeugt und übergeben werden.
Der jeweilige `HealthIndicator` muss dazu zu Beginn eine Korrelations-ID erzeugen, die im Logging-Kontext gesetzt werden muss.
Bei jedem Aufruf an ein Nachbarsystem ist diese ID anschließend zu übergeben.
