...
[source,java]
----
include::{sourcedir}main/java/de/bund/bva/isyfact/ueberwachung/autoconfigure/IsyActuatorSecurityAutoConfiguration.java[tag=actuatorSecurity]
----
...
[[implementierung-von-liveness-und-readiness-Zustaenden]]
==== Implementierung von Liveness und Readiness Zuständen

Wie in Kapitel xref:konzept/master.adoc#informationen-ueber-den-systemzustand[Informationen über den Systemzustand] beschrieben sind _Liveness_ und _Readiness_ optionale Zustände, die einem feingranularen Monitoring dienen.

Anwendungen nach dem IsyFact Standard sollen Informationen über diese Zustände zur Verfügung stellen.
Diese sind aktuell konzeptionell beschrieben und nicht in der `isy-überwachung`-Bibliothek implementiert.
Die tatsächlich sinnvolle Nutzung der Zustände hängt stark von den betrieblichen Gegebenheiten und der zu überwachenden Anwendung ab.
Daher werden hier Vorgaben für die bedarfsgerechte Implementierung gemacht.

[[auslesen-von-liveness-und-readiness-probes]]
===== Auslesen von Liveness und Readiness Probes

_Liveness_ und _Readiness_ Probes können in der Anwendungskonfiguration aktiviert werden durch (vgl. https://www.baeldung.com/spring-liveness-readiness-probes[Baeldung: Liveness und Readiness Probes in Spring Boot]):
...
Damit werden die der Anwendung zur Verfügung stehenden Informationen genutzt, um den jeweiligen Zustand zu bestimmen und über den Spring `ApplicationContext` zu veröffentlichen.
...
Für den Liveness Zustand sind Abhängigkeiten, die eine Erweiterung notwendig machen, selten gegeben.
...
(vgl. https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.spring-application.application-availability.liveness[Spring Boot Liveness]).

Ist der _Liveness_-Zustand neben dem funktionierenden `ApplicationContext`  von weiteren, anwendungsspezifische Komponenten abhängig, muss dieser entsprechend aktualisiert werden.
Dies ist der Fall, wenn die Anwendung in einem Zustand sein kein, der einen Neustart notwendig macht.
...
Andernfalls könnte es sein, dass einem Fehlschlagen eines externen Systems (Datenbank, Web API, externer Cache, ...) ein Neustart des Systems folgt, welcher Fehlerkaskaden über die Plattform auslöst.
====

[[update-des-readiness-states]]
===== Update des Readiness States

Häufiger als beim _Liveness_-Zustand sind für das korrekte Abbilden der _Readiness_ einer Anwendung weitere Zustandsindikatoren notwendig.
Diese Zustandsindikatoren bilden Umständen ab die, die Fähigkeit der Anwendung Anfragen korrekt zu verarbeiten, beeinflussen.
Dies können anwendungsinterne Vorgänge sein, z.B. das vollständige Laden von notwendigen Daten oder die Erreichbarkeit angebundener externen Systeme.
Angebundene Systeme, die im Readiness-Zustand berücksichtigt werden müssen, sind die gemäß dem IsyFact Standard als essenziell eingestufte Nachbarsysteme (Vgl. xref:konzept/inhalt.adoc#nachbarsystemcheck-liveness-Readiness[Konzept-Essenzielle Nachbarsysteme].)
...
Sind weitere Indikatoren für den _Liveness_- oder _Readiness_-Zustand relevant, ist die Nutzung von Gruppen sinnvoll (Vgl. https://docs.spring.io/spring-boot/docs/2.2.x/reference/html/production-ready-features.html#health-groups[Spring Boot Health Groups]).
Dieses Vorgehen gewährleistet eine gute Übersicht und erleichtert die Veränderbarkeit der Überwachung, wenn auf Anwendungsänderungen oder veränderte Betriebsbedingungen zu reagieren ist.
Darüber hinaus lassen sich für die korrekte Bestimmung der Readiness vorkonfiguriert Informationen aus dem `HealthIndicator` angebundener Systeme integrieren, wenn diese als Abhängigkeiten im Klassenpfad vorkommen.
Eine Liste der vorkonfigurierten `HealthIndicators` ist in der https://docs.spring.io/spring-boot/reference/actuator/endpoints.html#actuator.endpoints.health.auto-configured-health-indicators[hier] einzusehen.
Wird die _Readiness_ des Systems von der Erreichbarkeit weiterer essenzieller Nachbarsysteme beeinflusst, sind diese als Komponenten zu implementieren.
Bei einer Verwendung der Bibliothek `isy-überwachung`  kann hierfür der `NachbarsystemIndicator` genutzt werden.
Sind weitere Komponenten zu implementieren, sind die Vorgaben des Kapitels <<implementierung-weiterer-eigener-health-indikatoren>> zu beachten.
In den `application.properties` ist der Indicator in der jeweiligen Gruppe hinzuzufügen:
...
public void onApplicationEvent(AbstractHealthIndicator healthIndicator) {
    // Log der Veränderung des Liveness- oder Readiness-Zustands
}
----
...
Die Konfiguration für die unterstützten Monitoring-Systeme ist in https://micrometer.io/docs/[Monitoring-Systeme] beschrieben.
