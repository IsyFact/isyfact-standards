...
* Verfügbarkeit weiterer genutzter Ressourcen, wie beispielsweise der IAM-Server oder genutzte FTP-Verzeichnisse.
...
[NOTE]
====
Als Beispiel sei hier der IAM-Server genannt. Zur Prüfung des IAM-Servers wird in der Regel eine Beispielanfrage
an den Server gesendet.
Ist vor den IAM-Server ein Loadbalancer geschaltet, so kann es nach einem Fail-Over passieren, dass diese
