...
* Der Zugriff auf Endpoints wird mit OAuth2-Token abgesichert.
...
Standardmäßig wird der Zugriff auf Endpoints durch OAuth2 Authentifizierung abgesichert, wenn die Dependencies:

 * `org.springframework.security:spring-security-config` und
 * `org.springframework.security:spring-security-web`

im Klassenpfad verfügbar sind. D.h. hier muss ein Token mitgeschickt werden, das auf Gütligkeit geprüft wird.

Es kann die Security-Konfiguration der Applikation benutzt werden. Dann ist die Absicherung der Monitoring-Endpunkte gleich der API-Endpunkte.
Existiert keine Standard-Absicherung oder soll gegen einen anderen Endpunkt validiert werden, muss das Property `isy.ueberwachung.security.jwk-set-uri` gesetzt werden.
...
[source,properties]
----
# OAuth2 Absicherung der Überwachungs-Endpunkte (Default: Null)
# OpenID Connect Zertifikats‑Endpoint
isy.ueberwachung.security.jwk-set-uri=https://<keycloak-uri>:<port>/auth/realms/<realm>/protocol/openid-connect/certs
----

Nicht abgesichert ist der Health-Endpoint.
