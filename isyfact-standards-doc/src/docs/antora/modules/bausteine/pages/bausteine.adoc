...
Diese Bausteine liegen in unterschiedlichen Formen vor: es gibt fachliche und technische xref:glossary::terms-definitions.adoc#service[Services] im Sinne einer Service-Orientierten Architektur (SOA), querschnittlich nutzbare Komponenten sowie wiederverwendbare Bibliotheken und Programmiervorlagen.
...
.Entkopplung der Bausteine
****
Mit Veröffentlichung der IsyFact 4 hat ein umfassendes, technisches Redesign der Bausteine begonnen, dessen Ziel die *Entkopplung* der Bausteine ist.
Das bedeutet, dass:

* Abhängigkeiten zwischen Bausteinen aufgelöst werden,
* Bausteine nur diejenigen Produkte verwenden, die durch ihren Zweck _direkt_ begründet sind, und
* Bausteine unabhängig voneinander weiterentwickelt und veröffentlicht werden.

Der letzte Punkt betrifft ebenso die Dokumentation, sodass entkoppelte Bausteine jeweils einen _eigenen Bereich mit eigener Navigation_ besitzen.
Dieser Umstand ist in der Navigation mit dem Symbol icon:external-link-square[] gekennzeichnet.

Aktuell sind alle Bausteine bis auf die Bausteine *Fehlerbehandlung*, *Logging* und *Überwachung* entkoppelt.
****
