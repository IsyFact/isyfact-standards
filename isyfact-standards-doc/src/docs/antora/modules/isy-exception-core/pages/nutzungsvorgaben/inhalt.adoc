...
Voraussetzung für das Erstellen dieser Exception sind die Basis-Exceptions der Anwendung (hier `MeineAnwendungBusinessException`).
...
Der Konstruktor der Exception ruft den Konstruktor der abstrakten Eltern-Klasse auf (hier `MeineAnwendungTechnicalRuntimeException`):

.Konstruktor
[id="listing-Konstruktur",reftext="{listing-caption} {counter:listings }"]
[source,java]
----
 public MeineAnwendungTechnicalRuntimeException(FehlerSchluessel schluessel, String... parameter) {
