...
* `GenerischeAnwendungException`: Abstrakte Oberklasse innerhalb einer Anwendung für checked Exceptions
* `GenerischeAnwendungTechnicalRuntimeException`: Abstrakte Oberklasse innerhalb einer Anwendung für unchecked Exceptions
* `GenerischeAnwendungBusinessException`: Kindklasse von `GenerischeAnwendungException` für fachliche Exceptions
* `GenerischeAnwendungTechnicalException`: Kindklasse von `GenerischeAnwendungException` für technische Exceptions
...
Die letztlich in einer Anwendung eingesetzten Exceptions werden dann von den genannten Klassen `GenerischeAnwendungBusinessException`, `GenerischeAnwendungTechnicalException` und `GenerischeAnwendungTechnicalRuntimeException` abgeleitet.
...
Das Beispiel in <<image-007>> zeigt eine fachliche Exception eines Generischen Anwendung.
Die fachliche Exception `GenerischeNichtGefundenException` besitzt in diesem Beispiel nicht alle möglichen Konstruktoren.
...
Voraussetzung für das Erstellen dieser Exception sind die Basis-Exceptions der Anwendung (hier `GenerischeAnwendungBusinessException`).
...
public static final String MSG_ALLGEMEINER_FEHLER = "GNRMDL90001";
----
...
[source,properties]
----
GNRMDL90001 = Es ist ein allgemeiner Fehler im Modul Generisches Modul aufgetreten.
----

Beim Einsatz von Exceptions muss immer eine Konstante zur Referenzierung von Fehlern verwendet werden.
Die Fehlertexte dürfen nicht direkt mit dem String referenziert werden (z. B. hier `GNRMDL90001`).
...
[source,java]
----
new GenerischeFehlerhafteKonfigurationException(
    FehlerSchluessel.KONFIGURATION_FALSCH_INITIALISIERT);
----

Der Konstruktor der Exception ruft den Konstruktor der abstrakten Eltern-Klasse auf (hier `GenerischeAnwendungTechnicalRuntimeException`):

.Konstruktor
[id="listing-Konstruktur",reftext="{listing-caption} {counter:listings }"]
[source,java]
----
 public GenerischeAnwendungTechnicalRuntimeException(FehlerSchluessel schluessel, String... parameter) {

        super(schluessel.getCode(), FEHLERTEXT_PROVIDER, parameter);
    }
----

Dieser Konstruktor wiederum ruft den Konstruktor seiner Eltern-Klasse auf (hier `GenerischeAnwendungException`), welcher die oberste Exception-Hierarchie-Stufe einer Anwendung darstellt:

.Konstruktor der obersten Exception
[id="listing-ObersterKonstruktur",reftext="{listing-caption} {counter:listings }"]
[source,java]
----
  protected GenerischeAnwendungException(String ausnahmeId, FehlertextProvider fehlertextProvider, String... parameter) {
...
Die `GenerischeAnwendungException` hält eine Referenz zu einem <<fehlertextprovider>>, welcher die Möglichkeit bietet Fehlertexte auszulesen.
...
----
try {
    verwaltung.leseGenerischeNummer(generischeNummer);
} catch (GenerischeNichtGefundenException ex) {
