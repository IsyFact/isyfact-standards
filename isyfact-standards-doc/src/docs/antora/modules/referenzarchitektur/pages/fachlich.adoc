...
Eine Anwendungsdomäne gruppiert fachlich zusammengehörende Anwendungen zur Unterstützung von Geschäftsprozessen, die als eine Einheit angesehen werden können.
In <<awl-fachlich,der obigen Abbildung>> wurden zwei Domänen gebildet.

Zwischen den Anwendungen unterschiedlicher Anwendungsdomänen sollten nur wenige, klar definierte Nutzungsbeziehungen existieren.

*xref:glossary:glossary:master.adoc#glossar-anwendung[Anwendung]:* Eine Anwendung ist eine zusammengehörende, logische Einheit aus Funktionen, Daten und Schnittstellen.
Anwendungen unterstützen Geschäftsprozesse.
Anwendungen sind einer Anwendungsdomäne zugeordnet und setzen sich aus Anwendungskomponenten zusammen.
Eine Anwendung ist Bestandteil der xref:glossary:glossary:master.adoc#glossar-a-architektur[A-Architektur].
Aus Sicht der xref:glossary:glossary:master.adoc#glossar-t-architektur[T-Architektur] bildet ein xref:glossary:glossary:master.adoc#glossar-it-system[IT-System] die software-technische Umsetzung einer Anwendung ab.
...
*xref:glossary:glossary:master.adoc#glossar-querschnittsanwendung[Querschnittsanwendung]:* Eine Querschnittsanwendung ist eine Anwendung, die Services für mehrere Geschäftsanwendungen bereitstellt, wie z.B. ein Behördenverzeichnis oder ein Output-Management.
