...
Sie müssen von der jeweiligen Anwendung gegeben werden, da für diese unterschiedlichen Bedürfnisse an die Verfügbarkeit der jeweiligen Anwendung bestehen.
Sind die nicht-funktionalen Kriterien erfüllt, dann ist die Anwendung/das System verfügbar (_available_). Verfügbarkeit als Messung des _available_-Zustands über die Zeit stellt entsprechend dar, in welchem zeitlichen Umfang die nicht-funktionalen Kriterien erfüllt sind.
