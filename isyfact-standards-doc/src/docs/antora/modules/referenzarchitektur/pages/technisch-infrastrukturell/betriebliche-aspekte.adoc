...
Der Zustand eines IT-Systems wird in der Datenbank persistiert und bei jedem eintreffenden Service-Aufruf aus dieser gelesen und bei Beendigung des Service-Aufrufs wieder in die Datenbank geschrieben.
...
Bei der Software wird eine ablauffähige Einheit (JAR-Paket) ausgeliefert.
Dieses JAR-Paket wird unter Angabe der Parameter installiert.
...
Auf die Überwachung der IT-Systeme wird im Dokument xref:ueberwachung::konzept.adoc#einleitung[Konzept Überwachung] im Detail eingegangen.
...
Es müssen Abhängigkeitsgrafen für die Rechnersysteme und IT-Systeme der Anwendungen erstellt werden, an Hand derer das zentrale Überwachungstool sinnvolle Alarmierungen vornehmen kann.
...
Die Geschäftsdaten aller IT-Systeme der Anwendungen einer IsyFact-Systemumgebung werden in relationalen Datenbanken gehalten.
...
Fällt die Primärdatenbank aus, kann ihre Aufgabe von der Standby-Datenbank übernommen werden (Failover). Die Verfügbarkeit der IT-Systeme wird dadurch deutlich erhöht, denn das Failover dauert nur Sekunden oder Minuten, während das Zurückspielen einer Datensicherung (Restore) in der Regel deutlich länger dauern wird.
