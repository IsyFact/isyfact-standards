...
Im Folgenden wird auf wichtige Aspekte des Betriebs einer Systemlandschaft eingegangen.
...
Eine Beispielrechnung findet sich im xref:blaupausen:detailkonzept-komponente-service/inhalt.adoc#verfuegbarkeit[Detailkonzept Service].
...
Die Zustandslosigkeit der Applikationsserver (s. xref:technisch-infrastrukturell.adoc[]) unterstützt diese Redundanz und das Failover.
