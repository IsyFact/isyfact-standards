...
Die Funktionsweise des Batchrahmens und die verschiedenen Startarten sowie die Konfiguration folgt in Kapitel xref:software-technisch/batch/batchrahmen.adoc#der-batchrahmen["Der Batchrahmen"].
...
Das Kapitel xref:software-technisch/batch/batchrahmen.adoc#die-ausfuehrungsbeans["Die Ausführungsbeans"] beschreibt schließlich, wie die konkrete Batchlogik über Ausführungsbeans angebunden wird.
...
image::software-technisch/batch/architektur-it-system-batch-parts.dn.svg[]
...
Die meisten Batch-Schritte sind keine Anwendungen: Sie werdn komplett über ein Skript umgesetzt.
