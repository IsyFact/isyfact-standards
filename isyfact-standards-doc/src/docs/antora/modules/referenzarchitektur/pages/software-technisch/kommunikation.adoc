...
Die Kommunikation mit xref:glossary::terms-definitions.adoc#externes-system[externen Systemen] basiert auf Services, die beispielsweise über REST- oder SOAP-Schnittstellen bereitgestellt werden.
...
Aufruf von Services der Systemlandschaft:: Durch die Systemlandschaft wird externen Systemen die Schnittstelle eines Backends durch ein xref:glossary::terms-definitions.adoc#glossar-service-gateway[Service-Gateway] in Form eines xref:software-technisch/gateway.adoc#ingress-gateway[Ingress-Gateways] bereitgestellt. Das Backend muss hierzu nicht angepasst werden.
