...
Dies entkoppelt Sender und Empfänger voneinander und verhindert, dass Sender bis zum Erhalt einer Antwort blockieren.
Da der Sender keine direkte Antwort vom Empfänger erhält, muss er sich das Ergebnis seiner Nachricht über eine weitere Queue schicken oder mittels eines der anderen Kommunikationsmuster übermitteln lassen.

[[message-queue-sequence]]
.Ablauf der Kommunikation bei Message-Queue
image::software-technisch/queueing.dn.svg[]

[[message-queue-betrachtung]]
==== Betrachtung der Vorteile, Herausforderungen und Risiken

Der Einsatz von Message Queues kann die Erfüllung der folgenden Qualitätskriterien fördern.

.Message Queue: Vorteile nach Qualitätskriterien (ISO 25010)
...
|Message-Broker speichern Nachrichten persistent und überbrücken so Ausfälle der Empfänger. +
Message-Broker übernehmen Wiederholungsversuche, kümmern sich um unzustellbare Nachrichten und garantieren die Zustellung der Nachricht.
...
Message-Broker ermöglichen Lastverteilung und parallele Verarbeitung durch die Empfänger.

|Wartbarkeit (Maintainability)
|Sender und Empfänger sind lose miteinander gekoppelt.
Änderungen an Sender oder Empfänger sind isoliert möglich, solange das Nachrichtenformat stabil bleibt.

|===

Demgegenüber kann der Einsatz von Message Queues zu Herausforderungen und Risiken bei folgenden Qualitätskriterien führen.

.Message Queue: Herausforderungen und Risiken nach Qualitätskriterien (ISO 25010)
[cols="1,2", options="header"]
|===
|Qualitätskriterium
...
a|Message Queues sind nicht für Interaktionen geeignet, die:

* eine direkte Antwort erwarten,
* hohe Anforderung an das Zeitverhalten haben,
* von einer festgelegten Reihenfolge der Verarbeitung abhängig sind.

Solche Interaktionen können umgesetzt werden, erfordern aber zusätzliche Mechanismen zur Korrelation und Rückmeldung.

|Sicherheit (Security)
|Neben Sender und Empfänger muss auch der Message-Broker abgesichert werden, da er Nachrichten von vielen Systemen verarbeitet und speichert. +
Nachrichten können potenziell im Message-Broker abgefangen oder manipuliert werden.
Zur Minderung dieser Risiken sind zusätzliche Maßnahmen wie Signierung, Verschlüsselung oder Zugriffskontrollen nötig.

|Zuverlässigkeit (Reliability)
|Message-Broker sind zentrale Infrastrukturkomponenten.
Ein Ausfall des Brokers kann zu einem vollständigen Kommunikationsstillstand führen.
Auch temporäre Störungen wirken sich direkt auf die Verarbeitung aus.
Zur Minderung dieser Risiken sollte der Message-Broker als Cluster betrieben werden.
Außerdem sollten Maßnahmen gegen eine Überlastung des Brokers getroffen werden.
|===


[[message-queue-empfehlungen]]
==== Empfehlungen zum Einsatz

Das Kommunikationsmuster Message-Queue eignet sich für Interaktionen, die keine direkte Antwort benötigen und keine hohen Anforderungen an das Zeitverhalten stellen.

Weiterhin sollten Message-Broker nach dem Prinzip _at-leat-once_ funktionieren: Die Zustellung der Nachrichten wird garantiert, aber Duplikate sind möglich.
