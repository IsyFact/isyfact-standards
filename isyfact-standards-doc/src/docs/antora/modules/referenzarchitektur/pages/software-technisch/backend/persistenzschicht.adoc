...
Die Struktur der Daten, die von einem Backend dauerhaft gespeichert werden, kann sich im Laufe des Lebenszyklus des Backends ändern.
Das bedeutet, dass sich neben dem Backend auch das Datenbankschema ändert.
Das Backend und das Datenbankschema müssen zueinander passen.

Die Verwaltung von Versionsinformationen für ein Datenbankschema soll sicherstellen, dass das Backend und Skripte (zum Beispiel zur Datenmigration) erkennen können, ob ein Datenbankschema die erwartete Version hat.
...
Logging::
Beim Logging werden Notizen zu technischen Aufrufen innerhalb eines IT-Systems oder zwischen IT-Systemen in Dateien abgelegt.
...
Für die Historisierung von Datensätzen in einem Backend gelten folgende Anforderungen und Grundsätze:
