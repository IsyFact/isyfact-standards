...
In Ausnahmefällen kann die Transaktionssteuerung auch über die xref:blaupausen:detailkonzept-komponente-service/inhalt.adoc#transaktionssteuerung[Serviceschicht] erfolgen.
...
Zur Konzeption von Services definiert die Referenzarchitektur xref:blaupausen:detailkonzept-komponente-service/inhalt.adoc[weitere Vorgaben].
