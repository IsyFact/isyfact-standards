...
Die folgenden Vorgaben für Queries verhindern negative Auswirkungen auf die Stabilität, Verfügbarkeit oder Sicherheit von Backends:

* Der `%`-Operator ist nach Möglichkeit zu vermeiden.
Es können leicht lang laufende Queries entstehen, welche die Backends blockieren und die Datenbank unnötig belasten.
...
Wenn in einem Backend viele wiederkehrende Abfragen auf Entitäten erfolgen, können Hibernate Filter eingesetzt werden, um die "WHERE-Klauseln" der Abfragen zu vereinfachen oder zu ersetzen.
...
Im folgenden Beispiel würde ihr Einsatz dazu führen, dass das Backend generell keine gelöschten Items abfragen könnte.
...
Für künstliche IDs stehen zwei Strategien zur Verfügung:
sequenzbasierte numerische IDs und UUID-basierte IDs.
Die Wahl der Strategie betrifft die technische Primärschlüssel-Generierung in der Persistenzschicht und ist getrennt von der Frage zu behandeln, welcher Identifikator nach außen (REST, Events, Logs) exponiert wird (siehe <<oeffentliche-kennung>>).

.icon:shield[title=Sicherheit] Die Verwendung von UUIDs ersetzt keine Autorisierungsprüfung.
****
Ein Backend, das Ressourcen ohne Prüfung der Zugriffsberechtigung anhand des übergebenen Identifikators ausliefert, ist unabhängig vom ID-Schema fehlerhaft.
UUIDs reduzieren die Angreifbarkeit durch Enumeration, sie stellen keinen Ersatz für die vorgeschriebene Autorisierung an der Service- bzw. Use-Case-Grenze dar.
****

.Künstliche IDs: Vor- und Nachteile nach Strategie
[cols="1,2,2", options="header"]
|===
|Kriterium |Sequenz (numerisch) |UUID

|Erkennbarkeit / Enumeration
|Werte sind fortlaufend und vorhersagbar
|Werte sind praktisch nicht zu erraten

|Index-Verhalten (Schreiben)
|Anfügend, keine Fragmentierung
|Bei UUID v4 (zufällig) zufällige Einfügepositionen, Fragmentierung des B-Tree-Index.
Bei UUID v7 (zeitbasiert, RFC 9562) weitgehend anfügendes Verhalten, vergleichbar mit Sequenzen.
...
|Keine
|UUID v4: +
keine `UUID.randomUUID()`. +
 UUID v7: +
abhängig von der eingesetzten Hibernate-Version.
|===

.icon:university[title=Architekturregel] Wahl der künstlichen-ID-Strategie
****
Für Entitäten, deren künstliche ID an einer Systemgrenze (REST-Pfad, Event, Nachricht) sichtbar wird, ist eine separate, UUID-basierte öffentliche Kennung zu vergeben.
Die interne technische ID (Primärschlüssel in der Persistenzschicht) kann unverändert sequenzbasiert bleiben, sofern sie die Systemgrenze nicht verlässt.

Für neue Entitäten ohne Verwendung eines sequenzbasierten Primärschlüssels in bestehenden Fremdschlüsselbeziehungen kann der Primärschlüssel selbst als UUID angelegt werden.
Bei zeitkritischen Massendaten ist UUID Version 7 der Version 4 vorzuziehen.
****

Sequenzbasierte künstliche Primärschlüssel gelten ab als deprecated für neu exponierte Identifikatoren.
Bestehende, ausschließlich intern verwendete Primärschlüssel (z. B. in n:m-Zuordnungstabellen ohne externe Sichtbarkeit) sind von dieser Regelung nicht betroffen.

.Konfiguration einer UUID als künstliche ID (Beispiel)
[source,java]
----
@Entity
public class MyEntity {
    ...
    @Id
    @GeneratedValue
    @UuidGenerator // Default: UUID v4 (RANDOM)
    private UUID id;

}
----

[[oeffentliche-kennung]]
.Konfiguration einer separaten öffentlichen Kennung bei sequenzbasiertem Primärschlüssel
[source,java]
----
@Entity
public class MyEntity {
    ...
    private long id; // ausschließlich technisch, der die Persistenzschicht nicht verlässt

    @Column(name = "oeffentliche_kennung", unique = true, nullable = false)
    @UuidGenerator
    private UUID oeffentlicheKennung; // in Verwendung an einer Systemgrenze ( z. B. REST-Pfad, Event, Nachricht) zu verwenden
...
Wird vom Backend eine Sortierung benötigt und sind alle für die Sortierung benötigten Attribute in der Entität enthalten, dann kann auch eine Liste (`List`) verwendet werden, da die Datenbank effizienter sortieren kann als eine Java-Implementierung.
...
Hinweise zu deren Verwendung finden sich im xref:datetime::konzept.adoc[Baustein Datum & Zeit].
...
====
Für alte Backends, die nicht die _Java 8 Date Time API_, sondern noch `java.util.Date` verwenden, gelten die folgenden Vorgaben.
...
Die <<konfiguration-automatische-erzeugung-schema,Konfiguration>> hierzu geschieht in der Datei `application.properties` des Backends.
...
Befindet sich das Backend in Produktion, dann muss die automatische Erzeugung von Datenbankschemas abgeschaltet sein.
...
Fehlende Indizes führen häufig zu einer schlechten Performance des Backends und belasten die Datenbank durch das vermehrte Auftreten von Full-Table-Scans sehr stark.
