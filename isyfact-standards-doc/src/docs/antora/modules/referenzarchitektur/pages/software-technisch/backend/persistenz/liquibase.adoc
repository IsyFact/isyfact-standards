...
Eine konkrete Umsetzung dieses Konzepts folgt in xref:isy-persistence:nutzungsvorgaben/versionierung-mit-liquibase.adoc[Nutzungsvorgaben zur Versionierung mit Liquibase] und dient als Empfehlung.
...
Vor der Einführung von Liquibase in die IsyFact gab es eine eigens entwickelte xref:isy-persistence:konzept/versionierung.adoc[Versionierung], die den aktuellen Zustand der Datenbank dokumentiert hat.
Mit Liquibase wird ein Industriestandard verwendet, dessen Verwendung die Zukunftssicherheit verbessert und Abhängigkeiten von Eigenentwicklungen verringert.
