...
[WARNING]
====
Seit Liquibase 4.30.0 werden standardmäßig Analysedaten an Fremdserver übermittelt.
Um die Funktion zu deaktivieren, muss in der Konfiguration `liquibase.analyticsEnabled=false` beziehungsweise die Umgebungsvariable `LIQUIBASE_ANALYTICS_ENABLED=false` gesetzt werden.
====
...
Liquibase kann direkt in ein Backend integriert und kann automatisch mit dieser gestartet werden.
...
Liquibase nutzt JDBC für die Datenbankkommunikation in Java- beziehungsweise Spring-basierten Backends.
Dabei sind folgende Punkte zu beachten:

* Sicherstellen, dass sowohl Liquibase als auch das Backend über JDBC mit der Datenbank kommunizieren können.
* Bereitstellung eines geeigneten Treibers für das verwendete Datenbanksystem.
* Zentrale Verwaltung der Verbindungsinformationen innerhalb des Backends, um eine konsistente und sichere Nutzung der Datenbank zu gewährleisten.
...
* Darüber hinaus unterstützt Liquibase https://docs.liquibase.com/user-guide/what-are-preconditions[Preconditions].
