...
Sie zeigt ebenso die Verwendungen und Aufrufbeziehungen innerhalb einer Fachkomponente, sowie die Aufrufbeziehungen zu anderen Fachkomponenten und die Anbindung von Services anderer IT-Systeme über Service Consumer.
...
Bei komplexen Datenmodellen hat es sich bewährt, zwischen den Geschäftsobjekten zu unterscheiden, die für den Aufruf der Fachkomponente (Komponente `input` oder `eingabedaten`) verwendet werden, und denen, die von der Fachkomponente zurückgegeben werden (Komponente `output` oder `ausgabedaten`).
...
Falls Anwendungsfallklassen andere Fachkomponenten desselben Backends aufrufen, so geschieht dies direkt über die Schnittstellen dieser Fachkomponenten.
Benötigen Anwendungsfallklassen Funktionalität, die in anderen IT-Systemen umgesetzt ist, so geschieht dies über den jeweiligen Service Consumer im Anwendungskern.
Schließlich rufen Anwendungsfallklassen zur Persistierung ihrer Geschäftsobjekte die Persistenzschicht auf.
...
Service Consumer enthalten somit eine Schnittstelle und einen Adapter, der die Kommunikation mit dem Service des anderen IT-Systems übernimmt.
Die Schnittstelle ist gleichartig zur Fassade einer Fachkomponente des Anwendungskerns aufgebaut.
Der Adapter enthält die technische Logik, um mit dem Service zu kommunizieren, kapselt die Details der Kommunikation und übernimmt Aspekte wie Fehlerbehandlung und Logging.
Der <<image-aufbau-service-consumer>> fasst den Aufbau grafisch zusammen.

[[image-aufbau-service-consumer]]
.Aufbau eines Service Consumers
image::software-technisch/backend/aufbau_service_consumer.dn.svg[]
...
Fachkomponenten und Service Consumer werden mittels Spring konfiguriert.
Das umfasst vor allem die Fassade (Schnittstellen der Fachkomponente bzw. des Service Consumers) und ihre Implementierung, da diese für andere Fachkomponenten im Anwendungskern und darüber hinaus auffindbar sein müssen.

Bei den weiteren Bestandteilen einer Fachkomponente (Anwendungsfall-, Anwendungsfunktion- oder Hilfsklassen) oder eines Services Consumers (Service-Adapter) ist je nach Relevanz zu entscheiden, ob diese als eigene Spring Bean definiert werden.
Im Zweifel sollten die Klassen ohne Spring implementiert werden.
...
Ist eine solche Steuerung trotzdem unumgänglich, muss sie über die xref:software-technisch/backend/service/rest-querschnitt.adoc#transaktionssteuerung[Serviceschicht] erfolgen.
