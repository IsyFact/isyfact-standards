...
Entsprechend der xref:software-technisch/backend.adoc[] stellen Backends Services über die Serviceschicht zur Verfügung.
Zu beachten ist, dass die Services eines Backends somit lediglich innerhalb der Systemlandschaft verfügbar sind.
...
Die _Service-Fassade_ übernimmt darüber hinaus neben der Validierung der Aufrufparameter auch die Autorisierung des Aufrufs.
