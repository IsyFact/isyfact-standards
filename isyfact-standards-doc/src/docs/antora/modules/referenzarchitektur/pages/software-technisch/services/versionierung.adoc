...
Die Kommunikation zwischen IT-Systemen ist abwärtskompatibel, wenn Empfänger (von Services, Nachrichten oder Events) ohne Anpassung auch mit neuen Versionen derselben Services, Nachrichten oder Events kommunizieren können.
Diese Eigenschaft von Services ist wichtig, um die Auswirkungen von Änderungen auf die Empfänger zu minimieren.
Abwärtskompatibilität ermöglicht Übergangszeiträume, in denen sich die Empfänger schrittweise an die neuen Versionen anpassen können, ohne dass es zu Unterbrechungen oder Fehlern in der Kommunikation kommt.
Gleichzeitig befreit sie Sender davon, mehrere Versionen eines Services anbieten zu müssen, oder mehrere Versionen von Nachrichten oder Events zu versenden, um Empfänger auf unterschiedlichen Versionsständen zu bedienen.
