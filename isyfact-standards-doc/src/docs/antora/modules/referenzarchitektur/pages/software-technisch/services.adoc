...
Die Beschreibung beinhaltet eine vollständige Liste aller kompatibler Änderungen mit konkreten Details, wie zum Beispiel neue Servicemethoden, zusätzliche Attribute, geänderte Iniialisierungswert, o.ä., so dass der Leser sich ein genaues Bild der Versionsänderungen machen kann, um seine Anwendung und Implementierung zur Nutzung der verschiedenen Service-Versionen anzupassen.
...
Auch hier beinhaltet die Beschreibung eine vollständige Lister aller inkompatibler Änderungen mit konkreten Details, wie zum Beispiel entfallene Servicemethoden, entfernte Attribute, geänderte Datentypen, o.ä., so dass der Leser nachvollziehen kann, welche Anpassungen in seiner Anwendung und Implementierung notwendig werden, um die verschiedenen Service-Versionen zu nutzen.

In komplexen Fällen kann es auch notwendig sein, den Anwendungskern so zu erweitern, dass die Logik der komplexen Transformationen zwischen einzelnen Versionen nur dort enthalten sind.
...
Das Backend bietet dafür aber anstatt zwei Serviceversionen 1.0 und 1.1 lediglich die Version 1.1 an.
Denn bei rein abwärtskompatiblen Änderungen einer Serviceversion muss diese neue Version sicherstellen, dass Clients, die den Backendservice auf Basis der Version 1.0 aufrufen, mit der Backendservice-Version 1.1 fehlerfrei arbeiten.
