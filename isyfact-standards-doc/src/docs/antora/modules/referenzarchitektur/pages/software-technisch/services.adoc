...
Ein zentrales Element der technischen Architektur ist die Wahl geeigneter Kommunikationsmodelle zwischen IT-Systemen.
Dabei wird zwischen *synchroner Kommunikation* (meist direkte Anfrage-Antwort-Interaktionen) und *asynchroner Kommunikation* (meist zeitversetzte Verarbeitung ohne unmittelbare Antwort) unterschieden.
Die Wahl des Kommunikationsmodells beeinflusst direkt die technische Architektur und Umsetzung der IT-Systeme.
Das gewählte Kommunikationsmodell trägt maßgeblich dazu bei, die Qualitätskriterien der Architektur und der IT-Systeme zu erfüllen.

Die Referenzarchitektur der IsyFact empfiehlt grundsätzlich den Einsatz asynchroner Kommunikation, insbesondere bei der Umsetzung verteilter Systeme, der Verarbeitung großer Datenmengen und der Realisierung skalierbarer Architekturen.
Die Entscheidung für ein Kommunikationsmodell muss jedoch kontextabhängig und anhand der geforderten Qualitätskriterien der Architektur und der IT-Systeme erfolgen.
