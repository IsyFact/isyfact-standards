...
Eine "pro Service Versionierung" bedeutet, dass ein Service eines IT-Systems, egal wieviele Service-Methoden dieser Service hat und wieviele davon geändert wurden, jede fachliche und technische Änderung im Service-Code bedarf einer neuen Service-Version.
Eine Service-Version wird anhand gewisser Kriterien unterteilt in Major-, Minor- und BugFix-Version, das sich auch in der Schreibweise der Service-Versionierung (z.B. 1.0.0) ausdrückt.

Dann besteht die Notwendigkeit, Services auch in mehreren Versionen anzubieten, da bei einer Vielzahl an Service-Nutzern nicht alle zeitgleich auf die neue Version eines Service umschalten können.
...
Für jede neu bereitgestellte Version einer Schnittstelle im Backend und/oder ServiceGateway muss eine neue Schnittstellen-Beschreibung erstellt werden, die dem Anwender:

* die neuen Möglichkeiten, bzw. die veränderte Nutzbarkeit bereits bestehender Möglichkeiten der Schnittstelle erklärt.
Die Beschreibung beinhaltet eine vollständige Liste aller Änderungen mit konkreten Details, wie zum Beispiel welche Servicemethoden kamen neu hinzu, sind entfallen oder wurden umbenannt, welche Attribute wurden verändert in Bezug auf ihren Namen, ihren Datentyp bzw. ob optional oder mandatorisch, ihren Default-Wert, o.ä.
...
** Als kompatible Änderungen wird das Hinzufügen neuer Servicemethoden, neuer optionaler Attribute, geänderte Default-Werte, o.ä. eingestuft.

** Als inkompatible Änderungen wird das Löschen und Umbennen von Servicemethoden oder Attributen, das Ändern von Attributen von optional auf mandatorisch, die Änderung von Attribut-Datentypen bzw. -Datenstrukturen, o.ä. gewertet.
...
Im Service-Gateway erfolgt daher immer nur ein einfaches Mapping auf die entsprechenden Service-Schnittstellen-Version der angebundenen Backend-Komponenten.
Der Ausgleich der Versionsunterschiede erfolgt ausschließlich im Backend - sprich optionaler Service-Schicht und/oder Anwendungskern - und nicht im Service-Gateway.
...
Erfährt ein Service nur abwärtskompatible Änderungen, wie neu hinzugekommene Servicemethoden, zusätzliche Attribute, o.ä., dann wird dies über eine neue Minor-Version bei gleichbleibender Major-Version des Service bewerkstelligt.
...
Denn bei rein abwärtskompatiblen Änderungen einer Serviceversion muss diese neue Version sicherstellen, dass Clients, die den Backendservice allein auf Basis der Major-Versionsangabe - hier Version 1 - auswählen und aufrufen, auch mit einem Aufruf Backendservice-Version 1.0 fehlerfrei arbeiten.

Das bedeutet zum Beispiel in einem Service wird mit Version 1.1 ein zusätzliches Attribut für MethodeA eingeführt.
Wenn nun ein Client die Service-MethodeA gemäß Major-Version 1 aufruft und dabei aber die Servicesignatur von Version 1.0 erfüllt, dann ist das neu hinzugekommene Attribut gemäß Version 1.1 nicht in dem Aufruf des Clients enthalten.
Die Transformation eines Aufrufs der Service-MethodeA gemäß Version 1.0 muss nun das fehlende Attribut ergänzen und mit einem Default-Wert initialisieren, bevor die Verarbeitung in der Fachkomponente mit Version 1.1 stattfindet oder die Fachkomponente selbst akzeptiert mit Version 1.1 dieses neue Attribut als optional.
...
Dieser wird als Komponente des Backends im Anwendungskern implementiert.
...
Dazu wird ein neuer Service gleichen Namens als Transformationskomponente innerhalb der Serviceschicht für Version 1.0 implementiert, der die Meldung gemäß Signatur der Version 1.0 entgegennimmt, das fehlende Attribut mit dem Wert "unbekannt" ergänzt und dann im Anwendungskern den eigentlichen Service mit der Signatur der Version 1.1 aufruft.

Werden die beiden Services durch ein Service-Gateway nach außen verfügbar gemacht, existieren dort zwei parallele Mappings auf die jeweiligen Services des Backends - Mapping 1.0 auf neue Komponente der Serviceschicht und Mapping 1.1 auf bereits existierende und modifizierte Komponente 1.1 im Anwendungskern.
...
Bei inkompatiblen Veränderungen eines Service, zum Beispiel durch Wegfall von Servicemethoden, Änderung von Parameterdatentypen in einer bestehenden Servicemethoden Signatur, o.ä. ist es notwendig eine neue Major-Version des Service bereit zu stellen.

Da nicht alle Clients gleichzeitig ihre Anpassungen an eine neu bereitgestellte Service-Version durchführen können, bedarf es der Notwendigkeit mehrere Service-Major-Versionen parallel im Backend zu betreiben.

Dadurch erwächst die Anforderung, dass ein Client die von ihm aufzurufende Service-Version auswählen können muss.
...
Mit Angabe der Service-Version im Client-Aufruf wird die Wahl der gewünschten Service-Version, die im Backend ausgeführt werden soll, getroffen.

Ruft ein Client nun einen Service mit einer Version auf, die im Backend nicht mehr zur Verfügung steht, so bekommt der Client eine Fehlermeldung, dass dieser Service mit der gewünschten Version nicht mehr erreichbar ist.
