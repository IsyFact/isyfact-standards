...
Eine "pro Service Versionierung" bedeutet, dass für jeden Service eines IT-Systems, egal wieviele Service-Methoden ein Service hat und wieviele davon geändert wurden, alle fachlichen und technischen Änderungen eines Service je eine eigene Service-Version benötigt.
Eine Service-Version wird anhand gewisser Kriterien in Major- und Minor-Version unterteilt, das sich auch in der Schreibweise der Service-Versionierung (z.B. 1.0) ausdrückt.

Es besteht die Notwendigkeit, Services auch in mehreren Versionen anzubieten, da bei einer Vielzahl an Service-Nutzern nicht alle zeitgleich auf die neue Version eines Service umschalten können.
...
Für jede neu bereitgestellte Version eines Service im Backend und/oder Service-Gateway muss eine neue Schnittstellen-Beschreibung erstellt werden, die dem Anwender:

* alle Änderungen in der neuen Service-Version beschreibt.
Die Beschreibung beinhaltet dabei eine vollständige Liste aller Schnittstellen-Änderungen mit konkreten Details, wie zum Beispiel welche Servicemethoden neu hinzukamen, entfallen sind oder umbenannt wurden, welche Parameter-Attribute verändert wurden in Bezug auf ihren Namen, ihren Datentyp bzw. ob optional oder mandatorisch, ihren Default-Wert, o.ä. sowie im Falle von Logik-Änderungen alle zu deren Verwendung dafür notwendigen Einzelheiten.
...
Eine neue Service-Version mit mindestens einer inkompatiblen Änderung, bedarf auf jeden Fall einer neuen Major-Version und erfordert Anpassungen auf Seiten des Aufrufers.
...
Denn bei rein abwärtskompatiblen Änderungen einer Serviceversion muss diese neue Version sicherstellen, dass Clients, die den Service allein auf Basis der Major-Versionsangabe - hier Version 1 - auswählen und aufrufen, auch mit einem Aufruf der Service-Version 1.0 fehlerfrei arbeiten.

[[image-minorversServ]]
.Beispiel minor versionierter Service
image::referenzarchitektur:software-technisch/services/simple-minor-versionierung.dn.svg[]
...
Die Transformation eines Aufrufs der Service-MethodeA gemäß Version 1.0 muss nun das fehlende Attribut ergänzen und mit einem Default-Wert initialisieren, bevor die Verarbeitung in der Fachkomponente mit Version 1.1 stattfindet.
Oder die Fachkomponente selbst im Anwendungskern akzeptiert mit Version 1.1 dieses neue Attribut als optional, wodurch bei derart einfach kompatiblen Änderungen dann keine extra Transformations-Komponente in der Service-Schicht notwendig ist, wie das folgende xref:image-veryminorversServ[] zeigt.

[[image-veryminorversServ]]
.Beispiel sehr einfach minor versionierter Service
image::referenzarchitektur:software-technisch/services/very-simple-minor-versionierung.dn.svg[]

Werden die beiden Service-Versionen durch ein Service-Gateway nach außen verfügbar gemacht, existieren dort zwei parallele Mappings auf die jeweiligen Services des Backends - Mapping 1.0 auf die neue Komponente der Serviceschicht und Mapping 1.1 auf bereits existierende und modifizierte Komponente 1.1 im Anwendungskern.
Innerhalb des Service-Gateways existiert keine Geschäftslogik, d.h. die Abbildung von Version 1.0 auf 1.1 findet erst im Backend - entweder in der Service-Schicht oder im Anwendungskern - statt.
...
Ruft ein Client nun einen Service mit einer Version auf, die im Backend nicht mehr zur Verfügung steht - zum Beispiel Wegfall von V1 wie in xref:image-major3versServ[] gezeigt, so bekommt der Client eine Fehlermeldung, dass dieser Service mit der gewünschten Version V1 nicht mehr erreichbar ist.

[[image-major3versServ]]
.Beispiel Wegfall major versionierter Service
image::referenzarchitektur:software-technisch/services/major-versionierung-3.dn.svg[]
