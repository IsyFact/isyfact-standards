...
Das Backend bietet im Anwendungskern als Fachkomponente dafür aber anstatt zwei parallel betriebene Serviceversionen 1.0 und 1.1 lediglich die eine neue Version 1.1 an.
Denn bei rein abwärtskompatiblen Änderungen einer Serviceversion muss diese neue Version sicherstellen, dass Clients, die den Backendservice allein auf Basis der Major-Versionsangabe - hier Version 1 - auswählen und aufrufen, auch mit der Backendservice-Version 1.1 fehlerfrei arbeiten.
