...
Alle Schnittstellen eines Backends sollen in einer einzigen OpenAPI-Spezifikation beschrieben werden.
...
====
*Anforderung:* Das Backend erwartet bei jedem Aufruf einer Schnittstelle eine Externe ID.
...
====
*Anforderung:* Das Backend protokolliert bei jedem Aufruf einer Schnittstelle den Zugriffsgrund.
...
Die Änderungen werden in Form eines Changelogs in einer Markdown-Datei dokumentiert und neben der OpenAPI-Spezifikation abgelegt.
Für die korrekte Zuordnung des Changelogs sollte der gleiche Dateiname verwendet werden (Beispiel: `openapi.md`).
Das Changelog ist in der Asciidoc-Schnittstellendokumentation als Anhang einzubinden.

[[veroeffentlichung]]
== Veröffentlichung von OpenAPI-Spezifikationen
Die OpenAPI-Spezifikationen werden mit jeder Veröffentlichung (Release) des Backends ausgeliefert.
...
Die generierten AsciiDoc-Dateien können dann in Antora aufgenommen und gemeinsam mit der übrigen Dokumentation des Bausteins oder des Backends generiert und veröffentlicht werden.

Es empfehlt es sich auch, sowohl die OpenAPI-Spezifikation im YAML-Format als auch die Changelog-Datei in Antora zu verlinken.
...
Über einen Webbrowser kann so auf die aktuelle OpenAPI-Spezifikation des Backends im Betrieb zugegriffen werden, um z.B. Testwerkzeuge zu konfigurieren.
