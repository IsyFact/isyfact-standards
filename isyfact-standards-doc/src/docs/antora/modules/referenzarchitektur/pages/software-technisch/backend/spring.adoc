...
* Konfiguration der Persistenzschicht: beschrieben in den xref:isy-persistence:nutzungsvorgaben/umsetzung-datenzugriff.adoc[Nutzungsvorgaben JPA/Hibernate],
...
----
Diese Annotation ist equivalent zur gleichzeitigen Nutzung von der Annotationen `@EnableAutoConfiguration`, `@ComponentScan` und `@SpringBootConfiguration`.
Die automatische Konfiguration, die extra Konfiguration der Applikationsklasse und das Komponenten-Scannen werden aktiviert.
Damit wird die explizite `@Import`-Anweisung meist überflüssig.
Ist eine granulare Kontrolle bei Anwendungen, die die `@SpringBootApplication`-Annotation nutzen gewünscht, können mit `exclude`-Attributen Komponenten von der automatischen Konfiguration ausgeschlossen werden (Vlg. https://docs.spring.io/spring-boot/reference/using/auto-configuration.html#using.auto-configuration.disabling-specific[offizielle SpringBoot Dokumentation].
...
Mit diesem Vorgehen wird explizite Kontrolle über die Konfiguration und das Komponenten-Scannen geboten.
...
Alternativ zu diesem Zweck auch `@SpringBootConfiguration` genutzt werden.
...
Die zusätzliche Annotation ist nicht notwendig, wenn <<listing-applikationsklasse-mit-springbootapplication-annotation>> genutzt wird.
...
Konfiguriert werden Spring-Beans und ihre Abhängigkeiten durch die von Spring bereitgestellte Annotation `@Component` bzw. durch deren Spezialisierungen (`@Repositpory`, `@Service`, `@Contoller`, usw.).
...
Die Umsetzung geschieht im xref:software-technisch/backend/anwendungskern.adoc#transaktionssteuerung[Anwendungskern] oder in der xref:blaupausen:detailkonzept-komponente-service/master.adoc#transaktionssteuerung[Serviceschicht].
...
Statt AOP ist das in der Serviceschicht beheimatete Konstrukt der xref:blaupausen:detailkonzept-komponente-service/master.adoc#exception-fassade[Exception-Fassade] zu verwenden.
