...
* Application: Eine neue Instanz der Application-scoped Bean wird einmal für die gesamte Anwendung erstellt. Sie wird für alle Anfragen und Sitzungen gemeinsam genutzt.
* WebSocket:  Für jede WebSocket-Sitzung wird eine neue Instanz der WebSocket-scoped Bean erstellt.
...
https://docs.spring.io/spring-framework/reference/core/aop.html[Aspektorientierte Programmierung] (AOP) ist ein Programmieransatz, der dabei hilft, übergreifende Belange zu trennen - Funktionen, die sich über mehrere Module erstrecken, wie Protokollierung, Sicherheit, Transaktionsmanagement und Ausnahmebehandlung.
...
|Kann in Produktiv-Code eingesetzt werden und ist die Empfehlung von Spring.
