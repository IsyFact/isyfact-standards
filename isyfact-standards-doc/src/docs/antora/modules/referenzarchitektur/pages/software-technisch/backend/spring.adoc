...
Zusätzlich erbt die Klasse von `SpringBootServletInitializer`, damit das Backend im Tomcat deployt werden kann.
...
Die Umsetzung geschieht im xref:software-technisch/backend/anwendungskern.adoc#transaktionssteuerung[Anwendungskern] oder in der xref:blaupausen:detailkonzept-komponente-service/inhalt.adoc#transaktionssteuerung[Serviceschicht].
...
Statt AOP ist das in der Serviceschicht beheimatete Konstrukt der xref:blaupausen:detailkonzept-komponente-service/inhalt.adoc#exception-fassade[Exception-Fassade] zu verwenden.
