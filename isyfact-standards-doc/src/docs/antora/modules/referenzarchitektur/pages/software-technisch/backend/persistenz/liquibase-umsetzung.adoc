= Datenbankmanagement und Schemaversionierung
...
Dadurch gilt:
- Das Backend selbst hat niemals DDL-Rechte.
...
(niemals DDL-Rechte)
| Runtime-User des Backends führt ausschließlich Datenmanipulationen aus.
|===
...
=== Gefahr von Schema-Drift und semantische Versionskontrolle mit dem Baustein Util
...
Liquibase prüft nicht, ob die tatsächliche Datenbankversion mit der vom Code des Backends erwarteten Release-Version übereinstimmt.
Dadurch kann es trotz konsistentem Changelog zu https://www.liquibase.com/blog/database-drift[Schema-Drift] kommen.
...
Solche Abweichungen führen dazu, dass das Backend gegen ein nicht kompatibles Schema startet.
Dies birgt Risiken wie Laufzeitfehlern oder Datenverlust.

==== Enforcement mit dem Baustein Util
Um zu Verhindern, dass unbemerkte Schema-Abweichungen in Produktion zu Herausforderungen und Aufwand führen, und die Integrität zwischen Backend und Datenbank über alle Deployments hinweg gewährleistet ist oder die Durchsetzung von semantischen Schema-Versionen eine Anforderung ist, kann über den xref:util::konzept.adoc[Baustein Util] eine Schema-Versionskontrolle auf Systemebene eingebunden werden:

Eigene Properties

* `isy.persistence.datasource.schema-version` definiert die vom Backend erwartete Schema-Version (z. B. `1.2.3`).
* `isy.persistence.datasource.schema-invalid-version-action` bestimmt das Verhalten bei Abweichung (`fail` oder `warn`).

Technischer Ablauf:

* Beim Start prüft das Backend den Inhalt einer dedizierten Tabelle `m_schema_version`.
...
Die Kombination aus *Liquibase Changelogs* und *isy-util Version Enforcement* bietet für großskalige Umgebungen Vorteile:

* Deterministische Kompatibilität: Das Backend läuft nur gegen ein getestetes, freigegebenes Schema.
* Früherkennung: Fehlkonfigurationen oder manuelle Eingriffe werden beim Start erkannt.
* CaaS-Tauglichkeit: Auf einer Container-Plattform kann ein Init-Container die Schema-Version vor Pod-Start validieren.
...
Dabei wird das Backend so erweitert, dass sie auf dem neuen Feld arbeitet.
Gleichzeitig bleibt die Datenbank in einem Zustand, in dem ältere Versionen des Backends weiterhin ohne Anpassungen auf der Datenbank arbeiten können.
Sobald alle Altversionen abgeschaltet sind, wird in einer dritten Phase das alte Datenbankfeld aus der Datenbank entfernt.
...
+2.+ Phase::
Das Feature-Rollout findet im Backend statt.
Hier wird auf neues Feld umgestellt.
Falls nötig, ist ein Changeset für zusätzliche Indizes mit dem Label `app-deploy` zu nutzen.

+3.+ Phase::
Laufen alle Instanzen des Backends auf der neuen Version, ist mit einem Changeset mit dem Label `clean-up` das Datenbankschema zu bereinigen (z.B. `<dropColumn>`).

[NOTE]
====
Für CI-Pipelines sind diese Tätigkeiten am besten in einzelnen Jobs auszuführen, mindestens jedoch in zwei.
Phase 1 und Phase 2 können zusammen ausgeführt werden.
====
...
Hierfür können die Unterschiede zwischen dem IST-Zustand und einer leeren Datenbank erzeugt werden.

|liquibase label
|Labels können genutzt werden, um Datenbankänderungen, die ein Feature betreffen, zu kennzeichnen und zum gewünschten Zeitpunkt auszuführen. (vgl. https://docs.liquibase.com/reference-guide/changelog-attributes/labels[Liquibase-Doku-Labels]).
