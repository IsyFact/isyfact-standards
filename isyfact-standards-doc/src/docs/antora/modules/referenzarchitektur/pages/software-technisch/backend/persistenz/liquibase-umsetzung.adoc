...
Dadurch kann es trotz konsistentem Changelog zu einem xref:https://www.liquibase.com/blog/database-drift[Schema-Drift] kommen.
...
|liquibase label
|Labels können genutzt werden, um Datenbankänderungen, die ein Feature betreffen, zu kennzeichnen und zum gewünschten Zeitpunkt auszuführen. (Vgl. xref:https://docs.liquibase.com/reference-guide/changelog-attributes/labels[Liquibase-Doku-Labels]).
