...
Dadurch kann es trotz konsistentem Changelog zu einem https://www.liquibase.com/blog/database-drift[Schema-Drift] kommen.
...
Gleichzeitig bleibt die Datenbank in einem Zustand, in dem ältere Anwendungsversionen weiterhin ohne Anpassungen auf der Datenbank arbeiten können.
...
+3.+ Phase::
Laufen alle Instanzen der Anwendung auf der neuen Version, ist mit einem Changeset mit dem Label `clean-up` das Datenbankschema zu bereinigen (z.B. `<dropColumn>`).
