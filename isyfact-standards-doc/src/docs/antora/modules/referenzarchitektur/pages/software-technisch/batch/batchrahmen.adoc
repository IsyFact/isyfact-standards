...
Dieser arbeitet analog zum normalen Wirkbetrieb, jedoch werden keine Änderungen an Datenbeständen der Backends derselben Anwendung oder der Backends der Nachbarsysteme durchgeführt.
...
|Vollqualifizierte Namen der Spring-Konfigurationsklassen des Backends
...
|Batchrahmen.MaxWiederholungen
|Maximale Anzahl automatischer Neustarts für einen fehlerhaften Batch.
...
* Beans (z.B. der Serviceschicht), die für die Batch-Verarbeitung nicht benötigt werden, sollten deaktiviert werden,
...
Um Beans gezielt aus der Spring-Konfiguration des Backends für die Ausführung eines Batches auszuschließen, kann die
...
Üblicherweise ist das Locking-Verhalten in Backends optimistisch: Datensätze werden nicht explizit gelockt.
...
Über den Konfigurationsparameter `Batchrahmen.MaxWiederholungen` kann eine Obergrenze für automatische Neustarts eines fehlerhaften Batches festgelegt werden.
Ist der Parameter nicht gesetzt oder auf `-1` gesetzt, gibt es keine Begrenzung.

Wird ein Batch mit `-restart` gestartet und wurde die konfigurierte maximale Anzahl an Wiederholungen bereits erreicht, bricht der Batchrahmen den Start mit einer `BatchrahmenMaxWiederholungenException` ab.
Der Rückgabewert ist in diesem Fall `FEHLER_ABBRUCH`.
Die Exception wird nur auf Info-Niveau geloggt, da es sich um einen erwarteten Abbruch handelt.

Ein Zähler der bisherigen Neustarts wird persistent in der Batch-Statustabelle gespeichert.
Wird der Batch mit `-start` (oder `-start -ignoriereRestart`) neu gestartet, wird dieser Zähler zurückgesetzt.
...
[NOTE]
Abhängig vom gewählten Grant-Type wird eine erneute Authentifizierung gegebenenfalls nur dann durchgeführt, wenn ein durch die Spring-Implementierung vorgegebener Zeitraum vor dem Ablauf des Tokens überschritten wird (siehe xref:security::nutzungsvorgaben.adoc#authentifizierungsmanager-authentifiziere-method-grant-type-note[Hinweis zur Methode `authentifiziere`]).
|====

In der Regel wird der Benutzer aus der betrieblichen Konfiguration des Backends oder alternativ aus den Aufrufparametern beim Start des Batches gelesen.
...
Der Batch im Testmodus arbeitet analog zum normalen Wirkbetrieb, jedoch werden keine Änderungen an Datenbeständen der Backends derselben Anwendung oder der Backends der Nachbarsysteme durchgeführt.
...
[[fachliche-logik-in-den-komponenten-des-backends-implementieren]]
=== Geschäftslogik in den Komponenten des Backends implementieren

Falls für die Verarbeitung im Batch Geschäftslogik benötigt wird, welche für das Backend nicht benötigt wird, ist diese trotzdem den Fachkomponenten im Anwendungskern hinzuzufügen.
Die Ausführungsbean ruft die Geschäftslogik dort auf.

Auch wenn in Sonderfällen Datenbank-Aufrufe direkt durch die Ausführungsbean ausgeführt werden müssen, ist die sonstige Geschäftslogik an die Fachkomponenten des Backends zu delegieren.
...
Die gezeigte Satzverarbeitung setzt den Testmodus um, die Geschäftslogik ist weiterhin im Anwendungskern umgesetzt und sie übernimmt Logging, Protokollierung und Statistik-Zählung.
...
Das erstellte Paket enthält den Code des Batchrahmens sowie den Code des zugehörigen Backends inklusive der Batch-Ausführungsklassen und der benötigten nicht-betrieblichen Batch-Konfigurationsdateien.
...
Dieses Startskript soll im Deployment des Batch-Systems unter `<batch-projekt>/src/main/resources/bin` übernommen und mit der richtigen Java-Version versehen werden.
...
Bei der betrieblichen Konfiguration wird nicht zwischen dem Deployment als Backend und dem Deployment als Batch unterschieden: Es werden jeweils dieselben betrieblichen Konfigurationsdateien verwendet.
Damit der Betrieb Anpassungen dieser Dateien nicht zweimal durchführen muss, wird die betriebliche Konfiguration mit dem zugehörigen Backend deployt.
Vor dem Deployment des Batch-Pakets muss das zugehörige Backend deployt sein.
Falls der Batch nicht auf dem Server dieses Backends läuft, muss die Konfigurationsdatei des Backends vor der Installation des Batches auch auf diesem Server verfügbar gemacht (zum Beispiel gemountet oder kopiert) werden.
