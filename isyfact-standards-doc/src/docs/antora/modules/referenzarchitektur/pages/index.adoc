...
.Software-technische Referenzarchitektur
image::referenzarchitektur:software-technisch/backend/architektur-backend.dn.svg[]

Die *Persistenzschicht* kapselt alle Funktionen zum Zugriff auf die Datenbanken.
...
Ergänzend zu Service, Batch, Anwendungskern und Persistenz gibt es in den IT-Systemen *querschnittliche Funktionalitäten* wie beispielsweise Fehlerbehandlung, Logging oder Authentifizierung und Autorisierung.

Die Referenzarchitektur beschreibt die Nutzungsschichten Batch und Service sowie den Anwendungskern und die Persistenzschicht.
