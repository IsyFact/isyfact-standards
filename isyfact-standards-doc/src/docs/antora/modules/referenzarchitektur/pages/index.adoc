...
Die Menge aller Anwendungen einer Organisation und deren Nutzungsbeziehungen untereinander bilden eine oder mehrere Anwendungslandschaften.
...
Die xref:referenzarchitektur:fachlich.adoc[fachliche Referenzarchitektur] strukturiert die Anwendungslandschaft hierarchisch in Domänen, Anwendungen und Komponenten.
...
* *Querschnittsanwendungen* stellen xref:glossary::terms-definitions.adoc#service[Services] für mehrere Geschäftsanwendungen bereit, beispielsweise ein Suchverfahren oder die Anwenderverwaltung.
...
Externen Anwendungen stellt das API-Gateway die entsprechenden Schnittstellen zur Verfügung.
