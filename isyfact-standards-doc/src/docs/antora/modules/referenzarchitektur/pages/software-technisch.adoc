...
Wie in <<strukturierung-der-systemlandschaft>> beschrieben, kommunizieren IT-Systeme auf Basis von Services (s. <<servicekommunikation-intern-extern>>).
...
Synchrone Service-Aufrufe bieten die Möglichkeit der direkten Kommunikation zwischen zwei IT-Systemen (s. <<service-aufruf-synchron>>).
...
JMS-Nachrichten bestehen aus Header, Properties und einem Body (s. <<jms-message-aufbau>>).
...
Dies ist in <<services-typen>> dargestellt.
