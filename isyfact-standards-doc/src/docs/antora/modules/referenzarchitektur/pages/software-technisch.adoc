...
Eine detaillierte Beschreibung der erwähnten Zugriffskomponenten (Portal, Service-Gateway, Authentication-Gateway), des IAM-Service, der implementierten Authentifizierungsverfahren und der Autorisierungsinformationen (Access-Token) findet sich im xref:4.x@isyfact-standards-doku:isy-security:konzept/master.adoc[].
...
Wie genau Rollen und Rechte spezifiziert werden, beschreibt das xref:4.x@isyfact-standards-doku:isy-security:konzept/master.adoc[] im Kapitel xref:4.x@isyfact-standards-doku:isy-security:konzept/master.adoc#rollen-und-rechte[Rollen und Rechte].
...
Welche Maßnahmen für eine sichere Kommunikation realisiert werden, ist mit dem jeweiligen Partner, der einen xref:4.x@isyfact-standards-doku:referenzarchitektur:software-technisch/services.adoc[Service]  anbietet, abzustimmen.
