...
=== Nutzung von Zertifikaten
Ein Sicherheitszertifikat ist ein digitales Dokument, das die Identität einer Website oder eines Dienstes bestätigt und zur Absicherung der Kommunikationspartner einer Kommunikationsstrecke eingesetzt werden kann.
Eine Einsatzmöglichkeit ist die Authentifizierung von Benutzern, beispielsweise im Rahmen einer Public Key Infrastructure (PKI).
Hierbei besitzt jede Entität ein eigenes Schlüsselpaar und ein zugehöriges Zertifikat, das ihre Identität bestätigt.
...
* Privater Schlüssel und Zertifikate werden in einem PKCS#12-Format (siehe: https://tools.ietf.org/html/rfc7292[PKCS12]) abgelegt.
* Es werden nur die im Standardumfang des JDKs enthaltenen kryptografischen Verfahren verwendet (siehe https://docs.oracle.com/javase/8/docs/technotes/guides/security/jsse/JSSERefGuide.html[Java Secure Socket Extensions]).
...
Bei der Nutzung von einer verschlüsselten Verbindung via HTTPS ist eine zertifikatsbasierte Authentifizierung möglich.
 ...
 * ein Client-Zertifikat, das von einer vertrauenswürdigen CA signiert ist,
 * den zugehörigen privaten Schlüssel, welcher lokal im Keystore des Service-Consumers bleibt.
Die nutzende Anwendung übergibt dem Service-Consumer das Zertifikat beim TLS-Handshake automatisch an den Server.
...
In der nutzenden Anwendung werden das Client-Zertifikat und der private Schlüssel in einer PKCS#12-Datei im Config-Verzeichnis der Anwendung abgelegt werden.
Das Passwort für den privaten Schlüssel kann in einer Property zusammen mit den übrigen betrieblichen Konfigurationen der Anwendung vorgehalten werden.
...
Bei dieses Vorgehen ist zu beachten: Die Gefahr besteht, dass der private Schlüssel über Logs, Artefakte, Konfigurations-Viewer oder Versions-Verwaltungslösungen zugänglich wird.
...
Sofern der Service-Consumer nicht für alle Verbindungen denselben Schlüssel verwendet, müssen entsprechende SocketFactory-Instanzen pro Verbindung zur Verfügung gestellt werden.
Diese kann allerdings erst nach Initialisierung des ServiceStubs gesetzt werden, was dazu führt, dass die WSDL-Datei noch nicht über die gesicherte Verbindung abgerufen werden kann.
Es wird daher empfohlen die WSDL-Datei lokal bei dem Service-Consumer vorzuhalten und aus dem Dateisystem zu laden.

[[ablage-von-zertifikaten]]
=== Ablage von Zertifikaten
Bei einer verschlüsselten Verbindung authentifiziert sich der externe Service mit einem Server-Zertifikat.
Dies erfordert einen TrustStore, über den vertrauenswürdige CA-Zertifikate abspeichert werden, mit denen die Integrität der Server-Zertifikate validiert werden.
Aus Sicherheitsgründen wird ebenso eine mTLS Verbindung empfohlen.
TLS/mTLS sind immer auf Transportebene zu konfigurieren (server.ssl.*).
Für mTLS wird zusätzlich ein KeyStore benötigt, der das Client-Zertifikat samt dem privaten Schlüssel enthält.

[[installation-des-zertifikates-in-pkcs12]]
==== Installation des Zertifikats in einem PCKS#12-Keystore
Ein PKCS#12-Keystore enthält die eigenen privaten Schlüssel und die zugehörigen Zertifikate (ggf. Zertifikatskette).
Ein TrustStore enthält ausschließlich vertrauenswürdige CA-Zertifikate.
Bei der Authentifizierung prüft die lokale Komponente ein eingehendes Zertifikat gegen die im TrustStore enthaltenen CAs.
Die im Truststore gespeicherten Zertifikate dienen also der Validierung der Gegenstelle.
Der KeyStore dient der eigenen Authentifizierung des Services.
