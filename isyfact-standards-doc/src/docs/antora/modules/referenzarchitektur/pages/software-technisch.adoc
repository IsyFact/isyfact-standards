...
=== Nutzung von Zertifikaten
Welche Maßnahmen für eine sichere Verbindung realisiert werden, ist mit dem jeweiligen Partner, der den externen Service anbietet, abzustimmen.
...
* Für den Austausch privater Schlüssel wird das PKCS12-Format (siehe https://tools.ietf.org/html/rfc7292[PKCS12]) verwendet.
* Es werden nur die im Standard-Umfang des JDKs enthaltenen kryptografischen Verfahren verwendet (siehe https://docs.oracle.com/javase/8/docs/technotes/guides/security/jsse/JSSERefGuide.html[Java Secure Socket Extensions]).

Der Service-Consumer kann die Verbindung zum externen Service entweder unverschlüsselt per HTTP oder verschlüsselt per HTTPS aufbauen.
Bei der Nutzung von einer verschlüsselten Verbindung via HTTPS ist zusätzlich eine zertifikatsbasierte Authentifizierung möglich.
Der Service-Consumer unterstützt dabei die folgenden Varianten:

* Authentifizierung des externen Service gegenüber dem Service-Consumers (einseitige Authentifizierung).
Diese Art der Authentifizierung wird für den Aufbau verschlüsselter Verbindungen vorausgesetzt.
Der Aufbau von verschlüsselten Verbindungen ohne Authentifizierung des externen Service wird aus Sicherheitsgründen nicht unterstützt.
* Zusätzlich kann ein Service-Consumer eine Authentifizierung des Clients gegenüber dem externen Service (beidseitige Authentifizierung) unterstützen.

[[zertifikatsbasierte-authentifizierung]]
==== Zertifikatsbasierte Authentifizierung

Bei Verwendung von Authentifizierung per Zertifikat werden ein Client-Zertifikat und der zugehörige private Schlüssel benötigt.
...
Hierfür wird ein Keystore im PKCS12-Format als Byte-Array übertragen.
Zusätzlich übergibt die Anwendung das Passwort für den privaten Schlüssel (Keystore-Passwort).

In der nutzenden Anwendung werden das Client-Zertifikat und der private Schlüssel in einer PKCS12-Datei im Config-Verzeichnis der Anwendung abgelegt.
Das Passwort für den privaten Schlüssel wird in einer Property zusammen mit den übrigen betrieblichen Konfigurationen der Anwendung vorgehalten.
...
Sofern der Service-Consumer nicht für alle SSL-Verbindungen denselben Schlüssel verwendet, muss für jede Anfrage eine entsprechende SocketFactory zur Verfügung gestellt werden.
...
Es wird daher empfohlen die WSDL-Datei lokal bei dem Service-Consumer abzulegen und aus dem Dateisystem zu laden.


[[ablage-von-zertifikaten]]
=== Ablage von Zertifikaten

[[installation-des-zertifikates-in-jks]]
==== Installation des Zertifikats im Java Keystore (JKS)
Ein JKS ist eine Datei, die die eigenen privaten Schlüssel und deren zugehörige SSL/TLS-Zertifikate speichert.
...
[[installation-des-zertifikats-als-proptery-der-konfiguration]]
==== Installation des Zertifikats als Property Teil der Konfiguration
