...
* Produktionsumgebung,
* Staging-Umgebung,
* Externe Testumgebung und Abnahme-Testumgebung,
* Schulungsumgebung, und
...
Die Kommunikation erfolgt per HTTPS mit einem Browser oder ebenfalls per HTTPS oder SMTP via XML- oder Webservice-Schnittstelle direkt aus dem externen System über ein Service-Gateway, das im xref:technisch-infrastrukturell/api-gateway.adoc[] liegt.
Innerhalb der Produktionsumgebung sollten die IT-Systeme ebenfalls verschlüsselt miteinander kommunizieren.

Interne Drittsysteme, die aus dem internen LAN mit der IsyFact-Systemlandschaft kommunizieren, tun dies genau wie externe Systeme.

Die Authentifizierung geschieht über einen Identity & Access Management (IAM) Cluster, bzw. ein IAM-System, der von außen über das im xref:technisch-infrastrukturell/api-gateway.adoc[] liegende Authentication-Gateway erreichbar ist.
...
Web-Server und Service-Gateways kommunizieren mit den Applikationsservern der Logik- und Verarbeitungszone.
In der Produktionsumgebung wird aus Gründen der Vereinfachung davon ausgegangen, dass je Rechnersystem ein Applikationsserver betrieben wird.
...
Mit dem Begriff „Staging-Umgebung“ werden die Komponenten der technischen Infrastruktur bezeichnet, die zum internen Test verwendet werden und auf denen Probleme des produktiven Betriebs nachgestellt werden können.
...
Das heißt, an Stellen, in denen in der Produktionsumgebung ein Cluster mit mehr als zwei Knoten verwendet wird, wird in der Staging-Umgebung ein Cluster mit zwei Knoten eingesetzt.
...
Service-Gateways und das Portal benötigen zusätzlich noch einen Apache-Webserver.
