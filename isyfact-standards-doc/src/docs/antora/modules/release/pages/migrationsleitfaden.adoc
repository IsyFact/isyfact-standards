...
[[isy-security]]
=== Security
Method Security ist in der IsyFact 4 version von isy-security standardmäßig nicht mehr aktiviert und muss durch Setzen der `@EnableMethodSecurity(securedEnabled = true)`- Annotation an einer `@Configuration`-Klasse aktiviert werden.
Dies ist vor allem dann notwendig, wenn Controller-Klassen mit `@Secured` annotiert sind.
