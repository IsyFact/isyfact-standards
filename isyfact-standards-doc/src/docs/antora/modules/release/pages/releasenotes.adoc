...
Eine Abbildung einer Anwendung auf nur ein IT-System ist weiterhin möglich, stellt aber ein Spezialfall dar.
Die Beschreibung des "technischen Schnitts" zieht auch eine neue Definition des xref:glossary:glossary:master.adoc#glossar-it-system[IT-Systems] nach sich.
Neu eingeführt wurden die xref:glossary:glossary:master.adoc#glossar-it-systemtyp[IT-Systemtypen].
Sie stellen konkrete Ausprägungen des IT-Systems dar, besitzen eine jeweils eigene Systemarchitektur und können auch verschiedenartige Technologie-Stacks setzen.
...
IT-Systeme dürfen demnach keine UI-Komponenten mehr enthalten.
...
Der Baustein wurde entfernt.
Anwendungen müssen auf den xref:isy-service-rest:konzept/master.adoc[Baustein REST] migrieren.

=== JSF

Der Baustein wurde entfernt.
Anwendungen müssen auf den xref:isy-angular-widgets-doc::konzept/konzept.adoc[Baustein Angular] migrieren.

=== Sicherheit

Der Baustein wurde entfernt.
Anwendungen müssen auf den xref:isy-security:konzept/master.adoc[Baustein Security] migrieren.
