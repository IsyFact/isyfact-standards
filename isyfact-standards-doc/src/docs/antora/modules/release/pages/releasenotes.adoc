...
Die Inhalte über Services, bezogen auf die Kommunikation zwischen IT-Systemen, wurden auf eine xref:4.x@referenzarchitektur:software-technisch/services.adoc[eigene Seite] umgezogen.
...
Im Konzept des REST-Bausteins wurde das Kapitel xref:4.x@isyfact-standards-doku:isy-service-rest:konzept/master.adoc#verwendung-http-statuscodes[Verwendung von HTTP-Statuscodes] angepasst.
...
* Hinweis auf die seit Liquibase 4.30.0 standardmäßig aktivierte Telemetrie und deren Deaktivierung in der Persistenzschicht-Dokumentation ergänzt.
