...
Die Vorgaben der Versionierung wurden verfeinert und um Details zu kompatiblen und inkompatiblen Serviceänderungen ergänzt.
Es ist dort beschrieben, in welchen Schichten und Komponenten die Transformationslogik zwischen verschiedenen Service-Versionen zu implementieren ist, nach welchen Kriterien die Major und Minor-Versionierung stattfinden soll und wie ein Client parallel betriebene Service-Versionen auswählen kann.
