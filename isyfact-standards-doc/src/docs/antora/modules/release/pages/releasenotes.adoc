...
Die Inhalte über Services, bezogen auf die Kommunikation zwischen IT-Systemen, wurden auf eine xref:4.1.0@referenzarchitektur:software-technisch/services.adoc[eigene Seite] umgezogen.
...
Im Konzept des REST-Bausteins wurde das Kapitel xref:4.1.0@isyfact-standards-doku:isy-service-rest:konzept/master.adoc#verwendung-http-statuscodes[Verwendung von HTTP-Statuscodes] angepasst.
