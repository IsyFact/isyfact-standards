...
VirtualBox lässt sich unter xref:literaturextern:inhalt.adoc#litextern-virtual-box[Oracle VirtualBox] herunterladen.
...
Die Webseite xref:literaturextern:inhalt.adoc#litextern-osboxes[OSBoxes] bietet virtuelle Festplatten mit vorinstallierten Betriebssystemen an.
Dieser Leitfaden benutzt die Desktop-Edition von Ubuntu.
Diese ist unter xref:literaturextern:inhalt.adoc#litextern-osboxes-ubuntu[OSBoxes Ubuntu] verfügbar.
...
Auf der Webseite xref:literaturextern:inhalt.adoc#litextern-spring-tool-suite[Spring Tool Suite] bietet der Hersteller diesbezüglich eine Reihe von Download-Links an.
...
Um STS darüber hinaus über eine Desktop-Verknüpfung starten zu können, muss im Verzeichnis `.local/share/applications` Ihres Home-Verzeichnisses ein xref:literaturextern:inhalt.adoc#litextern-ubuntu-programmstarter[Programmstarter] angelegt werden.
...
Laden Sie Apache Tomcat von der Webseite xref:literaturextern:inhalt.adoc#litextern-apache[Apache] herunter.
...
NOTE: Mehr Details zur Verwendung der Checkstyle-Regeln finden sich im xref:literaturextern:inhalt.adoc#litextern-isyfact-checkstyle[README] des entsprechenden Repositories.
...
_Location_: xref:literaturextern:inhalt.adoc#litextern-eclipse-checkstyle-plugin[Eclipse Checkstyle Plugin]
