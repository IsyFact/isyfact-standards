...
TODO entfernen, Zeilenumbrüche ok
...
TODO Listings gemäß Stefan modifizieren
...
Näheres dazu in TODO xref


[[querverweise]]
=== Querverweise

Antora bietet die Möglichkeit, innerhalb einer Gesamtdokumentation beliebige Querverweise zu setzen, auch über Antora-Modul- und -
...
Generelle Hinweise zur Verwendung von Verweisen s. xref:seitenvorlage.adoc#struktur-verweise[Seitenvorlage Verweise].

[[anchors]]
==== Sprungziele (Anchor)

Für eine Überschrift einer IsyFact-Seite, s. <<ueberschriften>>, fehlen noch die _Inline-Anchor_, damit durch Klick auf Verweise auf diese Inline-Anchor gesprungen werden kann.
...
Verweise können auch auf andere Sprungziele zeigen als Überschriften, wie Abbildungen, Tabellen und Listings, s. auch xref:seitenvorlage.adoc#formatierung-verweise[Beispiele für Verweise auf Abbildungen, Tabellen und Listings].


[[verweise-gesamtdoku]]
==== Verweise innerhalb der Antora-Dokumentation

TODO Antora Module und Komponenten mussten weiter oben beschrieben sein

In diesem Abschnitt werden Verweise auf Inline-Anchor beschrieben, die nicht innerhalb derselben Seite in derselben Datei liegen, sondern in anderen Seiten oder anderen Antora-Modulen oder -Komponenten.
...
Ein Beispiel findet sich in xref:seitenvorlage.adoc#formatierung-verweise[Seitenvorlage Verweise].
...
Beispiele für alle selbstdefinierten Admonitions-Blocks finden sich in xref:seitenvorlage.adoc#formatierung-admonitions[Seitenvorlage Admonitions]. +
{empty} +
...
// tag::important[]
IMPORTANT: Abbildungen zu einer Seite werden stets im Unterordner `/images` unterhalb des Antora-Moduls abgelegt, in dem sich die Seite befindet.
Siehe auch TODO antora-doku
...
Es befindet sich im Repository `isy-documentation` in der Antora-Komponente TODO Link hinterlegen `glossary`.
In der Dokumentation ist es über die Liste der Antora Komponenten aufrufbar.

Eine Referenz auf einen Begriff des Glossars erfolgt über einen Antora xref-Link TODO Link auf Erklärung, wobei der Eintrag nach dem `#` die Id im Glossar in isy-documentation ist.
Die ID beginnt immer mit dem Bezeichner `glossar-`.
