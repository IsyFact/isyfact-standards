...
Als Beispiel dient das Repository xref:literaturextern:inhalt.adoc#litextern-isyfact-standards[icon:github[] isyfact-standards].
...
In der `antora.yml`-Datei wird mindestens der Name der xref:literaturextern:inhalt.adoc#litextern-antora-komponente[Dokumentationskomponente] und ihre Version angegeben.

[[antora-modul]]
== Antora-Modul

Im gleichen Verzeichnis wie `antora.yml`, `src/docs/antora`, muss das xref:literaturextern:inhalt.adoc#litextern-antora-verzeichnisstruktur[Verzeichnis für die Module] liegen.
...
Neben dem Verzeichnis `pages` kann es bei Bedarf xref:literaturextern:inhalt.adoc#litextern-antora-verzeichnisstruktur[noch weitere Verzeichnisse], z.B. für Bilder oder Anhänge, geben.
...
Unter "Bausteine" befinden sich beispielsweise die Bausteine, die im Repository xref:literaturextern:inhalt.adoc#litextern-isyfact-standards[icon:github[] isyfact-standards] dokumentiert werden.
====
...
Das Playbook befindet sich im Repository xref:literaturextern:inhalt.adoc#litextern-isyfact-antora[icon:github[] isy-antora-playbook].
