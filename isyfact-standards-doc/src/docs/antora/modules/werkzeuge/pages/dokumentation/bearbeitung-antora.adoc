...
* xref:literaturextern:inhalt.adoc#litextern-asciidoc[What is AsciiDoc? Why do we need it?] +
* xref:literaturextern:inhalt.adoc#litextern-asciidoctor-usermanual[Asciidoctor User Manual] +
* xref:literaturextern:inhalt.adoc#litextern-asciidoc-syntax[AsciiDoc Syntax Quick Reference] +
* xref:literaturextern:inhalt.adoc#litextern-asciidoc-tutorial[Tutorial: Using AsciiDoc and Asciidoctor to write documentation]
====
...
Diese Regel wird "ein Satz pro Zeile" (_one sentence per line_, s. xref:literaturextern:inhalt.adoc#litextern-asciidoc-recommended-practices[AsciiDoc Recommended Practices]) genannt und orientiert sich an der Art und Weise, wie Quellcode organisiert ist (eine Anweisung pro Zeile).
...
Antora bietet die Möglichkeit, innerhalb einer Gesamtdokumentation beliebige Querverweise zu setzen, auch über xref:dokumentation/erstellung-antora.adoc#antora-modul[Antora-Modul] - und xref:dokumentation/erstellung-antora.adoc#antora-komponente[Antora-Komponenten] - Grenzen hinweg.
...
Außerdem muss die ID des Anchors der xref:methodik:namenskonventionen/master.adoc#dokumentation[Namenskonvention für Namen in Dokumenten] folgen.
...
xref:literaturextern:inhalt.adoc#litextern-antora-xref-macro-structure[Beschreibung der Struktur des Xref-Makros].
...
Der Link auf einen Literatureintrag sieht wie folgt aus: xref:literaturextern:inhalt.adoc#litextern-asciidoctor-usermanual[Asciidoctor User Manual].
...
.icon:university[title=Architekturregel] Architekturregel: Beispiel für Architekturregel
****
...
.icon:shield[title=Sicherheit] Sicherheit: Beispiel für Sicherheitshinweis
****
...
.icon:paint-brush[] Bedienkonzept: Beispiel für Bedienkonzept
****
...
Seiten besitzen einen xref:seitenvorlage.adoc#struktur-thema[engen, thematischen Fokus] und sind direkt über die xref:dokumentation/erstellung-antora.adoc#linksnavigation-antora[Linksnavigation] aufrufbar.
...
NOTE: Eine vollständige Übersicht über die Funktionsweise von Tags bietet die xref:literaturextern:inhalt.adoc#litextern-AsciiDocTaggedRegions[Dokumentation von AsciiDoc zu _tagged regions_].
...
IMPORTANT: Abbildungen zu einer Seite werden stets im Unterordner `/images` unterhalb des Antora-Moduls abgelegt, in dem sich die Seite befindet, siehe auch xref:dokumentation/erstellung-antora.adoc#struktur-dokuseite[Struktur Dokumentation].
...
Als Produkt wird hierzu xref:literaturextern:inhalt.adoc#litextern-highlight-js[highlight.js] genutzt.
Ist für den Inhalt des Listings Syntax Highlighting verfügbar, muss es genutzt werden.

NOTE: Die xref:literaturextern:inhalt.adoc#litextern-highlight-js-uebersicht[Übersicht aller unterstützten Inhalte] zeigt, für welche Inhalte Syntax Highlighting verfügbar ist.
...
Es befindet sich im Repository `isy-documentation` in der xref:dokumentation/erstellung-antora.adoc#antora-komponente[Antora-Komponente] `glossary`.
...
In der Dokumentationsseite werden Bausteine, die deprecated sind, die demnach in einer späteren Version entfallen können, als Deprecation gekennzeichnet.
Es gibt mehrere Möglichkeiten, Deprecations auszuzeichnen.

*Deprecation für Baustein*

Ein vorbereiteter Text mit Icon aus `isy-documentation` wird inkludiert, siehe xref:seitenvorlage.adoc#deprecation-baustein[Deprecation für Baustein].
Damit wird ein vollständiger Baustein als deprecated gekennzeichnet.

*Deprecation für beliebigen Abschnitt*

Ein vorbereiteter Text mit Icon aus `isy-documentation` wird inkludiert, siehe xref:seitenvorlage.adoc#deprecation-abschnitt[Deprecation für Abschnitt], um beliebige Textabschnitte als deprecated zu kennzeichnen.
Dabei sollte das Attribut direkt unterhalb einer Überschrift platziert werden, damit der Abschnitt inklusive untergeordneter Abschnitte tieferer Ebenen bis zur nächsten Überschrift der gleichen Ebene als deprecated erkennbar ist.

*Deprecation mit Verweis auf neue Dokumentationsseite*

Sowohl Bausteine als auch Abschnitte können einen Zusatztext erhalten mit einem Verweis auf die neue Dokumentationsseite.
Dies erfolgt, indem vor der Inkludierung des Deprecation-Bausteins oder -Abschnitts ein Attribut mit dem entsprechenden Verweis angefügt wird, siehe xref:seitenvorlage.adoc#deprecation-verweis[Verweis auf neue Seite].
...
Wenn der Hinweis auf eine Deprecation in einer Tabellenzelle oder anderen kleinen Textstellen erfolgen soll, kann auch die xref:seitenvorlage.adoc#deprecation-kurzform[Kurzform] gewählt werden.
