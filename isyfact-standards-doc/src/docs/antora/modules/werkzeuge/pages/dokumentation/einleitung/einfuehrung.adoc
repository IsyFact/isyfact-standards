...
Zum Erstellen und Bearbeiten von AsciiDoc-Dateien wird ein Texteditor benötigt, empfohlen wird xref:glossary:glossary:master.adoc#glossar-vs-code[VS Code (Visual Studio Code)], sofern nicht der Editor der Entwicklungsumgebung verwendet wird.
...
Neben den ausgereiften Plugins für die Bearbeitung von AsciiDoc-Dokumenten:  xref:glossary:glossary:master.adoc#glossar-vs-code-asciidoc-erweiterung[AsciiDoc Erweiterung für VS Code] unterstützt VS Code Git-basierte Projekte mit vielfältigen Funktionen.
