...
Die `vs-einstufung` kann auch xref:dokumentation/generierung/generierung-asciidoctorj.adoc#listing-pom-asciidoc[im Maven-POM] als globales Attribut für alle zu generierenden Dokumente angegeben werden.
...
<<table-master-attributes,Die folgende Tabelle>> zeigt alle Attribute, die in der `master.adoc` Datei angepasst werden können.
...
In `anhaenge.adoc` sind Anhänge zum Dokuments platziert.
