...
Dieses Kapitel beschreibt die inhaltliche Bearbeitung und die dabei zu verwendenden Konventionen für die in xref:dokumentation/erstellung/erstellung-asciidoctorj.adoc#dokument-struktur[Struktur eines Dokuments] beschriebenen Dateien.
...
[[bedienkonzept-admonition]]
.BEDIENKONZEPT Admonition
image::handbuch/bedienkonzept.dn.svg[align="center"]
...
* BEDIENKONZEPT (Tag-Name: `bedienkonzept`).
...
Voraussetzung für die Aufnahme eines Eintrags in das lokale Literaturverzeichnis ist die Referenzierung des entsprechenden Dokuments und das Vorhandensein der referenzierten ID in der Literaturverzeichnis-Konfigurationsdatei der xref:dokumentation/generierung/generierung-asciidoctorj.adoc#isy-asciidoctorj-extensions[AsciidoctorJ-Extensions].

Eine Referenz auf einen Literatureintrag erfolgt über doppelte spitze Klammerpaare, wobei der erste Eintrag die ID in der Literaturverzeichnis-Konfigurationsdatei ist.

Ein Beispiel für referenzierte Literatur:

 <<AsciiDocRecommendedPractices, AsciiDoc Recommended Practices>>

Das Ergebnis sieht so aus:

xref:glossary:literaturextern:inhalt.adoc#litextern-asciidoc-recommended-practices[AsciiDoc Recommended Practices]

[[glossar]]
== Glossar

Am Ende eines Dokuments wird automatisch ein Glossar erstellt.
Voraussetzung für die Aufnahme eines Eintrags in das lokale Glossar ist die Referenzierung des entsprechenden Eintrags und das Vorhandensein des Eintrags mit der referenzierten Id im zentralen Glossar der xref:dokumentation/generierung/generierung-asciidoctorj.adoc#isy-asciidoctorj-extensions[AsciidoctorJ-Extensions].
...
In der Dokumentation werden Bausteine, die deprecated sind, die demnach in einer späteren Version entfallen können, als Deprecation gekennzeichnet.
Es gibt zwei Möglichkeiten, Deprecations auszuzeichnen.

*Deprecation für Baustein*

Mit dem Attribut `:deprecated-baustein:` im Kopf des Dokuments vor dem `colophon`-Eintrag wird ein vollständiger Baustein als deprecated gekennzeichnet.

*Deprecation für beliebigen Abschnitt*

Mit dem Attribut `:deprecated-abschnitt:` kann ein beliebiger Abschnitt mitten im Text als deprecated gekennzeichnet werden.
...
Für die Isy-AsciidoctorJ-Extensions wurde keine automatisierte Möglichkeit geschaffen, auf eine neuere Version der Dokumentation zu verweisen.
Dies muss im Bedarfsfall individuell im Text erstellt werden.
