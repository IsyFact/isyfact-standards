...
Damit stellt der Anwendungskern innerhalb des Systems allgemein nutzbare xref:glossary:glossary:master.adoc#glossar-ServiceFachlich[Services] zur Verfügung.
...
Vereinfachend können Anwendungskern-Komponenten persistente Daten an die xref:glossary:glossary:master.adoc#glossar-ServiceFachlich[Service]-Schicht (Kapitel <<servicezugriffe>>) weitergeben.
