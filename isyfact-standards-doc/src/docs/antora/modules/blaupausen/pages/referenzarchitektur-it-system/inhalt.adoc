...
Details zur Konstruktion eines IT-Systems werden in xref:methodik:vorlage-systementwurf/antora-master.adoc[IsyFact Systementwurf] festgelegt und für das jeweilige IT-System dokumentiert.
...
Details hierzu sind im  xref:detailkonzept-komponente-datenzugriff/master.adoc[Detailkonzept Komponente Datenzugriff] dokumentiert.
...
Der Anwendungskern stellt somit im wesentlichen allgemein nutzbare xref:glossary:glossary:master.adoc#glossar-Dienst[Dienste] zur Verfügung.
...
Weiterführende technische Details zu Services sind im xref:detailkonzept-komponente-service/master.adoc[Detailkonzept Komponente Service] enthalten.
...
Eine Übersicht dazu liefern die Beschreibungen der Bausteine der xref:einstieg:isyfact-standards.adoc#bausteine-der-isyfact-standards[IsyFact-Standard] sowie der xref:einstieg:isyfact-erweiterungen.adoc#bausteine-der-isyfact-erweiterungen[IsyFact-Erweiterungen].
