...
Services gliedern sich über eine technische Komponente „Service“ in die T-Architektur von IT-Systemen ein (siehe xref:image-IFRefArcITSysServ[]).
...
=== Service-Fassade
Die _Service-Fassade_ implementiert die Service-Operationen als Methoden.
Ihre Hauptaufgabe ist die Transformation der Service-Aufrufe in interne Aufrufe des _Anwendungskerns_ sowie die Transformation dessen Antworten in Antworten der _Service-Schnittstelle_.
Dabei transformiert sie die Geschäftsobjekte des _Anwendungskerns_ in Transportobjekte der _Service-Schnittstelle_ und umgekehrt.
...
Voraussetzung für die Autorisierung ist die Auswertung des mitgelieferten `SecurityContextes` aus der in der _Exception-Fassade_ vorausgegangenen Authentifizierung.
...
=== Exception-Fassade
Je nach verwendeter Schnittstellen-Technologie ist die _Exception-Fassade_ in das entsprechende Technologie-Framework eingebunden.
Dabei hat jede Schnittstellen-Technologie ihre speziellen Implementierungsdetails, die es pro Aufruf, für die dort einzubindende Parameter- und Kontextübernahme sowie eine notwendige Authentifizierung, zu beachten gilt.
Mit `IsyFact-3` wird mit dem Sicherheitsbaustein `isy-security` die xref:isyfact-standards-doku:isy-security:konzept/master.adoc#aufruf-service-gateway[Authentifizierung am ServiceGateway] auf `SpringSecurity OAuth2.0` eingeführt und gekapselt, der in der entsprechenden Schnittstellen-Technologie einzubinden ist.
Als Authentifizierungsergebnis wird dann ein `SecurityContext` mit allen security-relevanten Informationen an die _Exception-Fassade_ weitergereicht.

Die _Exception-Fassade_ ist verantwortlich die durch den _Anwendungskern_ oder die _Service-Fassade_ geworfenen Exceptions in Exceptions der Service-Schnittstelle umzuwandeln.
...
Bevor die _Exception-Fassade_ aufgerufen wird, muss die `Korrelations-ID` aus dem `SecurityContext` im _Logging-Kontext_ registriert sein.
...
Die Inhalte finden sich in den Dokumenten  xref:isy-serviceapi-core:konzept/master.adoc#einleitung[Konzept HTTP-Invoker (deprecated)] und xref:isy-service-rest:konzept/master.adoc#einleitung[Konzept REST-Schnittstellen] sowie xref:isy-serviceapi-core:nutzungsvorgaben/master.adoc#einleitung[Nutzungsvorgaben HTTP-Invoker (deprecated)] und xref:isy-service-rest:nutzungsvorgaben/master.adoc#einleitung[Nutzungsvorgaben REST-Schnittstellen].
====
...
Die Tabelle xref:standardisierte-metadaten[] zeigt, welche Metadaten in der IsyFact standardisiert übertragen werden.

[[standardisierte-metadaten]]
.Benennung standardisierter Metadaten
[id="standardisierte-metadaten", cols="1s,1m,2",options="header"]
|===
...
Für das obige Beispiel verdeutlicht dies das Code-Beispiel xref:listing-service-fassade-tx[].

.Transaktionssteuerung in der Service-Fassade
[id="listing-service-fassade-tx"]
[source,java]
----
public class BeispielServiceFassade {
    ...
    public BeispielHolenAntwortTo holeBeispiel(
        SecurityContextTo kontext, BeispielHolenAnfrageTo anfrage)
...
Es ist möglich, pro Service-Version ein eigenes Service-Gateway zu erstellen (siehe xref:image-archversServ[]).
...
.Realisierung der Versionierungsvorgaben für Schnittstellen per Maven-Buildprozess
...
Wie die xref:table-GMTMT[] zeigt, ergibt sich eine Gesamtverfügbarkeit von über 98 % bei einer Verfügbarkeit von 99,7 % pro System.
...
Kommt es zu einem xref:ausfall-von-hw-oder-sw[Hard- oder Softwareausfall] auf einem Server-Knoten, so werden alle Anfragen von einem vorgeschalteten Loadbalancer auf einen anderen Server-Knoten umgeleitet.
...
Diese Maßnahme hilft bei Nichtverfügbarkeit aufgrund von xref:ausfall-deployment[geplanten Wartungsarbeiten].
...
Von xref:retries-loadbalancer[Loadbalancern ausgeführte Retries] können zu einer Erhöhung der Antwortzeit führen.
