...
Voraussetzung für die Autorisierung ist die Auswertung des mitgelieferten `SecurityContextes` aus der in der *Exception-Fassade* vorausgegangenen Authentifizierung.
Über die Annotation `@Secured` wird die Berechtigung zum Zugriff auf die Methode anhand der Rechte des Aufrufers geprüft.
Alternativ kann die Annotation `@Secured` auch an der Service-Fassade selbst verwendet werden, wenn alle Methoden die gleichen Rechte erfordern.

=== Exception-Fassade
Je nach verwendeter Schnittstellen-Technologie ist die Exception-Fassade in das entsprechende Technologie-Framework eingebunden.
...
Als Authentifizierungsergebnis wird dann ein `SecurityContext` mit allen security-relevanten Informationen an die Exception-Fassade weitergereicht.
...
Bevor die Exception-Fassade aufgerufen wird, muss die Korrelations-ID aus dem `SecurityContext` im _Logging-Kontext_ registriert sein.
