...
Die Fachkomponenten der Servicelogik decken vier wesentliche Aufgaben ab.
...
Fehlerbehandlung:: Tritt bei der Verarbeitung eines Service-Aufrufs im Anwendungskern oder in der Servicelogik selbst eine Exception auf, so muss diese Exception in eine Exception der Service-Schnittstelle umgewandelt werden.
...
[[servicelogik-fachkomponenten]]
=== Fachkomponenten

Die Fachkomponenten setzen im Kern *Service-Schnittstellen* um.
Sie bestehen aus einer *Service-Fassade* und einer *Exception-Fassade*.
...
Dazu nutzt sie die xref:isy-sicherheit:nutzungsvorgaben/master.adoc#autorisierung[Möglichkeiten des Bausteins Sicherheit].
