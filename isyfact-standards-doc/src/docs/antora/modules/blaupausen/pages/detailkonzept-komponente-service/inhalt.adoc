...
Die Fachkomponenten der *Servicelogik* beinhalten Klassen mit Service-Methoden, die vom Service-Framework bei eingehenden (Technologie-spezifischen) Requests aufgerufen werden. 
Die aufgerufenen Service-Methoden nehmen Transport-Objekte als Argumente entgegen, rufen ihrerseits entsprechende Service-Methoden des Anwendungskerns auf,
und liefern Transport-Objekte als Ergebnis zurück.
[[servicelogik]]
== Servicelogik
...
Die _Service-Fassade_ übernimmt weiterhin auch die Validierung der Aufrufparameter auch die Autorisierung des Aufrufs.
