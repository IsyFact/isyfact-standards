...
Die Fachkomponenten der *Servicelogik* Servicelogik beinhalten Service-Endpoints, 
die über das Service-Framework bei eingehenden (Technologie-spezifischen) Requests aufgerufen werden. 
Die Service-Endpoints wiederum beinhalten Methoden, die ihrerseits entsprechende Methoden des Anwendungskerns aufrufen.
Die Methoden der Service-Endpoints nehmen Transport-Objekte als Argumente entgegen und liefern Transport-Objekte als Ergebnis zurück.
[[servicelogik]]
== Servicelogik
...
Sie decken fünf wesentliche Aufgaben ab.

Vaidierung:: Wenn für bestimmte Parameter von Methoden eines Service-Endpoints Bedingungen definiert sind, 
(z.B. ob es sich dabei um eine Pflicht-Parameter oder nur um einen optionalen Parameter handelt, 
oder ob für diesen Parameter bestimmte Ausprägungen zulässig sind), so ist es die Aufgabe der Service-Logik, diese Bedingungen zu prüfen.
...
Eine Authentfizierung des Aufrufers ist dem vorgelagert und wird implizit mit durchgeführt. 
...
=== Service-Fassade
Die Service-Fassade Fassade beinhaltet Fachkomponenten mit Service-Endpoints. 
Die Methoden dieser Service-Endpoints rufen entsprechende Methoden des Anwendungskerns auf. 
Zur Parameterübergabe werden dabei Transport-Objekte der Service-Schnittstelle in Geschäftsobjekte des Anwendungskerns transformiert. 
Für die Ergebnisübergabe ist eine Transformation in umgekehrter Richtung erforderlich. 
Für die Transformation wird in der Regel ein Bean Mapper verwendet.
...
Zu beachten ist, dass diese Exceptions stets ohne Stacktraces übermittelt werden sollten, bzw. dass stattdessen Exception-Transport-Objekte als Response zurückzugeben sind. 
Die Umwandlung der Exceptions sollte nicht redundant in den Service-Endpoint-Methoden, sondern an zentraler Stelle implementiert sein. 
Für die Behandlung und Umwandlung aller Exceptions des Anwendungskerns sollten Log-Einträge erzeugt werden. 
Für das Logging ist sicherzustellen, dass zum Zeitpunkt der Log-Ausgabe eine Korrelations-ID verfügbar ist.

[NOTE]
====
Die weiteren Inhalte zum Aufbau, zur Realisierung und zur Nutzung von Service-Schnittstellen wurden wegen ihrer Abhängigkeit zur konkreten Schnittstellentechnologie 
in den entsprechenden Technologie-Baustein verschoben.
