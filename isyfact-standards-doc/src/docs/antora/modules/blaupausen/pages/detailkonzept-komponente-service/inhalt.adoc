...
Die Fachkomponenten der *Servicelogik* beinhalten Service-Endpoints.
Darunter verstehen wir Klassen, die vom Service-Framework bei eingehenden (Technologie-spezifischen) Requests aufgerufen werden. 
...
Hauptaufgabe der Methoden der Service-Endpoints ist die Transformation der Service-Aufrufe in interne Aufrufe des Anwendungskerns sowie die Transformation der zugehörigen Antworten in Antworten der Service-Schnittstelle.
Beim Service-Aufruf werden Transportobjekte der Service-Schnittstelle in Geschäftsobjekte des Anwendungskerns umgewandelt. 
