...
Die Transaktionalität einzelner Service-Methoden wird über Annotationen an diesen Methoden kofiguriert:
...
Wenn man sich für eine Service-Versionierung entscheidet, so sollte man sich bewusst sein, wie sich diese auf Klassen und Packages abbildet:

* In der Service-Fassade sollten verschiedene Service-Versionen anhand einer zweistelligen Versionsnummer Major.Minor unterscheidbar sein. 
Bei kompatiblen Änderungen wird die nur Minor Version, bei inkompatiblen Änderungen immer die Major und die Minor Version angepasst.

* Im Anwendungskern dagegen findet man lediglich zu jeder Major-Version eine jeweils neue Service-Implementierung.
