...
Voraussetzung für die Autorisierung ist die Auswertung des mitgelieferten `SecurityContextes` über die Annotation `@StelltAufrufKontextBereit` an der Methode oder an der Service-Fassade selbst.
Anschließend wird über die Annotation `@Secured` die Berechtigung zum Zugriff auf die Methode anhand der Rechte des Aufrufers geprüft.
