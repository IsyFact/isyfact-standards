...
Dazu nutzt sie die Möglichkeiten des Bausteins Sicherheit (siehe  <<TODO anderes Dokument: NutzungsvorgabenSicherheit>>).
...
Die Inhalte finden sich in den Dokumenten  <<TODO anderes Dokument: KonzeptHttpInvoker>> sowie  <<TODO anderes Dokument: NutzungsvorgabenHttpInvoker>>.
====
...
Die Services verwenden alle denselben xref:glossary:glossary:master.adoc#glossar-Anwendungskern[Anwendungskerns (Glossar)].
...
NOTE: Für die Versionierung von Schnittstellen gelten gesonderte Vorgaben, die in  <<TODO anderes Dokument: IsyFactVersionierung>> definiert sind.
...
Die Parameter sind Bestandteil der betrieblichen Konfiguration (s.  <<TODO anderes Dokument: KonzeptKonfiguration>>).
====
