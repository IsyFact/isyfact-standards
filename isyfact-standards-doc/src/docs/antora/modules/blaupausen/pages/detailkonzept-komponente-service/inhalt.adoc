...
Validierung:: Wenn für bestimmte Parameter von Methoden eines Service-Endpoints Bedingungen definiert sind, 
...
=== Service-Fassade
Die _Service-Fassade_ beinhaltet Fachkomponenten mit Service-Endpoints. 
...
Die _Service-Fassade_ übernimmt neben der Validierung der Aufrufparameter auch die Autorisierung des Aufrufs.
...
Dabei ist sicherzustellen, dass dem Aufufer keine Stacktraces zurückgegeben werden. 
Exceptions sollten deshalb in der _Exception-Fassade_ nicht einfach nur durchgereicht werden, sondern sie sind in der _Exception-Fassade_ zu modifizieren oder in entsprechende Transport-Objekte umzuwandeln. 
 
Für die Behandlung und Umwandlung dieser Exceptions sollten Log-Einträge erzeugt werden. 
