...
<<image-TIArchPRU>> skizziert die TI-Architektur der Produktionsumgebung. Einzelne Server werden durch UML-Knoten (engl. _UML nodes_) dargestellt.
Die größeren Knoten gruppieren einzelne Server zu einer logischen Einheit, die _Cluster_ genannt wird.
Die Knoten eines Clusters können bei Bedarf auf mehrere Rechenzentren verteilt sein, um bestmögliche Ausfallsicherheit zu erreichen.
