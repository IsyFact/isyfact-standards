...
Folgende Prinzipien müssen bei der Erstellung einer Systemlandschaft eingehalten werden:
...
* Service-Gateway: +
Das Service-Gateway hat die Aufgabe, Anfragen von _externen Anwendungssystemen_ entgegenzunehmen und zusammen mit den Autorisierunginformationen an den passenden Service der Geschäftsanwendung weiterzuleiten.
...
Nach erfolgreicher Authentifizierung leitet das Service-Gateway die Anfrage zusammen mit den Autorisierunginformationen an die Geschäftsanwendung weiter.
...
Nach erfolgreicher Authentifizierung gibt das Authentication-Gateway die Autorisierunginformationen an die externe Anwendung zurück. Die externe Anwendung sendet die erhaltenen Autorisierunginformationen gemeinsam mit der fachlichen Anfrage über das Service-Gateway an die Geschäftsanwendung.
...
* IAM-Service: +
Der IAM-Service verwaltet *Benutzer* (menschliche Anwender und technische Systeme) und ihre *Rollen* zentral für alle Komponenten der Anwendungs- und Systemlandschaft.
Hier werden die Authentifizierungsanfragen verarbeitet und mit Autorisierunginformationen beantwortet.
...
Die tatsächliche Identität und Authentifizierungsinformationen des Benutzers spielen ab hier keine Rolle mehr und sind deshalb nicht Teil des Access-Tokens.
...
Im IAM-Service sind zu einem Benutzer die ihm zugewiesenen *Rollen* in der Systemlandschaft verknüpft und Bestandteil der Autorisierungsinformationen.
