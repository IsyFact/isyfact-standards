...
Beschäftigt man sich mit dem Entwurf und der Implementierung von Anwendungslandschaften, so stellt man fest, dass sich die einzelnen xref:glossary:glossary:master.adoc#glossar-Geschaeftsanwendung[Geschäftsanwendung (Glossar)] im Allgemeinen sehr ähnlich sind.
...
.Durch die IsyFact abgedeckte Bereiche   (TODO Bild fehlt)
[id="image-IFBereiche",reftext="{figure-caption} {counter:figures}"]
image::IFBereiche.png[align="center",pdfwidth=45%,width=45%]
...
*Blaupausen:* xref:glossary:glossary:master.adoc#glossar-Blaupause[Blaupausen (Glossar)] beschreiben die vorgegebene Architektur und Konzepte einer Anwendungslandschaft für den Betrieb von Geschäftsanwendungen aus drei verschiedenen Sichten: der fachlichen Sicht, der Sicht der softwaretechnischen Umsetzung und der Sicht der technischen Infrastruktur.

*Bausteine:* xref:glossary:glossary:master.adoc#glossar-Baustein[Bausteine (Glossar)] sind konkrete, strukturierte Lösungen zur Umsetzung der in den Blaupausen beschriebenen Architektur und Konzepte.
...
*Betriebsplattform:* Die xref:glossary:glossary:master.adoc#glossar-Portal[Portal (Glossar)] definiert allgemeine Vorgaben und Rahmenbedingungen für den Betrieb von Anwendungslandschaften, die sich aus der Verwendung der IsyFact ergeben.
...
*Methodik:* Im Rahmen einer Software-Factory bildet die xref:glossary:glossary:master.adoc#glossar-Methodik[Methodik (Glossar)] die Grundlage für die Umsetzung von Geschäftsanwendungen.
...
Dazu bietet sie Konfigurationsanleitungen ausgewählter xref:glossary:glossary:master.adoc#glossar-Werkzeug[Werkzeug (Glossar)], z. B. für die Modellierung, die Programmierung, Tests oder die Fehlerverfolgung.
...
*Fachliche Referenzarchitektur:* Hierunter fällt eine fachliche Beschreibung der xref:glossary:glossary:master.adoc#glossar-Anwendungslandschaft[Anwendungslandschaft (Glossar)].
...
*Software-Referenzarchitektur:* Hierunter fällt eine software-technische Beschreibung der xref:glossary:glossary:master.adoc#glossar-Systemlandschaft[Systemlandschaft (Glossar)].
...
In einem spezifischen xref:glossary:glossary:master.adoc#glossar-Anwendungskontext[Anwendunskontextes (Glossar)] wird man in der Regel jedoch viel weitergehende Festlegungen innerhalb der Anwendungs-Architektur treffen, die die Fachlichkeit des Kontexts abbilden.
...
Zur Strukturierung der xref:glossary:glossary:master.adoc#glossar-Anwendungslandschaft[Anwendungslandschaft (Glossar)] werden drei fachliche Hierarchieebenen festgelegt:

*xref:glossary:glossary:master.adoc#glossar-Anwendungsdomaenelandschaft[Anwendungsdomäne (Glossar)]:* Auf der obersten Hierarchieebene werden Anwendungsdomänen gebildet: Anwendungsdomänen sind fachlich eng zusammengehörige Mengen von Anwendungssystemen
...
*xref:glossary:glossary:master.adoc#glossar-Anwendungssystem[Anwendungssystem (Glossar)]:* Ein Anwendungssystem ist eine zusammengehörende, logische Einheit aus Funktionen, Daten und Benutzungsschnittstellen.
...
* *xref:glossary:glossary:master.adoc#glossar-Geschaeftsanwendung[Geschäftsanwendung (Glossar)]:* Eine Geschäftsanwendung implementiert für sich genommen oder im Zusammenspiel mit anderen Geschäftsanwendungen einen oder mehrere Geschäftsprozesse einer Anwendungsdomäne.
...
* *xref:glossary:glossary:master.adoc#glossar-Querschnittssystem[Querschnittssysteme(Glossar)]* sind Anwendungssysteme, die Basisdienste für mehrere Geschäftsanwendungen bereitstellen, wie z. B. ein Behördenverzeichnis oder ein Outputmanagement.

* *xref:glossary:glossary:master.adoc#glossar-Anwendungskomponente[Anwendungskomponenten(Glossar)]:* Eine Anwendungskomponente beschreibt eine Menge funktional zusammenhängender Anwendungsfälle.
...
Die technische Entsprechung hierfür ist der Begriff der xref:glossary:glossary:master.adoc#glossar-Systemlandschaft[Systemlandschaft (Glossar)].
...
*Das xref:glossary:glossary:master.adoc#glossar-Service-Gateway[Service Gateway (Glossar)]:* Service-Gateway-Systeme haben die Aufgabe, Aufrufe von internen Anwendungssystemen an externe Systeme und Aufrufe von externen Systemen
...
*Die xref:glossary:glossary:master.adoc#glossar-Geschaeftsanwendung[Geschäftsanwendung (Glossar)]*: Eine Geschäftsanwendung implementiert allgemein die Geschäftsprozesse einer Domäne.
Geschäftsanwendungen können monolithisch strukturiert sein und alle Schichten der Software-Architektur umfassen, von der Benutzeroberfläche über Prozesse und xref:glossary:glossary:master.adoc#glossar-Geschaeftslogik[Geschäftslogik (Glossar)] bis hin zur Datenspeicherung.
...
*Das xref:glossary:glossary:master.adoc#glossar-Portal[Portal (Glossar)]:* Die Referenzarchitektur sieht keinen Portal-Server im klassischen Sinne vor: Es gibt weder einen systemübergreifenden Rahmen noch eine systemübergreifende Navigation.
...
*Das xref:glossary:glossary:master.adoc#glossar-Querschnittssystem[Querschnittssysteme(Glossar)]:* Ein Querschnittssystem bietet Geschäftsanwendungen querschnittlich genutzte Funktionalitäten an, etwa für die Bereitstellung von Schlüsseldaten.
...
image::servicekommunikation.dn.png[align="center"]
...
image::synchroner-service-aufruf.dn.png[align="center"]
...
image::http-messages-aufbau.dn.png[align="center"]
...
image::asynchroner-service-aufruf.dn.png[align="center"]
...
image::queueing.dn.png[align="center"]
...
image::jms-message-aufbau.dn.png[align="center"]
...
Die Gesamtheit aller Service-Provider und Service-Consumer eines internen IT-Systems wird als xref:glossary:glossary:master.adoc#glossar-Service-Gateway[Service Gateway (Glossar)] bezeichnet.
...
Zur Implementierung der Geschäftslogik kann die Geschäftsanwendung auch xref:glossary:glossary:master.adoc#glossar-Service[Services (Glossar)] anderer Geschäftsanwendung aufrufen.

*Das Portal:* Wie in Kapitel <<strukturierung-der-systemlandschaft>> beschrieben, nimmt das Portal die xref:glossary:glossary:master.adoc#glossar-Anwender[Anwender (Glossar)]-Aufrufe entgegen und leitet sie an die Geschäftsanwendung weiter.
...
Die Referenzarchitektur der technischen Infrastruktur, auch xref:glossary:glossary:master.adoc#glossar-IT-Architektur>[IT-Architektur (Glossar)] genannt, beschreibt den Aufbau der Betriebsumgebung für die IT-Systeme einer IsyFact-Systemlandschaft.
...
*  *xref:glossary:glossary:master.adoc#glossar-Auslieferung[Auslieferung (Glossar)]:* Mit dem Begriff „Auslieferung“ wird die Übergabe von Liefergegenständen aus der Hoheit der Software-Entwicklung in die Hoheit des Betriebs bezeichnet.
...
Im Releaseletter sind auch die Installationsanleitung inklusive der Parameter der Installation und die Aufstellung der mit dem xref:glossary:glossary:master.adoc#glossar-Release[Release (Glossar)] geschlossenen Fehler-Meldungen enthalten.
...
Auf die Überwachung der IT-Systeme wird im Dokument<<TODO anderes Dokument: KonzeptUeberwachung>> im Detail eingegangen.
