...
In der Informations- und Dienstezone erfolgt eine Zweifaktor-Berechtigungsprüfung:
...
-  Im Portal bzw. Gateway wird die Identität des Nutzers geprüft (Authentifizierung).
Der Nutzer muss dafür geheimes Wissen übermitteln (Name und Passwort für Anwender bzw. Client-ID und Secret für Anwendungen), das an einen zentralen Service (IAM-Service) in der Anwendungslandschaft weitergereicht wird.
