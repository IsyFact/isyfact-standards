...
Ein Sicherheitsaspekt ist der Schutz vor unberechtigtem Zugriff auf vertrauliche Daten.
Wie bereits in <<image-GA-AW2>> angedeutet, können Nutzer einer IsyFact-Anwendung / -Domäne nicht direkt darauf zugreifen, sondern nur über eine dazwischenliegenden Informations- und Dienstezone:
...
- In zukünftigen Versionen von IsyFact muss sich eine externe Anwendung erst über das Authentication-Gateway durch den IAM-Service authentifizieren lassen, bevor es über ein Service-Gateway Zugang zu einer IsyFact-Anwendung / -Domäne erhält.
...
Bevor der Zugang zu der IsyFact-Anwendung / -Domäne gewährt wird, erfolgt in der Informations- und Dienstezone eine Zweifaktor-Berechtigungsprüfung:

- Ein vorgeschalteter Loadbalancer (nicht im Bild) verlangt ein Client-Zertifikat des BVA/ITZ-Bund.
Clients (Browser oder Anwendung), die nicht im Besitz dieses Client-Zertifikats sind, werden direkt abgewiesen.
-  Im Portal bzw. Authentication-Gateway wird die Identität des Nutzers geprüft (Authentifizierung).
Der Nutzer muss dafür geheimes Wissen übermitteln (z.B. Name und Passwort für Anwender bzw. Client-ID und Secret für Anwendungen), das an einen zentralen Service (IAM-Service) in der Anwendungslandschaft weitergereicht wird.
...
Der Aufruf des Nutzers wird zusammen mit diesem Access-Token an die (Interaktions- bzw. Service-Schicht /) Systemgrenze der IsyFact-Fachanwendung weitergeleitet.
