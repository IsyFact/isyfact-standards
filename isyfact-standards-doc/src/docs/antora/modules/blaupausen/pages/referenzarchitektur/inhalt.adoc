...
In der Informations- und Dienstezone erfolgt eine erste (mehrstufige) Berechtigungsprüfung:

. (Loadbalancer: Zertifikat des BVA/ITZ-Bund (gültig? , BHKNZ?))
. Im Portal bzw. Gateway wird die Identität des Aufrufers geprüft (Authentifizierung) und es werden nur für die Anwendungslandschaft zugelassene durchgelassen.
Dafür wird ein zentraler Service (IAM-Service) in der Anwendungslandschaft genutzt.
Ist der Nutzer dem IAM-Service bekannt, wird gemäß der Autorisierungs- und Authentifizierungsprotokolle OAuth2.0 und OpenID Connect ein Access-Token ausgestellt, das auch die dem Nutzer zugeordneten Rollen enthält.
. Der Aufruf wird zusammen mit diesem Access-Token an die (Interaktions- bzw.) Service-Schicht / Systemgrenze der IsyFact-Fachanwendung weitergeleitet.
Dort wird (über ein Rolle-Rechte Mapping) geprüft, ob der Nutzer das zur Ausführung des gewünschten Service notwendige Recht besitzt.
