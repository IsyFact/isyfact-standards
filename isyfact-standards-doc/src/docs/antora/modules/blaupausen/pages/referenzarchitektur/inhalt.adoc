...
Ein Ziel der Referenzarchitektur (<<die-ziele-der-referenzarchitektur>>) ist die Gewährleistung von Sicherheitsaspekten als technische Querschnittsaufgabe.
Ein Teilbereich davon ist der Schutz vor unberechtigtem Zugriff auf vertrauliche Daten.
IsyFact gewährleistet das durch die Implementierung der standardisierten Protokolle zur Authentifizierung (OpenID Connect) und Autorisierung (OAuth2.0).
An den Außengrenzen der Anwendungslandschaft erfolgt die Authentifizierung des Anwenders und an den Systemgrenzen der Anwendungen die Prüfung der Autorisierung zur Nutzung des angefragten Zugriffs.
...
Geschäftsanwendungen einer IsyFact Anwendungslandschaft sind für Anwender von außen nur über die Zugriff-steuernden Komponenten Portalanwendung, Service-Gateway und zukünftige Authentication-Gateway erreichbar, die somit die sichtbare Außengrenze der Anwendungslandschaft darstellen.
Dort erfolgt die Authentifizierung des Anwenders (_schwarzes Schlüssel_-Symbol) mit Hilfe eines zentralen Querschnittservice je Anwendungslandschaft, dem IAM-Service.
Jeder Zugriff innerhalb der Anwendungslandschaft ist somit authentifiziert (_grünes Schlüssel_-Symbol).
Eine grobe Autorisierung von Zugriffen sollte ebenfalls an den Außengrenzen geschehen, z.B. durch Client-Zertifikate.

Die feingranulare Autorisierung zur Verwendung einer Funktion oder eines Service einer IsyFact-Geschäftsanwendung muss an den Systemgrenzen der Anwendung stattfinden (_Schloss_-Symbol). Es dürfen nur authentifizierte und autorisierte Zugriffe auf eine Anwendung möglich sein (_Dokument mit grünem Schlüssel_-Symbol).

Die Authentifizierungs- und Autorisierungsprotokolle _OpenID Connect_ und _OAuth2.0_ definieren verschiedene Verfahren zur Authentifizierung der Anwenderanmeldedaten (_schwarzes Schlüssel_-Symbol)  gegenüber dem IAM und die Ausstellung eines Access Token (_grünes Schlüssel_-Symbol) zur Autorisierung an den Systemgrenzen der Anwendung.
