...
In einer Algemeinen Anwendung gibt es z.B. die Geschäftsobjekte `Aufgabe`, `Zeitpunkt` und `Nutzer`.
...
In der einer allgemeinen Anwendung ist z.B. „Aspektanwendung“ eine Teilanwendung.
Zu dieser Teilanwendung gehören in der Schicht Nutzung die Oberfläche _Aspektanwendung_ und der Batch `AspektanwendungLoeschBatch`, in der Schicht Anwendungslogik die Fachkomponenten _Erstellen_, _Verwalten_ und _Datenpflege_ und der Schicht Datenhaltung die Entitäten `Aspektanwendung`, `Zeitpunkt`, `Nutzer`, `Intervall` und `Nutzerintervall`.
...
Die folgende Abbildung zeigt einen Ausschnitt der für eine allgemeine Anwendung erstellten Beans.
Diese werden grün dargestellt und besitzen „referenzierte“ Abhängigkeiten zu benötigten anderen Beans.

.Spring-Beans einer allgemeinen Anwendung
[id="image-SpringBeansBeispiel",reftext="{figure-caption} {counter:figures}"]
image::blaupausen:detailkonzept-anwendungskern/SpringBeansBeispiel.svg[align="center",width=80%,pdfwidth=80%]

<<image-SpringBeansBeispiel>> zeigt bereits, dass nicht alle Klassen der Anwendung als Beans konfiguriert werden: Für die Komponente
...
Ein Beispiel dafür ist die Klasse `VerwaltungImpl` der allgemeinen Anwendung.
