...
[[querschnittsservices-als-beans-konfigurieren]]
===== Querschnittsservices als Beans konfigurieren

Querschnittsservices (etwa für JMX, für Nachrichten, für die Versendung von Mails) sind als Beans zu konfigurieren.
Dies gilt unabhängig davon, ob die Querschnittsservices statisch verfügbar sind oder über Dependency Injection gesetzt werden (siehe Kapitel <<statischer-zugriff-auf-bean-instanzen-nur-in-sonderfaellen>>).
