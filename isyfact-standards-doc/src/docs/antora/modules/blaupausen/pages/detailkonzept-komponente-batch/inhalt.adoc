...
Enthält eine Anwendung mehrere Batches, so enthält sie auch mehrere BatchKonfigurationen und kann jeden Batch mit einem eigenen Benutzer ausstatten.
...
Der Batchrahmen fordert von der BatchKonfiguration die Bereitstellung der Informationen zum Benutzer, um diesen über die T-Komponente Sicherheit zu authentifizieren.
