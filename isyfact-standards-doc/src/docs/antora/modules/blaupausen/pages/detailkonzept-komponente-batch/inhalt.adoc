...
Der Batchrahmen verwendet den Baustein `isy-security`, um den anfragenden Benutzer über einen der Authentifizierungswege Client Credential oder Ressource Owner Password (deprecated) via `registrationId` zu authentifizieren.
...
|====
|Property |Beschreibung
|`registrationId` |Die ID der Client Registration aus `isy-security`, die zur Authentifizierung genutzt wird
|====
...
Enthält eine Anwendung mehrere Batches, so enthält sie auch mehrere BatchKonfigurationen und kann jeden Batch mit einer eigenen `registrationId` ausstatten.
...
Der Batchrahmen fordert von der BatchKonfiguration die Bereitstellung der `registrationId` zur ClientRegistration, um den Batch über den Baustein `isy-security` zu authentifizieren.
