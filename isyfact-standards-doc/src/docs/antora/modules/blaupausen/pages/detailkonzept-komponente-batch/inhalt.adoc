...
|`Batchrahmen.MaxWiederholungen` |Maximale Anzahl automatischer Neustarts für einen fehlerhaft abgebrochenen Batch. Mit Parameter `ignoriereRestart` wird Zähler auf 0 zurückgesetzt.
|====
...
[[maximale-anzahl-automatischer-neustarts]]
==== Maximale Anzahl automatischer Neustarts

Der Batchrahmen ermöglicht es, die Anzahl automatischer Neustarts eines fehlerhaft abgebrochenen Batches zu begrenzen.
Über den Konfigurationsparameter `Batchrahmen.MaxWiederholungen` wird eine Obergrenze für Restart-Versuche festgelegt.

Ist der aktuelle Restart-Zähler (also die Anzahl der bisherigen Restarts seit dem letzten erfolgreichen Start) größer oder gleich dem konfigurierten Wert, bricht der Batchrahmen den Restart vorzeitig ab.
Es wird eine `BatchrahmenMaxWiederholungenException` geworfen, die lediglich auf Info-Niveau geloggt wird.
Der Batch verbleibt im Status „abgebrochen".

Ein neuer Start mit dem Parameter `-start` (ggf. in Kombination mit `-ignoriereRestart`) setzt den Zähler zurück, so dass anschließend wieder Restarts möglich sind.

Ist `Batchrahmen.MaxWiederholungen` nicht konfiguriert oder auf einen negativen Wert (z. B. -1) gesetzt, gibt es keine Begrenzung der Neustarts.
...
|`oauth2ClientRegistrationId` |null |Die ID der OAuth 2.0 Client Registration aus `isy-security`, die zur Authentifizierung genutzt wird.
...
(optional)|60 |Die Zeit in Sekunden, die ein Token mindestens weiterhin gültig sein muss, um nicht erneuert zu werden.
...
[NOTE]
Abhängig vom gewählten Grant-Type wird eine erneute Authentifizierung gegebenenfalls nur dann durchgeführt, wenn ein durch die Spring-Implementierung vorgegebener Zeitraum vor dem Ablauf des Tokens überschritten wird.
Für weitere Informationen siehe xref:isy-security:nutzungsvorgaben/master.adoc#authentifizierungsmanager-authentifiziere-method-grant-type-note[Hinweis zur Methode `authentifiziere`] im Kapitel Authentifizierungsmanager der Nutzungsvorgaben Security.
|====
