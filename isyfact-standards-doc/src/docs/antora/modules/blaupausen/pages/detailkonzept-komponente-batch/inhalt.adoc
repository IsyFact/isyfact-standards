...
Der Batchrahmen verwendet den Baustein `isy-security`, um den anfragenden Benutzer über einen der Authentifizierungswege Client Credential oder Ressource Owner Password (deprecated) via `oauth2ClientRegistrationId` zu authentifizieren.
...
Der Batchrahmen fordert von der BatchKonfiguration die Bereitstellung der `oauth2ClientRegistrationId` zur ClientRegistration, um den Batch über den Baustein `isy-security` zu authentifizieren.
