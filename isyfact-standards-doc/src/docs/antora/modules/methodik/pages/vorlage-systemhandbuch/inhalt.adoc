...
:desc-listing-settings-von-maven: Datei settings.xml von Maven
[id="listing-settings-von-maven",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-settings-von-maven}
...
:desc-listing-maven-build-statement: Maven Build Anweisung
[id="listing-maven-build-statement",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-maven-build-statement}
[source,xml,subs="verbatim,attributes"]
----
...
[[konfigurationsdatei-cams-webagentconf]]
==== Konfigurationsdatei `cams-webagent.conf`

Über diese Datei wird der CAMS-Web-Agent konfiguriert.
...
Änderungen an den Einstellungen müssen bei heruntergefahrener Anwendung durchgeführt werden.
Dieses Verhalten gilt nicht für

* alle Einstellungen der Datei `log4j.properties`

Änderungen an diesen Einstellungen können bei laufender Anwendung ausgeführt werden.

[[konfigurationsdatei-log4j-properties]]
===== Konfigurationsdatei log4j.properties

Die `log4j.properties`-Datei bestimmt die Logging-Einstellungen und ist nach den Logging-Vorgaben aufgebaut.

Nach der Installation müssen in dieser Datei die Namen der künftigen Log-Dateien (Error, Info und Debug) angegeben werden.
Dazu ist in den folgenden Zeilen der [.underline]#HOSTNAME# durch den tatsächlichen Hostnamen zu ersetzen.

:desc-listing-apache-tomcat-workerlogforj: Konfiguration log4j.appender
[id="listing-apache-tomcat-workerlogforj",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-apache-tomcat-workerlogforj}
[source,xml,subs="verbatim,attributes"]
----
log4j.appender.ERROR_APPENDER.File=/var/log/isy-xxx-ga/error_log_*[.underline]#HOSTNAME#*_xxx-ga

log4j.appender.INFO_APPENDER.File=/var/log/isy-xxx-ga /info_log_*[.underline]#HOSTNAME#*_xxx-ga

log4j.appender.
DEBUG_APPENDER.
File=/var/log/isy-xxx-ga /debug_log_*[.underline]#HOSTNAME#*_xxx-ga
----

Die Log-Dateien werden mit diesen Einstellungen nach `/var/log/isy-xxx-ga` für `_XXX-GA_` geschrieben.

[[konfigurationsdatei-gaproperties]]
===== Konfigurationsdatei xxx.ga.properties
...
:desc-table-parameter-ga-properties: Konfigurationsparameter .ga.properties
...
Die Konfigurationsdateien der Batch-Anwendungen sind nach der Erstinstallation mit den Konfigurationsdateien der Tomcat-Anwendung identisch mit Ausnahme der speziellen Logging-Konfiguration für die Batches in der Datei `log4j-batch.properties` (siehe Kapitel <<konfigurationsdatei-log4j-batchproperties>>). Es ist möglich die bereits angepassten Konfigurationsdateien der Tomcat-Anwendung in die Konfigurationsverzeichnisse der Batch-Anwendungen zu kopieren. Anschließend müssen dort nur noch die abweichenden Einstellungen angepasst werden.

[[konfigurationsdatei-log4j-batchproperties]]
===== Konfigurationsdatei log4j-batch.properties

Diese Konfigurationsdatei ist analog zur Datei `log4j.properties` aufgebaut.
Sie wird für das Logging der Batches verwendet.
Der Unterschied zur `log4j.properties` liegt darin, dass ein eigener Pfad für das Logging verwendet wird und dass die Batch-Id an den Namen der Log-Datei angehängt wird.
Außerdem werden die Log-Dateien nicht rolliert.

[[konfigurationsdatei-jpaproperties-gateway]]
===== Konfigurationsdatei `jpa.properties`

Die Konfiguration ist identisch zu Kapitel <<konfigurationsdatei-jpaproperties-batch, Installation von XXX-GA und XXX-GA-Batch/Konfigurationssdatei jpa.properties>>.
...
Änderungen an den Einstellungen müssen bei heruntergefahrener Anwendung durchgeführt werden. Dieses Verhalten gilt nicht für

* alle Einstellungen der Datei log4j.properties

Änderungen an diesen Einstellungen können bei laufender Anwendung ausgeführt werden.

[[konfigurationsdatei-log4jproperties]]
===== Konfigurationsdatei log4j.properties

Die `log4j.properties`-Datei bestimmt die Logging-Einstellungen und ist nach den Logging-Vorgaben aufgebaut.

Nach der Installation müssen in dieser Datei die Namen der künftigen Log-Dateien (Error, Info und Debug) angegeben werden. Dazu ist in den folgenden Zeilen der [.underline]#HOSTNAME# durch den tatsächlichen Hostnamen zu ersetzen.

:desc-listing-log4j-einstellungen: Logging Einstellungen nach Logging-Vorgaben
[id="listing-log4j-einstellungen",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-log4j-einstellungen}
[source,xml,subs="verbatim,attributes"]
----
log4j.appender.ERROR_APPENDER.File=/var/log/isy-xxx-sgw/error_log_*[.underline]#HOSTNAME#*_xxx-sgw

log4j.appender.INFO_APPENDER.File=/var/log/isy-xxx-sgw /info_log_*[.underline]#HOSTNAME#*_xxx-sgw

log4j.appender.DEBUG_APPENDER.File=/var/log/isy-xxx-sgw /debug_log_*[.underline]#HOSTNAME#*_xxx-sgw
----

Die Log-Dateien werden mit diesen Einstellungen nach `/var/log/isy-xxx-sgw` für das _XXX-SGW_ geschrieben.
...
:desc-table-konfigurationsdateien: Konfigurationsdateien
[id="table-konfigurationsdateien", reftext = "{table-caption} {counter:tables}"]
.{desc-table-konfigurationsdateien}
[cols="2,6", options="header"]
|===
...
|`/etc/isy-xxx-ga/log4j.properties` |Logging-Konfiguration
|`/etc/isy-xxx-ga/cams-webagent.conf` |Konfiguration des CAMS
2+^|*XXX-SGW*
...
|`/etc/isy-xxx-sgw/log4j.properties` |Logging-Konfiguration
|`/etc/isy-xxx-sgw/cams-webagent.conf` |Konfiguration des CAMS
|===
...
:desc-table-mitarbeiterverwaltung: Datenbankfelder Tabelle Mitarbeiter
[id="table-mitarbeiterverwaltung", reftext = "{table-caption} {counter:tables}"]
.{desc-table-mitarbeiterverwaltung}
[cols="3,3,5,1,1", options="header"]
|===
|Feld |Datentyp |Beschreibung |PK |FK
|ID |Number(10, 0) |Technische ID der Person ^.^a|image::vorlage-systemhandbuch/checkmark.png[align = center, width=40%]|
...
|GRUPPE_ID |Number (10, 0) |Verweis auf die Gruppe, zu der die Person gehört (siehe Tabelle GRUPPE) | ^.^a|image::vorlage-systemhandbuch/checkmark.png[align = center, width=40%]
|===
...
:desc-table-indizes: Datenbankfelder Indizes
...
NACHNACHME |
|… | | ^.^a|image::vorlage-systemhandbuch/checkmark.png[align = center, width=40%]
||
|===
...
Die erste Zeile der `log4j.properties-Datei` (bzw. `log4j-batch.properties`) bestimmt den Log-Level:
...
Das Logging sollte auf der Produktiv-Umgebung nie auf DEBUG gestellt werden. Aufgrund der zahlreichen und umfangreichen Datenausgabe könnte es zu Performance-Problemen kommen. Gleichzeitig ist nicht sichergestellt, dass keine sicherheitsrelevanten Daten ausgegeben werden.
...
:desc-table-fehlercodes: Fehlercodes Datenbank
[id="table-fehlercodes", reftext = "{table-caption} {counter:tables}"]
.{desc-table-fehlercodes}
...
:desc-table-fehlerszenarien: Fehlerszenarien
[id="table-fehlerszenarien", reftext = "{table-caption} {counter:tables}"]
.{desc-table-fehlerszenarien}
