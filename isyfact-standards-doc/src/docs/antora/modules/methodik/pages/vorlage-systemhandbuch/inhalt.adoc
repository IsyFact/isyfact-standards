...
Das Kapitel <<bau-anwendung>> beschreibt, wie aus dem Quellcode die Deployment-Einheiten entstehen.
...
Für den Betrieb der Anwendung {anwendung} empfiehlt es, einen separaten Nutzer und eine Gruppe anzulegen.
...
Für Anwendungen, die als ausführbares Spring Boot-Fat-JAR ausgeliefert werden, empfiehlt die Spring Boot-Dokumentation den Betrieb über einen `systemd`-Service.

NOTE: Siehe https://docs.spring.io/spring-boot/how-to/deployment/installing.html#howto.deployment.installing[Spring Boot Deployment Guide]

.1. Artefakt bereitstellen
* Kopieren Sie `{it-system-ga}.jar` nach `/opt/{it-system-ga}/{it-system-ga}.jar`.
* Setzen Sie Besitz und Rechte, damit `{it-system-ga}` das JAR lesen kann (`chown {it-system-ga}:{it-system-ga}` und `chmod 750`).

.2. Service-Benutzer anlegen
* Legen Sie, falls nicht vorhanden, einen Systembenutzer ohne Login an (`useradd --system --shell /usr/sbin/nologin {it-system-ga}`).

.3. `systemd`-Unit erstellen
Speichern Sie die Unit-Datei unter `/etc/systemd/system/{it-system-ga}.service`.

:desc-listing-systemd-service: {it-system-ga}: systemd-Service
[id="listing-systemd-service",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-systemd-service}
[source,ini,subs="verbatim,attributes"]
----
[Unit]
Description={anwendung-ga} (Spring Boot)
After=network.target

[Service]
User={it-system-ga}
Group={it-system-ga}
WorkingDirectory=/opt/{it-system-ga}
ExecStart=/usr/bin/java -jar {it-system-ga}.jar --spring.config.location=file:/etc/{it-system-ga}/
...
* `systemctl enable --now {it-system-ga}.service`

Damit wird das ausführbare JAR automatisch beim Booten gestartet und kann über `systemctl status {it-system-ga}` überwacht werden.
...
Für Wartungszwecke kann es notwendig sein, dass die {anwendung-ga} keine weiteren Anfragen vom Loadbalancer mehr zugewiesen bekommt.
...
Nach der Installation liegen die Konfigurationsdateien des {anwendung-batch} unter `/etc/{it-system-batch}`.
...
====
Inhalt::
_Beschreibung der Unterschiede bei der Konfiguration des {anwendung-batch} gegenüber der Konfiguration der {anwendung-ga}._
...
Änderungen an den Konfigurationsdateien müssen bei heruntergefahrenem Service-Gateway durchgeführt werden.

IMPORTANT: Dieses Verhalten gilt nicht für die Konfigurationsdatei `logback.xml`.
Änderungen an dieser Konfigurationsdatei können im laufenden Betrieb ausgeführt werden.
...
Für Wartungszwecke kann es notwendig sein, dass das {anwendung-sgw} keine weiteren Anfragen vom Loadbalancer mehr zugewiesen bekommt.
Dazu überprüft das Loadbalancer-Servlet regelmäßig, ob die folgende (leere) Datei existiert.
...
|{it-system-ga}.properties |Konfiguration des IT-Systems
...
|isAlive |Steuerdatei für Loadbalancer
|{it-system-sgw}.properties |Konfiguration des IT-Systems
|jpa.properties |Konfiguration der Datenbank-Verbindung
|logback.xml |Logging-Konfiguration
|===
...
In diesem Abschnitt wird das Ausführen der Batches beschrieben.
Die Batches sind unabhängig voneinander, d.h. sie können in beliebiger Reihenfolge ausgeführt werden.

Die Batches werden auf der Kommandozeile über ein Startskript gestartet.
...
IMPORTANT: Eine wichtige Voraussetzung für die Ausführung von Batches ist das Setzen der Umgebungsvariable `JAVA_HOME` auf den korrekten Wert, vor allem, wenn mehrere Java-Versionen installiert sind.
...
Die IT-Systeme der {anwendung} stellen folgende Standard-Metriken über den Endpunkt `/actuator/metrics` zur Verfügung.
...
|System
|Metriken zur CPU-Auslastung, dem Dateisystem und der Uptime.
|system. +
process. +
disk.

|Application Startup
|Metriken zur Startzeit.
|application.
...
_Ebenso müssen Lücken gefüllt werden, da sie systemspezifisch sind._
====

[[monitoring-batches]]
==== Monitoring Batches

Die Batches werden durch die Skripte zunächst ohne JMX gestartet.
...
Folgender Test kann für jedes IT-System der Anwendung ausgeführt werden, um zu prüfen, ob es läuft:
...
Die Log-Datei der Anwendung darf nach dem Starten des JARs keine Fehler enthalten.
...
|_{anwendung-ga}, +
{anwendung-batch}_
|_Das System startet nicht. Fehlercode: ..._
