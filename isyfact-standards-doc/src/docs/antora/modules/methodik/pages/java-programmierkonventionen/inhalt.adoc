...
Vorgaben für die Struktur einzelner Dokumente finden sich im xref:werkzeuge:handbuch_dokumentation/master.adoc[Handbuch für technische Autoren].
...
NOTE: Werkzeug, mit dem sich Unterschiede zwischen zwei Textdateien xref:glossary:literaturextern:inhalt.adoc#litextern-Synopse[synoptisch] darstellen lassen (diff unter xref:glossary:literaturextern:inhalt.adoc#litextern-Unix[Unix]; .UltraEdit)
