...
[[kryptographie]]
==== Kryptographie
In der kryptografischen Absicherung der Datenbankverbindungen werden Zufallszahlen benötigt. Diese werden mit `dev/urandom` generiert. Im Gegensatz zu `dev/random` blockiert dieses die Datenbankverbindungen nicht auf lange Zeit um Zufallszahlen zu generieren.

WARNING: `dev/urandom` liefert, um die Folgeprozesse nicht zu blockieren, im Zweifelsfall schwächere Zufallszahlen. Dies kann zu Risiken beim Einsatz von kryptografischen Funktionen führen.
