...
Das Werkzeug "Enterprise Architect" (nachfolgend kurz "EA") unterstützt im Kontext der IsyFact in erster Linie die Modellierung und Diagrammerstellung im Zuge der Systemspezifikation und Geschäftsprozessdokumentation.
...
Die Dokumente Systemspezifikation und Geschäftsprozesse werden nach den hier beschriebenen Vorgaben erstellt.
...
Wird dieselbe Entität in mehreren Diagrammen verwendet, wird diese nicht neu angelegt, sondern aus den UML-Profilen herausgezogen.
Nun kann die Entität aus dem Project Browser heraus in anderen Diagrammen genutzt werden.
...
Im Dialog menu:Tools[Options > Diagram] kann der Rahmen eines Diagramms beim Druck (Checkbox btn:[Print with Border]) oder Export über den Zwischenspeicher (Checkbox btn:[On Clipboard Images]) unterdrückt werden.
...
Beide Vorgehen können in unterschiedlichen Situationen sinnvoll sein: Wenn z.B. eine Spezifikation eines bestehenden Systems "kopiert" wird, um das neue System zu beschreiben, dann sollen beide Spezifikationen nicht vermischt werden.
...
[[vorgehen-erstellung-systementwurf]]
== Vorgehen zur Erstellung des Systementwurfs

Der Enterprise Architect kann für die Modellierung folgender Themenbereiche eingesetzt werden:
...
Neben den Diagrammen sind vor allem die konkreten Komponenten mit ihren Interfaces, Klassen und Tabellen ein wichtiges Ergebnis der Modellierung.
...
Mit dem Enterprise Architect kann das im xref:vorlage-systementwurf/antora-master.adoc#datenmodell[Systementwurf] beschriebene Datenmodell so erstellt werden, dass gleichzeitig eine grundlegende Implementierung entsteht:

. Aus dem technischen Datenmodell werden die Java-Klassen für die Entitäten generiert.
. Die generierten Java-Klassen werden mit Annotationen für das DB-Mapping ergänzt.
. Aus den annotierten Klassen wird mithilfe der entsprechenden Hibernate-Werkzeuge das passende DB-Schema generiert.
. Dieses Schema wird als physisches Datenbankschema in den Enterprise Architect importiert.
Damit erhält man eine Darstellung der Entitäten, die die Grundlage für die Grafiken zum Datenmodell im Systementwurf sind.
