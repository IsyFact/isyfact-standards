...
Ohne Installation kann die Browser-Variante über xref:literaturextern:inhalt.adoc#litextern-diagrams-net[diagrams.net] direkt aufgerufen werden.
Für zahlreiche Betriebssystem-Plattformen stehen außerdem Installationspakete bereit.
Darüber hinaus gibt es Unterstützung für weit verbreitete IDEs, beispielsweise für xref:literaturextern:inhalt.adoc#litextern-diagrams-net-intellij[IntelliJ IDEA], allerdings ohne Konfigurationsmöglichkeit, oder xref:literaturextern:inhalt.adoc#litextern-diagrams-net-vs-code[Visual Studio Code].
