...
Die Service-Logik besteht, wie im xref:blaupausen:detailkonzept-komponente-service/master.adoc[Detailkonzept Komponente Service] beschrieben, aus einem Remote-Bean, einer Exception-Fassade und einer Service-Fassade pro Service.
...
Diese Klassen werden üblicherweise in einer Bibliothek pro Service bereitgestellt, deren Benennung in xref:methodik:namenskonventionen/master.adoc[Namenskonventionen] im Abschnitt der Paketierung als Bibliothek spezifiziert wird.
