= Nutzungsvorgaben Bedienkonzept
...
// tag::inhalt[]
Dieses Dokument enthält die technische Dokumentation zum Bedienkonzept.
...
Die Bootstrap-Styles wurden entsprechend den Definitionen im Bedienkonzept (Aussehen, Form, Farbe und Effekte) angepasst.
...
4.  Zum Abschluss müssen die Konformität mit dem Bedienkonzept, die visuell fehlerfreie Darstellung und fehlerfreie Funktionalität der Applikation überprüft werden.
