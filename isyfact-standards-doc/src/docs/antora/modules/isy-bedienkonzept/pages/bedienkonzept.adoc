...
Ein Button ist ein Bedienelement, das als Aktionselement designt und verwendet wird.
...
Alle anderen Buttons werden in der Sekundärfarbe verwendet, falls keine spezifische Funktion zugrunde liegt.
...
Auf <<formulare,Formularseiten>> oder auch Full-Page Designs wird hingegen der Primär-Button links unten gesetzt.
...
Falls die <<image-buttoncolors,farblich gefüllten Buttons>> zu bunt für das Frontenddesign sind, können auch die invertierten Varianten benutzt werden:
...
Aus diesem Grund sollten Icon-Buttons mit Bedacht eingesetzt werden und können somit nicht jeden beliebigen beschrifteten Button ersetzen.
Am häufigsten wird diese Button-Art in der Aktionsspalte einer Tabelle benutzt.
...
Im Sinne der Barrierefreiheit sollte bei Icon-Buttons immer ein beschreibendes <<alt-tag,Alt-Attribut>> vorhanden sein, um Screenreadern den Zugang zum Bedienelement zu gewähren.
Ein Tooltip ist dafür nicht ausreichend.
...
Um den Nutzenden zu vermitteln welche Funktion oder Eigenschaft ein- bzw. ausgeschaltet werden kann, sollte ein Label rechts neben dem Input Switch platziert werden.

// ToDo: neues Bild einfügen
[[image-inputswitch]]
.Input Switch
image::Button_Toggle.png[align="center"]
...
Hierfür werden unter dem Button „Exportieren“ die Funktionen „Exportieren als XLS“, „Exportieren als PDF“ und „Exportieren als PNG“ zusammengefasst.

Menü-Buttons bestehen aus einem <<label>> und einem Pfeil Icon.
Optional kann vor dem Label ein weiteres <<icons,Icon>> hinzugefügt werden, dass zur Funktion des Menü-Buttons passt.
Wird der Menü-Button geklickt, werden die Funktionen wie in einem Dropdown-Menü angezeigt.
