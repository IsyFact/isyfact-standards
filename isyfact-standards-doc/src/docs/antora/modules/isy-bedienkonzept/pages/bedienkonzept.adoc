...
* Klick = Bearbeiten -> erste Aktion = Bearbeiten (wie im Beispiel)
* Die aktive/Focus Zeile hat die Farbe Primary und der Hover entspricht dem PrimeNG Standard
...
Die Fachlichkeit sollte sich auf ein subset von wichtigen Informationen beschränken.
* Überlaufende Textlabels werden abgetrennt und mit einem Tooltip ergänzt.
* Wähle Spalten Titel so kurz wie möglich und prägnant. Abkürzungen sind zulässig.
* Die initiale oder maximale Breite von Spalten muss sich an dem zu erwartenden Inhalten orientieren, auch wenn der Titel dadurch abgeschnitten wird (Es gibt keine feste maximale Buchstabenanzahl).
...
* Erzeuge keine Tabellen mit 1 oder 2 Spalten.
...
Die Clicktargetarea des Icons ist dabei 44px x 44px groß (siehe Beispiel).

* Der erste Klick auf die Sortierungsicons, sollte die Tabelle nach den Werten in der gewählten Spalte, aufsteigend sortieren.
* Das wechselnde Icon zeigt die aktuelle Sortierung an.

[[filterung-tabelle]]
==== Filterung in einer Tabelle

Es bietet sich an, dass Tabellen auch filterbar gestaltet werden. Dafür können verschiedene fachliche Filter definiert werden.
...
* Aktive Filter sollten deutliche erkennbar sein.

[[selektionsverhalten-tabelle]]
==== Selektionsverhalten in einer Tabelle

Wie allgemeinen Kapitel zu Tabellen beschrieben, gibt es verschiedene Möglichkeiten ein Element in der Tabelle zu selektieren.
...
Über einen "Mehr anzeigen"-Button, Endless Scrolling oder einen Paginator kann der Benutzer sich bei Bedarf mehr Daten anzeigen lassen.
