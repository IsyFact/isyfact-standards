...
Das Iconing in Anwendungen soll einem einheitlichen Stil Folgen und sich primär an Icons der Bibliothekt von:
xref:glossary:literaturextern:inhalt.adoc#litextern-prime-faces-icons[Prime Faces Icons]
bedienen. Sollten fachlich notwendige Icons in Prime Faces nicht vorhanden sein, können in Ausnahmefällen Icons aus der
Bibliothek Font Awesome bezogen werden. Beide Fonts müssen in die Webanwendung eingebunden werden.
Wichtig ist es dabei darauf zu achten, dass die Icons im Stil und ihrer Art so ähnlich wie möglich
zu den primär verwendeten Icons sind. Es ist darauf zu achten, dass:
-Gleiche Bedeutungen mit unterschiedlichen Icons entsteht
(Beispiel: Warenkorb = Einkaufwagen und Beutel), hier wird die von Prime Faces vorgegebe Lösung genutzt.
Ist ein Icon für sich nicht ausreichend aussagekräftig ist der Nutzen/die Anwendung mit einem Textattribut zu ergänzen.

Zur Sicherstellung der Barrierefreiheit ist ein Icon mit einem entsprechenden alt-Tag zu versehen.
(Beispiel: Stift = bearbeiten alt="bearbeiten")
Icons sind einfarbig und komplementär zum Hintergrund zu gestalten.
(Beispiel: schwarze Icons auf weißem Hintergrund)
