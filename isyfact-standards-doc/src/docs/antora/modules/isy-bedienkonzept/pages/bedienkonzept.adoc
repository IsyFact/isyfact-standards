...
** *Breadcrumb* (ähnlich einer xref:glossary:literaturextern:inhalt.adoc#litextern-location-breadcrumb[Location Breadcrumb]): In dieser werden der Objekttitel und der zum Objekt gehörige „Ort" angezeigt (siehe <<image-applikationdetailbeispiel>>). Dieser Ort kann je nach Anzahl der Hierarchieebenen variieren.
...
* Ein Paginator ist nur ein Hilfswerkzeug und sollte nicht als Standard genutzt werden.
Bestehen keine systembedingten Restriktionen, sollte auch kein Paginator eingesetzt werden.
...
In diesem Abschnitt werden Design Patterns für die Eingabe von Daten beschrieben.
...
In diesem Abschnitt werden Design Patterns für die Selektion von Daten beschrieben.
...
In diesem Abschnitt werden Design Patterns für die Darstellung von Daten beschrieben.
...
image::Formulare_Raster.drawio.svg[align="center"]
...
* Floating-Labels sollten verwendet werden. Sie dienen als initiale Platzhalter im Eingabefeld und ersetzen damit traditionelle Platzhaltertexte. Dies trägt zu einem minimalistischen und sauberen Design bei.
* Sobald ein Eingabefeld fokussiert wird oder eine Eingabe enthält, sollte das Floating-Label sich nach oben bewegen, an den oberen Rand des Eingabefeldes. Dies signalisiert dem Benutzer, dass das Feld aktiv ist und erleichtert die Zuordnung der Label zum entsprechnden Eingabefeld.
...
image::Formular_BeispielBAV.drawio.svg[align="center"]

.Gruppierung über 3 Spalten
[[image-gruppierungdreispalten]]

image::Formular_Gruppierung3Spalten.drawio.svg[align="center"]

.Gruppierungen über 1 bzw. 2 Spalten
[[image-gruppierunguebereinszwei]]

image::Formular_Gruppierung2Spalten.drawio.svg[align="center"]


.Gruppierungen über 1 Spalte
[[image-gruppierunguebereinespalte]]

image::Formular_Gruppierung1Spalte.drawio.svg[align="center"]

.Gruppierung mittels Whitespace
[[image-gruppierungwhitespace]]

image::Formulare_Gruppierung_Whitespace.drawio.svg[align="center"]
...
image::Formulare_FixeFlexibleSpalten.drawio.svg[align="center"]


*Richtlinien zur Anwendung*

* Formularspalten sollten flexibel gestaltet werden, um den horizontal verfügbaren Platz bei größeren Auflösungen optimal zu nutzen.
* Bei ausreichendem Platz sollten Formularinhalte nebeneinander dargestellt werden. Bei geringeren Auflösungen sollten sie untereinander angeordnet werden, um sich der begrenzten Breite anzupassen.
* Die Größenanpassung erfolgt innerhalb von gewissen Bereichen:
** Wird das Browserfenster kleiner als der darzustellende Inhalt (i.d.R. bei Tablets und Smartphones), so wird das gesamte Fenster scrollbar.
...
* Auf der Webseite
xref:glossary:literaturextern:inhalt.adoc#litextern-vicheck[ViCheck]
...
zum Einsatz.
Diese Schriftart wird nur fehlende UNICODE zeichen in der BundesSans ergänzen, wodurch eine Unkonformität mit der BITV durch eventuelle Verwechslungen mit einem großen I und kleinen l ausgeschlossen werden kann.
...
Hier ist der Kontrast größtmöglich zu wählen (z.B. dunkelgraue Schrift auf weißen Hintergrund). Ist dies aus technischer oder fachlicher Notwendigkeit nicht möglich, so muss der Kontrast bei kleiner Schrift mindestens 4:1 entsprechen. Überschriften und großer/dicker Text muss mindestens einen Kontrast von 3:1 aufweisen.

[[icons-ueberscrift]]
=== Icons

Der Einsatz von Icons in Anwendungen soll einem einheitlichen Stil folgen und sich primär an Icons der Bibliothek xref:glossary:literaturextern:inhalt.adoc#litextern-prime-faces-icons[Prime Faces Icons] bedienen.
Sollten fachlich notwendige Icons in Prime Faces nicht vorhanden sein, können in Ausnahmefällen Icons aus der Bibliothek Font Awesome bezogen werden.
Beide Fonts müssen in die Webanwendung eingebunden werden.
Wichtig ist es dabei darauf zu achten, dass die Icons im Stil und ihrer Art so ähnlich wie möglich zu den primär verwendeten Icons sind.
Es ist darauf zu achten, dass:

* Gleiche Bedeutungen mit unterschiedlichen Icons entstehen (Beispiel: Warenkorb = Einkaufwagen und Beutel). Hier wird die von Prime Faces vorgegeben Lösung genutzt.
Ist ein Icon für sich nicht ausreichend aussagekräftig ist der Nutzen/die Anwendung mit einem Textattribut zu ergänzen.

* Zur Sicherstellung der Barrierefreiheit ist ein Icon mit einem entsprechenden alt-Tag zu versehen (Beispiel: Stift = bearbeiten alt="bearbeiten").

*  Icons sind einfarbig und komplementär zum Hintergrund zu gestalten (Beispiel: schwarze Icons auf weißem Hintergrund).
Sollten Icons aus fachlicher oder technischer Notwendigkeit nicht mit einem höchstmöglichen Kontrast umzusetzen sein, so muss ein Kontrast von mindestens 4:1 sichergestellt sein.

Eine Gesamtübersicht der zur Verfügung stehenden Icons ist auf xref:glossary:literaturextern:inhalt.adoc#litextern-prime-faces-icons[Prime Faces Icons] zu finden.

In der folgenden Tabelle sind mögliche Icons und deren Funktion dargestellt, sowie die dazugehörige CSS-Klasse.

// Die CSS-Klassen beziehen sich auf primeicons
