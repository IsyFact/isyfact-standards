...
.icon:university[] Architekturregel
****
Logback wird durch eine Anwendung niemals direkt, sondern ausschließlich über die Bibliothek `isy-logging` aufgerufen. Dadurch wird die einheitliche Nutzung von _logback_ sichergestellt. Ausnahme sind hierbei externe Bibliotheken (Siehe <<logging-externer-bibliotheken>>).
****
