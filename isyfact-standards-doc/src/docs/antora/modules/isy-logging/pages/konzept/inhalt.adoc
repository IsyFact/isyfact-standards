...
****
Beim Loggen der Nachrichten sind grundsätzlich die Vorgaben der Datenschutzgrundverordnung (xref:glossary:literaturextern:inhalt.adoc#litextern-dsgvo[DSGVO]) einzuhalten.
...
.icon:university[] Architekturregel
****
Logback wird durch eine Anwendung niemals direkt, sondern ausschließlich über die Querschnittsbibliothek `isy-logging` aufgerufen.
