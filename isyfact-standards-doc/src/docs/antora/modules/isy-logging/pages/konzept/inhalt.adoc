...
Die Vorgaben zu Logdateien sowie deren Rotation und Komprimierung sind in den Nutzungsvorgaben _Logging_  xref:nutzungsvorgaben/master.adoc#einleitung[Logging - Nutzungsvorgaben] beschrieben.

[[logging-framework]]
=== Logging-Framework

Als Logging-Framework wird in der IsyFact _logback_ eingesetzt (Siehe <<TODO anderes Dokument: ProduktKatalog>>).
...
image::isy-logging:konzept/logfass.png[align="center"]
...
image::isy-logging:konzept/interface-isylogger.png[align="center",width=80%,pdfwidth=80%]
...
image::isy-logging:konzept/iaufbvLogent.png[align="center"]
...
Durch die Anwendungen geloggte Attribute werden durch die Entität `ETY_Logeintrag` beschrieben. Die zusätzliche Attribute werden durch `ETY_Marker` beschrieben (Siehe <<entitaeten>>).
...
`de.bund.bva.isyfact.logging.hilfsklassen.TestZielKlasse.setzeName Exception(de.bund.bva.Isyfact.logging.hilfsklassen.TestZielParameterPerson,java.lang.String)`
|dauer a|
...
Die Wrapper-Logs sind Teil des Drittsystem Tomcats (siehe Tanuki Service Wrapper im <<TODO anderes Dokument: ProduktKatalog>>).
...
Die in den Logs der Apache HTTP-Server bereitgestellten Informationen werden im Konzept HTTP-Server Apache <<TODO anderes Dokument: NutzungskonzeptHTTPServer>> beschrieben.
...
Sollten die Requestparameter fachliche Daten enthalten (bspw. weil Suchanfragen über einen REST-Webservice realisiert wurden), so sind alle Apache-Logs des jeweiligen Webservers als „Fachdaten“ zu kennzeichnen (siehe xref:nutzungsvorgaben/master.adoc#einleitung[Logging - Nutzungsvorgaben]).
