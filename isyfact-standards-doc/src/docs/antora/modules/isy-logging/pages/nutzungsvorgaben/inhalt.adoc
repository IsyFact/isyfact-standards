...
Die Korrelations-ID (siehe xref:nutzungsvorgaben/master.adoc#einleitung[Logging - Konzept]) ist in jedem Eintrag mitzuloggen, damit die Logeinträge einzelnen Aufrufen zugeordnet und über die Komponenten der xref:glossary:glossary:master.adoc#glossar-Anwendungslandschaft[Anwendungslandschaft] verfolgt werden können.
...
Beim Einsatz von xref:glossary:glossary:master.adoc#glossar-Bibliothek[Bibliotheken], die nicht nach der IsyFact entwickelt wurden, muss daher unterschieden werden:
