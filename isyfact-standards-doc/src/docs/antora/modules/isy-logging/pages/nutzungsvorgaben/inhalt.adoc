...
Falls die Anwendung Service-Schnittstellen besitzt, muss diese Bean im Anwendungssystem so überschrieben werden, dass sie für diese Schnittstellen deaktiviert ist.
...
Stellt eine Anwendung beispielsweise REST-Schnittstellen unter `"/rest-endpoint1"`, `"/api/rest/example/endpoint2"` und `"/api/rest/example/endpoint3"` bereit, muss die Bean im Anwendungssystem wie folgt überschrieben werden:
...
[[neueinlesen-eines-geaenderten-konfigurationsparameters]]
