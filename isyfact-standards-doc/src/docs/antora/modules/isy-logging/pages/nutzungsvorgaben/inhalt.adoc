...
Diese ist analog zur Identifikation, die auch bei den Ausnahme-IDs (siehe <<TODO anderes Dokument: KonzeptFehlerbehandlung>>) verwendet wird.
+
...
|SYSTEM-ID |_Durch den Technischen Chefdesigner für die jeweilige Anwendung in Abstimmung mit dem Auftraggeber festzulegen (siehe <<TODO anderes Dokument: IsyFactNamenskonventionen>>)._ a|Name der Anwendung bzw. des Batches.
...
Zu beachten ist, dass die Klasse `IsyHttpInvokerClientInterceptor`, welche durch den Baustein Service bereitgestellt wird (siehe <<TODO anderes Dokument: DetailkonzeptKomponenteService>>), bereits einen entsprechenden Aufruf des Invokers durchführt.
...
Die Korrelations-ID (siehe xref:nutzungsvorgaben/master.adoc#einleitung[Logging - Konzept]) ist in jedem Eintrag mitzuloggen, damit die Logeinträge einzelnen Aufrufen zugeordnet und über die Komponenten der xref:glossary:glossary:master.adoc#glossar-Anwendungslandschaft[Anwendungslandschaft (Glossar)] verfolgt werden können.
...
Die Ablage der Konfigurationsdatei in Sourcen und Kompilaten ist durch das Konzept <<TODO anderes Dokument: KonzeptKonfiguration>> definiert.
...
Sollte ein Logeintrag länger als dieser Wert sein, so wird der Logeintrag gekürzt (siehe xref:nutzungsvorgaben/master.adoc#einleitung[Logging - Konzept]).
...
Im Folgenden werden die Spring-Konfigurationen zur Integration von logback in Spring und zur Konfiguration der genutzten Hilfsmechanismen (vgl. xref:nutzungsvorgaben/master.adoc#einleitung[Logging - Konzept]) beschrieben.
...
Beim Einsatz von xref:glossary:glossary:master.adoc#glossar-Bibliothek[Bibliotheken (Glossar)], die nicht nach der IsyFact entwickelt wurden, muss daher unterschieden werden:
...
In diesem Abschnitt werden Auswertungen beschrieben, die auf den Logs der xref:glossary:glossary:master.adoc#glossar-Anwendungslandschaft[Anwendungslandschaft (Glossar)] durchgeführt werden können müssen.
...
Detaillierte Informationen zur Systemprüfung und der zu erstellenden Logeinträge ist in <<TODO anderes Dokument: KonzeptUeberwachung>> beschrieben.
|====
...
Dies wird bereits durch den Baustein HTTP Invoker umgesetzt (siehe <<TODO anderes Dokument: NutzungsvorgabenHttpInvoker>>), sodass hierfür keine Anpassung notwendig ist.
...
verwendet werden (vgl. <<TODO anderes Dokument: konzeptlogging>>). +
