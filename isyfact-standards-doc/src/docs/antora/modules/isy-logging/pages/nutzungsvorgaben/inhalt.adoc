...
=== Einbinden des Frameworks in der Anwendung

Zusätzlich zu der Deklaration als Abhängigkeit muss für isy-logging der Build der verwendenden Anwendung angepasst werden, sodass per AspectJ das Performance-Logging zur Build-Zeit der Anwendung eingebunden werden kann. Hierfür kann folgende Konfiguration genutzt werden:
...
Der MDC wird über eine statische Methode gesetzt, und zwar pro Thread:
 ...
 MDC.put("korrelationsId", "<Korrelations-ID>");
...
Die IDs müssen hintereinander gehängt, getrennt durch ein Semikolon, im Kontext gesetzt werden, bspw.:
 ...
 MDC.put("korrelationsId", "c15638a2-4c38-4d18-b887-5ebd2a1c427d;f60143b3-3408-4501-9947-240ec1c48667;c893d44f-3b8e-446e-a360-06a520440e64");

Am Ende der Verarbeitung ist der MDC wieder zu entfernen:

 MDC.remove("korrelationsId");
...
[[logging-von-webclient-aufrufen-mit-isyrestlogger]]
===== Logging von WebClient-Aufrufen mit IsyRestLogger

[cols="2s,5"]
|====
|Beschreibung |Es wird ein Nachbarsystem via ``WebClient`` aufgerufen (REST-Aufruf).
|Logging a|
Für das Logging von `WebClient`-Aufrufen stellt `isy-logging` die Klasse `IsyRestLogger` bereit.
Sie erstellt eine `ExchangeFilterFunction`, die in die ``WebClient``-Builder-Kette integriert wird.

Die erstellten Logeinträge entsprechen den Ereignisschlüsseln für Nachbarsystemaufrufe (EISYLO01011 – EISYLO01015, vgl. <<ereignisschluessel-isy-logging>>).

Die Konfiguration erfolgt über die gleichen Properties wie der `boundaryLogInterceptor` (Präfix `isy.logging.boundary`, vgl. <<loggingmethodinterceptor>>).

.Voraussetzung: spring-webflux als explizite Abhängigkeit
Die Nutzung von `IsyRestLogger` erfordert die explizite Hinzufügung der `spring-webflux`-Abhängigkeit zur Anwendung, da `isy-logging` diese nur als optionale Abhängigkeit mitliefert.
Weitere Details dazu finden sich im xref:konzept/inhalt.adoc#isyrestlogger[Logging - Konzept].
