...
Für die Authentifizierung nutzt der Baustein Task Scheduling die xref:isy-security:konzept/master.adoc#aussensicht-der-komponente-security[Schnittstelle zur Authentifizierung] aus dem xref:isy-security:konzept/master.adoc#absicherung-task[Baustein Security] und präsentiert dem Service das so erhaltene Access-Token zur Autorisierung.
Die bisherige Art der Authentifizierung des Tasks als 'technischer Nutzer' (mit Nutzername u. Passwort) soll abgelöst werden durch eine Authentifizierung als 'Client' (mit Client-ID u. -Secret, vergeben vom IAM-System).
Für eine weiche Migration bietet die Konfiguration von Tasks dazu die Möglichkeit, alternativ die Authentifizierungs-Credentials Nutzername & Passwort bzw. Client-ID & Client-Secret zu setzen.
...
Für die Authentifizierung nutzt der Batchrahmen die xref:isy-security:konzept/master.adoc#aussensicht-der-komponente-security[Schnittstelle zur Authentifizierung] aus dem xref:isy-security:konzept/master.adoc#absicherung-task[Baustein Security] und präsentiert dem Service das so erhaltene Access-Token zur Autorisierung.
Die bisherige Art der Authentifizierung des Batch als 'technischer Nutzer' (mit Nutzername u. Passwort) soll abgelöst werden durch eine Authentifizierung als 'Client' (mit Client-ID u. -Secret, vergeben vom IAM-System).
Für eine weiche Migration bietet die Konfiguration von Batches dazu die Möglichkeit, alternativ die Authentifizierungs-Credentials Nutzername & Passwort bzw. Client-ID & Client-Secret zu setzen.
