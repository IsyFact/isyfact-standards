...
Für die Nutzung gesicherter Services muss ein gestarteter Task sich selbst gegenüber dem IAM-Service authentifizieren.
...
Die bisherige Art der Authentifizierung über den 'Resource-Owner-Password-Credential Flow' und damit verbundene Führung eines 'technischen Benutzers' im Benutzerverzeichnis soll abgelöst werden durch den 'Client-Credential Flow' und Führung eines 'Client' im IAM-System.
Die Konfiguration von Tasks bietet dazu die Möglichkeit, alternativ die Authentifizierungs-Credentials für 'Resource-Owner-Password-Credential' (Benutzername & Passwort) Flow bzw. 'Client-Credential Flow' (Client-ID & Client-Secret) zu setzen.
Abhängig davon, welche Art von Authentifizierungs-Credentials in der Konfiguration abgelegt sind, wird der dazu passende Credential Flow genommen.

[[kapitel-batchrahmen]]
=== Batchrahmen

Umstellung der Authentifizierung auf isy-security::
Für die Nutzung gesicherter Services muss ein gestarteter Batch sich selbst gegenüber dem IAM-Service authentifizieren.
...
Die bisherige Art der Authentifizierung über den 'Resource-Owner-Password-Credential Flow' und damit verbundene Führung eines 'technischen Benutzers' im Benutzerverzeichnis soll abgelöst werden durch den 'Client-Credential Flow' und Führung eines 'Client' im IAM-System.
Die Konfiguration von Batches bietet dazu die Möglichkeit, alternativ die Authentifizierungs-Credentials für 'Resource-Owner-Password-Credential' (Benutzername & Passwort) Flow bzw. 'Client-Credential Flow' (Client-ID & Client-Secret) zu setzen.
Abhängig davon, welche Art von Authentifizierungs-Credentials in der Konfiguration abgelegt sind, wird der dazu passende Credential Flow genommen.
