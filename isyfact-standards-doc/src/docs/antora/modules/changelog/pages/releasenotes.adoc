...
Der xref:isy-sicherheit:konzept/master.adoc[Baustein Sicherheit] und damit die Verwendung des Aufrufkontextes zur Authentifizierung gilt als deprecated und wird mit einem folgenden Release entfernt. Als Nachfolger zum Thema Sicherheit gibt es jetzt den Baustein Security.

[[kapitel-baustein-sicherheit-keykloak-deprecated]]
=== Baustein Sicherheit Keycloak deprecated icon:ban[title=Deprecation]
Der Baustein Sicherheit Keycloak mit dem Spring Keycloak Adapter gilt als deprecated und wird mit einem nachfolgenden Release entfernt.
...
Umstellung der Authentifizierung auf isy-security::
Für die Nutzung gesicherter Services muss die Task-Instanz autorisiert sein.
Für die Authentifizierung nutzt der Baustein Task Scheduling den xref:isy-security:konzept/master.adoc#absicherung-task[Baustein Security] und dessen xref:isy-security:konzept/master.adoc#aussensicht-der-komponente-security[Schnittstelle zur Authentifizierung] und präsentiert dem Service das so erhaltene Access-Token zur Autorisierung.
Die Konfiguration von Tasks, sowie die globale Konfiguration aller Tasks, bietet dazu die Möglichkeit, Authentifizierungs-Credentials (d.h. Benutzernamen & Passwort für Resource-Owner-Password-Credential Flow bzw. Client-ID & Client-Secret für Client-Credential Flow) sowie Behördenkennzeichen zu setzen.

[[kapitel-batchrahmen]]
=== Batchrahmen

Umstellung der Authentifizierung auf isy-security::
Für die Nutzung gesicherter Services muss die Batch-Instanz autorisiert sein.
Für die Authentifizierung nutzt der Batchrahmen den xref:isy-security:konzept/master.adoc#absicherung-batch[Baustein Security] und dessen xref:isy-security:konzept/master.adoc#aussensicht-der-komponente-security[Schnittstelle zur Authentifizierung] und präsentiert dem Service das so erhaltene Access-Token zur Autorisierung.
Die Konfiguration von Batches, sowie die globale Konfiguration aller Batches, bietet dazu die Möglichkeit, Authentifizierungs-Credentials (d.h. Benutzernamen & Passwort für Resource-Owner-Password-Credential Flow bzw. Client-ID & Client-Secret für Client-Credential Flow) sowie Behördenkennzeichen zu setzen.
