...
Analog wird der `NutzerAuthentifizierungInterceptor` entfernt und durch den `AuthenticateInterceptor` ersetzt.
...
Zusätzliche Beans für die Aktivierung der Annotation, wie beispielsweise die `AuthenticateInterceptor`-Bean, müssen nicht mehr definiert werden,
da deren Erstellung durch die `IsyOAuth2ClientAutoConfiguration` übernommen wird, solange die Gegebenheiten für die Erstellung von ClientRegistrations vorhanden sind.
