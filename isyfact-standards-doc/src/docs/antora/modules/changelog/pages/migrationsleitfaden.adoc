...
|- |`isy.security.oauth2.registration.<registration-id>.username` |Neu, ersetzt Authentifizierungsdaten die vorher individuell pro Anwendung konfiguriert wurden und benutzt wurden, um einen AufrufKontext zu authentifizieren.
|- |`isy.security.oauth2.registration.<registration-id>.password` |Neu, ersetzt Authentifizierungsdaten die vorher individuell pro Anwendung konfiguriert wurden und benutzt wurden, um einen AufrufKontext zu authentifizieren.
|- |`isy.security.oauth2.registration.<registration-id>.bhknz` |Neu, ersetzt Authentifizierungsdaten die vorher individuell pro Anwendung konfiguriert wurden und benutzt wurden, um einen AufrufKontext zu authentifizieren.
...
** Die Methode `getAuthentifizierungsmanager()` ermöglicht Zugang zum Interface `Authentifizierungsmanager`, über das eine Authentifizierung mit dem konfigurierten OpenID Connect Provider (bspw. Keycloak) durchgeführt werden kann.
...
** Die Methode `getBerechtigungsmanager()` ermöglicht Zugang zum Interface `Berechtigungsmanager`, über das auf die im Bearer Token enthaltenen Informationen zugegriffen werden kann.
...
// TODO: Abschnitt zur Nachfolge-Annotation von @NutzerAuthentifizierung (ISY-219)
