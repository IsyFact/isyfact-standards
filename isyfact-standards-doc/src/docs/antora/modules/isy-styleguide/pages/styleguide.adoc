...
Ein Styleguide ist eine Sammlung von Designrichtlinien und Standards, die zur Wahrung einer konsistenten visuellen Identität einer Marke oder eines Unternehmens beitragen und konzentriert sich auf visuelle Elemente wie Farben, Typografie, Grafiken und Logo-Verwendung.
...
Dieser Styleguide muss von der jeweiligen Behörde ausgearbeitet werden.

Während das Bedienkonzept in erster Linie für Designer und Entwickler relevant ist, die eine Anwendung entwerfen, ist ein Styleguide eher für Marken- und Kommunikationsexperten relevant, die sicherstellen möchten, dass die visuelle Identität und Markenpositionierung ihrer Behörde konsistent bleibt.
Die Entwickler setzen dann die vom Styleguide definierten visuelle Elemente um.
...
Hierfür kann zum Beispiel der Kennzeichner in einer Tabellenzeile genutzt werden (ähnlich wie beim Aufruf der Detailseite aus einer Tabelle heraus, siehe Kapitel Selektionsverhalten in einer Tabelle).
...
Dieses Kapitel zeigt einige oft nicht befolgte Regeln und häufige Verstöße gegen den Styleguide auf.
