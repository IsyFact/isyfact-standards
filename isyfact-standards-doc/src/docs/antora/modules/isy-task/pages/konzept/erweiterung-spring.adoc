...
NOTE: Mehr Informationen zu dem Thema finden sich in der xref:glossary:literaturextern:inhalt.adoc#litextern-spring-boot-task-scheduling[Dokumentation über Task Scheduling von Spring] sowie in der xref:glossary:literaturextern:inhalt.adoc#litextern-spring-task-scheduling-annotations[Dokumentation über die Annotationen zum Scheduling des Spring Frameworks].
