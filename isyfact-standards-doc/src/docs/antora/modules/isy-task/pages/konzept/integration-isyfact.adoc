...
Die IsyFact stellt einige Anforderung an die Ausführung fachlicher und technischer Operationen.
...
Der Baustein schreibt folgende Informationen ins Log:

* Start und Ende der Ausführung eines Tasks (Log-Level: `INFO`),
* Fehler in der Konfiguration von Tasks (Log-Level: `ERROR`).

.icon:university[title=Architekturregel] Logging für Tasks
****
Tasks müssen mindestens die folgenden Informationen ins Log schreiben:
...
Der Baustein Task Scheduling nutzt hierfür den Baustein Security und dessen xref:isy-security:konzept/master.adoc#aussensicht-der-komponente-security[Schnittstelle zur Authentifizierung].
...
NOTE: Mehr zu der Annotation `@Timed` findet sich in der xref:glossary:literaturextern:inhalt.adoc#litextern-micrometer-timed-annotation[Dokumentation von Micrometer].

[[konfiguration]]
== Konfiguration

Tasks müssen über eine xref:isy-konfiguration:konzept/master.adoc#table-typKonfZiel2[betriebliche Konfiguration] konfigurierbar sein.
