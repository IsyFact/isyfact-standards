...
Der Baustein Task Scheduling setzt direkt auf das Task Scheduling von Spring Boot auf, und erweitert dieses.
Außerdem nutzt er Spring AOP aus dem Spring Framework und die @AspectJ-Annotationen des Produkts AspectJ.
Wie der Baustein diese Produkte nutzt, beschreibt die Seite "xref:isy-task:konzept/erweiterung-spring-boot.adoc[]".

Des Weiteren nutzt er mehrere Bausteine der IsyFact, um ihre Funktion für Tasks verfügbar zu machen.
Dazu gehören die Bausteine xref:isy-logging:konzept/master.adoc[Logging], xref:isy-security:konzept/master.adoc[Security], xref:isy-ueberwachung:konzept/master.adoc[Überwachung] und xref:isy-konfiguration:konzept/master.adoc[Konfiguration].
