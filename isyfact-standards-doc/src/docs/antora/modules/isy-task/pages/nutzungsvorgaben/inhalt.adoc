...
Die Nutzung der Bibliothek wird im Dokument <<TODO anderes Dokument: NutzungsvorgabenSicherheit>> beschrieben.
...
Die Dauer muss die Form eines Zeitraums gemäß dem <<TODO anderes Dokument: KonzeptDatumZeit>> besitzen:
...
Der MBean-Exporter wird gemäß dem <<TODO anderes Dokument: KonzeptUeberwachung>> konfiguriert.
...
Werden hierbei die Besonderheiten der Java Multithreading API nicht berücksichtigt, kann dies zu einem fehlerhaften Verhalten in der xref:glossary:glossary:master.adoc#glossar-Geschaeftsanwendung[Geschäftsanwendung (Glossar)] führen.
