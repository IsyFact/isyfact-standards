...
Im folgenden Beispiel wird ein Task mit einer Verzögerung von jeweils drei Sekunden ausgeführt. Nach zehn Sekunden wird der Task durch die `cancel()`-Methode manuell gestoppt. Folglich wird der Task drei mal erfolgreich ausgeführt.
...
[source,properties]
----
isy.task.tasks.scheduledTasks-scheduleTaskWithFixedDelay.benutzer=TestUser1
isy.task.tasks.scheduledTasks-scheduleTaskWithFixedDelay.passwort=TestPasswort1
isy.task.tasks.scheduledTasks-scheduleTaskWithFixedDelay.bhkz=BHKZ1
isy.task.tasks.scheduledTasks-scheduleTaskWithFixedDelay.host=ComputerName
----

Zu beachten ist die Konvention zur Namensgebung für Tasks. Sie folgt immer dem Schema `"klassenName-geschedulterMethodenName"`.
...
[source, java]
----
taskConfig = isyTaskConfigurationProperties.getTasks()
    .computeIfAbsent(taskId, k -> new TaskConfig());

taskConfig.setDeaktiviert(true);
taskConfig.setBenutzer("testBenutzer");
taskConfig.setPasswort("123");
----
...
Für die Absicherung eines Tasks benötigt es lediglich eine OAuth 2.0 Client Registration ID, um die erforderlichen Sicherheitsmaßnahmen zu implementieren.
...
möglich. Dies ist der Typ des Rückgabewerts, wenn ein Task über `taskScheduler.schedule()` manuell gestartet wird. Mit dem Future kann exemplarisch geprüft werden, ob ein Task erledigt ist. Oder es kann geprüft werden, wie viel Zeit bis zum Beginn der Ausführung verbleibt:

[source,java]
----
myTask.isDone();
myTask.getDelay(TimeUnit.MINUTES);
----
