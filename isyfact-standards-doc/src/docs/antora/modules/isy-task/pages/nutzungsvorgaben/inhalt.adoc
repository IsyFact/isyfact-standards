...
[source,properties]
----
isy.task.tasks.halloWeltTask.registration-id=halloWelt-client-registration
...
Die ID der Client Registration aus `isy-security` zur Authentifizierung, wenn keine Task-spezifische `registration-id` konfiguriert wird:

[source,properties]
----
isy.task.default.registration-id
----
...
[source,properties]
----
isy.task.tasks.<Task>.registration-id
----
