...
Die <<TODO anderes Dokument: IsyFactReferenzarchitekturITSystem>> beschreibt drei Möglichkeiten des Zugriffs auf ein IT-System: Grafische Oberflächen (GUIs), Services und Batches.
...
Weitere Details zur Umsetzung der Autorisierung in der GUI sind in <<TODO anderes Dokument: DetailkonzeptKomponenteWebGUI>> beschrieben.
...
Weitere Details zur Umsetzung der Autorisierung bei Batches sind in <<TODO anderes Dokument: DetailkonzeptKomponenteBatch>> beschrieben.
...
Ab diesem Punkt kann der xref:glossary:glossary:master.adoc#glossar-Batch[Batch (Glossar)] jederzeit auf den Aufrufkontext zugreifen, um im Zuge des Batchlaufes Berechtigungsprüfungen vorzunehmen oder Services von Nachbarsystemen (vgl. <<aufrufen-von-nachbarsystemen>>) unter Bereitstellung des Aufrufkontextes aufzurufen.
...
Das Service-Gateway übergibt den Geschäftsanwendungen bei der Weiterleitung der Anfragen den Aufrufkontext als Transportobjekt an die jeweilige Service-Schnittstelle (vgl. <<aufrufen-von-nachbarsystemen>>).
...
Details zum Aufbau der Korrelations-ID sind in <<TODO anderes Dokument: NutzungsvorgabenLogging>> beschrieben.
...
Diese Methode kann in den Watchdog des IT-Systems (siehe <<TODO anderes Dokument: KonzeptUeberwachung>>) eingebunden werden.
