...
* Die Bean für die Komponente `SicherheitAdmin` wird konfiguriert, wenn ein IAM-System-Bean vorhanden ist.
...
Des Weiteren muss eine Bean für den verwendeten `IAM-System` existieren.
...
[IMPORTANT]
====
Die Implementierung der Schnittstelle `IAM-System` stellt einer der IsyFact-Bausteine für die Authentifizierung bereit.
