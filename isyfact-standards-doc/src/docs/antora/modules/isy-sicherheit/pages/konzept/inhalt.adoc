...
In der Regel erfolgt die Authentifizierung von xref:glossary:glossary:master.adoc#glossar-Anwender[Anwender (Glossar)] über eine zentrale Komponente der Systemlandschaft, in welche das IT-System eingebettet ist.
...
Wie genau Rollen und Rechte spezifiziert werden, beschreibt das Kapitel <<rollen-und-rechte>>.
...
<<image-rollen-beziehungen>> verdeutlicht den Inhalt der folgenden Abschnitte grafisch.
...
Anwender gelangen in der Regel entweder über das glossar-Querschnittskomponente xref:glossary:glossary:master.adoc#glossar-Portal[Portal (Glossar)] oder den xref:glossary:glossary:master.adoc#glossar-Service-Gateway[Service Gateway (Glossar)] in eine Anwendungslandschaft.
...
Wenn die Querschnitts-Rolle den Zugangsrollen untergeordnet ist, darf jeder Anwender mit Zugriff automatich auch auf den Querschnitt zugreifen.
...
Alle Parteien verfügen über unterschiedliches, sich ergänzendes Fachwissen, das essentiell für die Erstellung von Rollen ist.
