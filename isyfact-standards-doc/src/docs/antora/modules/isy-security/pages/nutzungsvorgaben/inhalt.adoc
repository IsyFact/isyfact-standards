...
NOTE: Es empfiehlt sich, die Beans direkt zu autowiren und das Interface hauptsächlich für eine Migration von isy-sicherheit zu verwenden.
...
[[absicherung_von_batches]]
== Absicherung von Batches

Für detaillierte Informationen und Anleitung zur Absicherung von Batches wird auf die Dokumentation unter xref:blaupausen:detailkonzept-komponente-batch/inhalt.adoc#authentifizierung-und-autorisierung[Authentifizierung und Autorisierung von Batches] verwiesen.

[[absicherung_von_tasks]]
== Absicherung von Tasks

Um detaillierte Informationen und Anleitung zur Absicherung von Tasks zu erhalten, wird auf die Nutzungsvorgaben unter xref:isy-task:nutzungsvorgaben/inhalt.adoc#absicherung-von-tasks[Absicherung von Tasks] verwiesen.
Diese bieten praxisorientierte Richtlinien zur Gewährleistung der Sicherheit von Tasks.
