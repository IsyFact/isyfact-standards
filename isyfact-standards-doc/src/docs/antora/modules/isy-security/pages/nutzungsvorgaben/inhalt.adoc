...
Zur manuellen authentifizierung von Clients stehen folgende Methoden zur Verfügung:

`void authentifiziere(String registrationId)`:: Authentifiziert einen Client anhand einer ClientRegistrationId. Unterstützt `client_credentials` und `password` GrantType.
Zugangsdaten für technische Nutzer müssen im Fall von `password` GrantType konfiguriert sein (siehe <<isyfact-specific-client-registration>> für _Resource-Owner-Password-Credential_ Flow).

`void authentifiziereClient(String clientId, String clientSecret, String issuerLocation)`:: Authentifiziert einen Client anhand `clientId`, `clientSecret` und `issuerLocation`.

`void authentifiziereSystem(String username, String password, String registrationId, @Nullable String bhknz)`:: Authentifiziert einen Client anhand `username`, `password`, `registrationId` und (optional) `bhknz`.
...
Der Client-Credential Flow ist bevorzugt zu nutzen.
****
...
[[table-isyfact-client-registratoin-extension]]
...
=== Manuelle Authentifizierung eines Clients mit dem Authentifizierungsmanager

Die manuelle Authentifizierung eines Client erfolgt über den Authentifizierungsmanager, der entsprechende Methoden zur Verwendung mit und ohne zuvor konfigurierte `ClientRegistrations` bereitstellt.

1. Authentifizierung per `registrationId`.
...
2. Authentifizierung per `clientId`, `clientSecret` und `issuerLocation`.
Diese Methode nutzt den _Client-Credentials_ Flow und kann verwendet werden wenn die zugangsdaten (`clientId` & `clientSecret`) erst zur Laufzeit bekannt sind.
3. Authentifizierung per `username`, `password`, `registrationId` und (optional) `bhknz`.
Diese Methode nutzt den _Resource-Owner-Password-Credentials_ Flow und setzt eine entsprechende `ClientRegistration` mit `authorization-grant-type=password` voraus.
Die im <<listing-client-registrations>> konfigurierten Isyfact spezifischen ClientRegistration properties werden nicht benötigt, da die Zugangsdaten dynamisch zur Laufzeit als Methodenparameter übergeben werden.

[[listing-client-registrations]]
.Beispiel: ClientRegistrations
[source,properties]
----
[...]
spring.security.oauth2.client.provider.testrealm.issuer-uri=http://localhost:9095/auth/realms/testrealm

### Beispiel: ClientRegistration für Client-Credentials Flow
...
### Beispiel: ClientRegistration für Resource-Owner-Password-Credentials Flow
# Spring Security Konfiguration
...
# Isyfact spezifische ClientRegistration properties für Resource-Owner-Password-Credentials Flow
...
isy.security.oauth2.client.registration.client2.bhknz=testbhknz
