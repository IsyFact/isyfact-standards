...
[[konfiguration-von-multi-iam-urls]]
=== Konfiguration von mehreren IAM-URLs
Zur Konfiguration von mehreren IAM-URLs ist es erforderlich, die Konfigurationsproperties `issuer-uri` und `jwk-set-uri` entsprechend zu definieren und mit besonderem Property-Pfad zu gruppieren.
...
Die nachstehende Konfigurationstabelle zeigt eine Liste von Property-Pfad-Beispielen, um mehrere IAM-URLs zu definieren.
...
.Konfigurationsbeispiele für Multi IAM-URLs
[cols="3,7",options="header"]
|===
|Parameter |Beschreibung
| tenants.*portal*.issuer-uri
tenants.*portal*.jwk-set-uri| Property-Set zur Definition einer *externen* und *internen* IAM-URL für den Mandant *Portal*
| tenants.*sgw-1*.issuer-uri
tenants.*sgw-1*.jwk-set-uri| Property-Set zur Definition einer *externen* und *internen* IAM-URL für den Mandant *ServiceGateway-1*
| tenants.*sgw-2*.issuer-uri
tenants.*sgw-2*.jwk-set-uri| Property-Set zur Definition zweier *interner* IAM-URLs für den Mandant *ServiceGateway-2*
| tenants.*batch*.issuer-uri
tenants.*batch*.jwk-set-uri| Property-Set zur Definition zweier *interner* IAM-URLs für den Mandant *Batch*
| tenants.*ga-A-1*.issuer-uri
tenants.*ga-A-1*.jwk-set-uri| Erstes Property-Set zur Definition einer *externen* und *internen* IAM-URL für den Mandant *Geschäftsanwendung-A*
| tenants.*ga-A-2*.issuer-uri
tenants.*ga-A-2*.jwk-set-uri| Zweites Property-Set zur Definition zweier *interner* IAM-URLs für den Mandant *Geschäftsanwendung-A*
| tenants.*ga-A-3*.issuer-uri
tenants.*ga-A-3*.jwk-set-uri| Drittes Property-Set zur weiteren Definition zweier *interner* IAM-URLs für den Mandant *Geschäftsanwendung-A*
|===
...
Sobald also beide Properties für einen Mandant gesetzt sind, dann wird der Wert von `issuer-uri` für den Aufruf auf den Authentifizierungsserver des IAM verwendet, insbesonderen für *externe* Aufrufe von ausserhalb der Anwendungslandschaft. Der Wert von  `jwk-set-uri` wird dann nur bei der *internen* Token-Validierung herangezogen.

Gegebenenfalls erweitert KeyCloak intern bei der Token-Validierung die  `jwk-set-uri` um den Standard-Pfad von KeyCloak `protocol/openid-connect/certs` um das notwendige Zertifikat zu ermitteln.
...
Im Falle einer nicht vorhanden`spring.security.oauth2.resourceserver.jwten Datei verbleibt die Liste der Rollen und Rechte leer, d. h. eine Absicherung der Anwendung findet nicht statt.
