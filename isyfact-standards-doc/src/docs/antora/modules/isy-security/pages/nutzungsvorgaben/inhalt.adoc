...
Das Property `issuer-uri` wird im Normalfall mit der URI des _Authentifizierungsservers_ des IAM initialisiert.
Diese URI wird einerseits zum Aufruf von Requests an das IAM verwendet, aber andererseits auch vom Authentifizierungserver bei der Token-Generierung als Validierungskriterium gegengeprüft und letztendlich als Inhalt in das `iss-Claim` des Tokens eingepackt, so dass bei der späteren Token-Validierung diese URI für die ggf. notwendige Ermittlung bzw. Nachladung der benötigten Zertifikate verwendet werden kann.

Wird jetzt allerdings, wie beispielsweise bei einer Authentifizierung über das _Portal_, in der `issuer-uri` eine *externe* URI benötigt, damit eine _externe Anwendung_ den _IAM_ überhaupt von ausserhalb der Anwendungslandschaft erreichen kann, dann würde diese *externe* URI auch bei der späteren Token-Validierung herangezogen.
...
Sobald also beide Properties für einen Mandant gesetzt sind, dann wird der Wert von `issuer-uri` für den Aufruf gegen den Authentifizierungsserver des IAM verwendet, insbesonderen für *externe* Aufrufe von ausserhalb der Anwendungslandschaft.
Der Wert von  `jwk-set-uri` wird nach der Authentifizierung für die darauf folgende *interne* Token-Validierung in der *internen* Geschäftsanwendung herangezogen, damit ggf. das Zertifikat vom IAM nachgeladen werden kann, um die Token-Signatur gegen das IAM-Zertifikat zu validieren.
