...
* `authentifiziereClient(String clientId, String clientSecret, String issuerLocation)`
* bzw. `authentifiziereSystem(String registrationId, String username, String password, @Nullable String bhknz)`

. Die beiden Methoden erlauben es technische Nutzer zur Laufzeit mit ihren Zugangsdaten über den IAM-Service zu authentifizieren.
