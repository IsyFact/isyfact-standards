...
Für detaillierte Informationen und Anleitung zur Absicherung von Batches wird auf die Dokumentation unter xref:blaupausen:detailkonzept-komponente-batch/inhalt.adoc#authentifizierung-und-autorisierung verwiesen.

[[absicherung_von_tasks]]
== Absicherung von Tasks

Um detaillierte Informationen und Anleitung zur Absicherung von Tasks zu erhalten, wird auf die Nutzungsvorgaben unter xref:isy-task:nutzungsvorgaben/inhalt.adoc#absicherung-von-tasks verwiesen.
