...
Die Authentifizierung & Autorisierung wird über Multi-Tenancy strukturiert, um die unterschiedlichen Zugriffswege gemäß den  xref:isyfact-standards-doku:referenzarchitektur:software-technisch.adoc#prinzipien-der-sicherheitsarchitektur[Prinzipien der Sicherheitsarchitektur] darüber abzubilden, wie das nachfolgende Bild zeigt:
...
Diese Trennung und Identifizierung wird - sofern gewünscht - durch den Sicherheitsbaustein _isy-security_ unterstützt.
...
`<property-key-pfad>.tenants.*<tenant-id>*`
...
- `<property-key-pfad>.tenants.*portal*`
- `<property-key-pfad>.tenants.*sgw*`
- `<property-key-pfad>.tenants.*agw*`
...
- `<property-key-pfad>.tenants.*sgw-1*`
- `<property-key-pfad>.tenants.*sgw-2*`
