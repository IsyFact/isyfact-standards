...
Dadurch ist es möglich fachliche Unterschiede über den gleichen technischen Kanal bei der Authentifizierung zu machen.

Das nachfolgende Kapitel  erläutert diese Mandanten spezifische Trennung.
...
Das Property `issuer-uri` wird im Normalfall mit der URL des Authentifizierungsservers des IAM initialisiert.
Diese URL wird bei der Token-Generierung als Inhalt in das Token gepackt, so dass bei der späteren Token-Validierung diese URL für die ggf. notwendige Ermittlung der benötigten Zertifikate verwendet werden kann.

Wird jetzt allerdings, wie beim Portal-Gateway für die `issuer-uri` eine URL benötigt, die von einer _externen Anwendung_ erreichbar ist, dann wird diese URL mit dem Token zusammen bei der Token-Validierung wiederverwendet.
Aber dann kann bei der späteren Token-Validierung in der  _internen ServiceBean_ der Authentifizierungsserver des IAM nicht über diese _externe URL_ erreicht werden.

*Property `jwk-set-uri`*

Um diesen Konflikt zu lösen wird das zweite Property `jwk-set-uri` verwendet.
Sobald also beide Properties für einen Mandant gesetzt sind, dann wird der Wert von `jwk-set-uri` für den Zugriff auf den Authentifizierungsserver der IAM verwendet und der Wert von `issuer-uri` wird nur bei der Token-Validierung herangezogen.
Gegebenenfalls erweitert Spring intern bei der Token-Validierung die `issuer-uri` um den Standard-Pfad `protocol/openid-connect/certs` um das notwendige Zertifikat zu ermitteln.
