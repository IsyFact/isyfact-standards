...
Wenn in einer Anwendungslandschaft das IAM über mehrere unterschiedliche URLs aufgerufen werden kann, dann benötigt eine Anwendung mindestens eine bis alle dieser URIs, um eine Authentifizierung mit dem IAM durchzuführen.

Dies ist zum Beispiel der Fall, wenn eine interne URI des IAM für Aufrufe von innerhalb der Anwendungslandschaft und eine externe URI für Aufrufe von außerhalb der Anwendungslandschaft benötigt werden oder wenn aufgrund verschiedener Netzwerk-Zonen bzw. technischer Kommunikationskanäle die URLs sich unterscheiden müssen. Die nachfolgende Skizze zeigt dies schematisch auf.
...
Die verwendete URL (Issuer-URL) zum Aufruf des IAM wird dort nach valider Authentifizierung im ausgestellten Access-Token hinterlegt und mit der in der Geschäftsanwendung konfigurierten URL im Rahmen der Tokenprüfung abgeglichen.
...
Um nun in der Anwendung Access-Tokens erfolgreich zu validieren, die über eine in der Anwendung abweichend konfigurierte IAM-URL ausgestellt wurden, müssen alle genutzten URLs des IAM-Systems in der Anwendung hinterlegt werden. Dies geschieht über die Anwendungskonfiguration. Auf diese Weise können beliebig viele URLs konfiguriert werden.

Weitere Details zur Konfiguration mehrerer URLs eines IAM wird in den xref:nutzungsvorgaben/inhalt.adoc#konfiguration-von-multi-iam-urls[Nutzungsvorgaben - Konfiguration von mehreren IAM-URLs] beschrieben.
