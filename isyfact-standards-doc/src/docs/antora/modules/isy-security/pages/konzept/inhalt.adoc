...
Also Voraussetzung für diesen Flow ist die Bereitstellung eines Vermittlers ('Relying Party') notwendig, das die Kommunikation zwischen Web-Oberfläche und Autorisierungsserver verwaltet.
Dieser Vermittler muss ein OAuth2.0 _Confidential Client_ sein, also ein serverseitiges System.
Dies kann ein als ReverseProxy konfiguriertert Webserver (z.B. Apache http-Server mit `mod_auth_openidc` Modul) sein, der zwischen  Web-Client (Web-Browser) und Web-Anwendung geschaltet ist.
...
Nach erfolgtem Login wird die Autorisierung an die _User-Session_ gebunden und bei jeder Interaktion mit der Anwendung an die Web-Anwendung weitergeleitet - bis die Session beendet wird.
