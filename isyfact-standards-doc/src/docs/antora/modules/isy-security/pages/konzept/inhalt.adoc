...
Sobald also beide Properties für einen Mandant gesetzt sind, dann wird der Wert von `jwk-set-uri` für den Zugriff auf den Authentifizierungsserver der IAM verwendet und der Wert von `issuer-uri` wird nur bei der Token-Validierung zur Pürfung des `iss`-Claims herangezogen.
