...
Für den Aufruf von Ressourcen einer externen Anwendung ist eine Authentifizierung notwendig.
 
Die nachfolgende Abbildung zeigt die aktuell verwendete System-Architektur, in der das Service-Gateway mithilfe des IAM-Service diese Authentifizierung durchführt.
...
In `isy-task` wird die übergebene `TaskID` verwendet, um in der Betriebskonfiguration gemäß den xref:isy-task:nutzungsvorgaben/master.adoc#konfigurationsschluessel[Task-Konfigurations-Konventionen] nach den entsprechenden Konfigurationswerten aufzulösen.
