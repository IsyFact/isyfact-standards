...
Liegt der Batch - wie in <<image-AwLogBat>> dargestellt - in der Nutzungsschicht und damit außerhalb des Anwendungskerns, dann muss der Authentifizierungsaufruf zum IAM - wie in Kapitel <<aufruf-service-gateway>> beschrieben - über das SergiveGateway geroutet werden.
...
Das bedeutet der _isy-batchrahmen_ verfügt mit _IsyFact-3_ innerhalb der Satz-Verarbeitungsschleife über einen Validierungsmechanismus, der bei jedem Durchlauf einer Datensatz-Verarbeitung vor den eigentlichen Aufrufen der Fachanwendungskomponenten die Gültigkeitsdauer des _AccessTokens_ überprüft.
