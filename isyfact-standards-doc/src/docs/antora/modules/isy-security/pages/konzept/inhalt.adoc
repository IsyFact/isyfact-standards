...
Das nachfolgende Kapitel erläutert die Mandanten spezifische Trennung der Authentifizierungswege.
...
Das bedeutet, dass für jeden Mandanten unterschiedliche Konfigurationen möglich sind.
Ein _Portal_-Gateway bedarf beispielsweise einer anderen oder gar erweiterten Konfiguration gegenüber einem _Service_-Gateway.
Das _Authentication_-Gateway wiederum erfüllt neuere und erweiterte Sicherheitsanforderungen der Authentifizierung als die beiden anderen Kanäle bisher.
Wiederum eine weitere Instanz eines Authentication-Gateways bedarf einer fachlichen Trennung gegenüber dem bisherigen Authentication-Gateway und wird mit einer weiteren IAM-Instanz verbunden.

Dies beruht auf der Art und Weise, wie die Authentifzierung mit `isy-security`, dem ausgestellten Access-Token bei valider Nutzer-Authentifizierung und der nachfolgenden Token-Validierung während des fachlichen Prozesses stattfindet.

So bedarf beispielsweise die Authentifizierung einer externen Webclient-Anwendung über das _Portal_-Gateway eine Konfiguration des IAM-Service, die berücksichtigt, dass der Aufruf des IAM-Service hier eine Unterscheidung zwischen _internem_ und _externem_ Aufruf benötigt.
Um dies zu ermöglichen bietet die _Spring-Konfiguration_ folgende beiden Properties:

- `spring.security.oauth2.resourceserver.jwt.tenants.<realm>.issuer-uri`
- `spring.security.oauth2.resourceserver.jwt.tenants.<realm>.jwk-set-uri`

*Mandanten-Bezug (realm)*

Um nun für jeden Mandanten eine eindeutige Zuordnung der Properties herzustellen, enthält der Property-Key den Pfad
`spring.security.oauth2.resourceserver.jwt.tenants.<realm>`

Der Platzhalter `<realm>` wird in der konkreten Konfigurationsausprägung auf den Wert bzw. Namen des Mandanten gesetzt.
