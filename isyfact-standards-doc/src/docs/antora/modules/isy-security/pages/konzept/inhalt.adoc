...
Für menschliche Nutzer wurde mit dem Refresh Token ein Mechanismus eingeführt, der eine automatische Aktualisierung des Access Tokens erlaubt, ohne dass der Benutzer etwas davon merkt.
Das Webserver-Modul `mod_auth_openidc` koordiniert die Erneuerung des Access-Tokens (automatisch mit Refresh-Token bzw. durch erneute Authentifizierung (siehe: <<aufruf-weboberflaeche-portal>>)) - der Baustein Security wird hierfür nicht verwendet. +
Technische Nutzer können ihr Access-Token nur durch eine erneute Authentifizierung (siehe: <<aufruf-service-gateway>> bzw. <<ausfuehrung-batch-u-task>>) erneuern.

Die Verwendung eines abgelaufenen Access-Tokens bei einer Anfrage wird vom empfangenden System erkannt und mit HTTP status error code: 401 und der Fehlerbeschreibung: "The access token expired" beantwortet, wie in (<<validierung-der-token>>) beschrieben.
Als Reaktion darauf fordert der Baustein `isy-security` bzw. das `Spring Security Framework` (bzw. das Webserver-Modul `mod_auth_openidc`) beim IAM-Service ein neues Access-Token an, legt es ab und sendet die Anfrage erneut mit dem neuen Access-Token.
Zur Minimierung dieser Fehlversuche prüft der Baustein `isy-security` bzw. das `Spring Security Framework` (bzw. das Webserver-Modul `mod_auth_openidc`) vor jeder Anfrage die verbleibende Lebensdauer des Access Tokens und fordert proaktiv ein neues Access-Token an, wenn es zu knapp würde.
