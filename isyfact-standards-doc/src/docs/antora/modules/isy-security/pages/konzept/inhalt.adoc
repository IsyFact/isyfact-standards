...
Der _Client-Credential-Flow_ dient dabei der Authentifizierung eines Clients und über den _Resource-Owner-Password-Credentials-Flow_ erfolgt die Authentifizierung eines technischen Nutzers.
...
* Dabei muss der verwendete technische Nutzer für diese externe Anwendung noch nicht gemäß _SpringSecurity (OAuth2.0)_ auf einen _OAuth2.0 Client_ (mit Client-ID u. Client-Secret) migriert worden sein, 
...
. Für die Authentifizierung einer externen Anwendung existiert ein eigener Servicepunkt, das _Authentication-Gateway_. An diesen Servicepunkt werden von der extenen Anwendung die Authentifizierungsdaten übergeben.
...
Die neue System-Architektur hat den Vorteil, dass sie das Service-Gateway von der Aufgabe befreit, neben dem Aufruf einer anderen Geschäftsanwendung auch einen entsprechenden Authentifizierungs-Prozess mit durchzuführen.
