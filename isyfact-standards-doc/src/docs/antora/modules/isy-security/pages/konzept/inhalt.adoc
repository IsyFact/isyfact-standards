...
Die bisher aufgezeigten Authentifizierungswege über _Portal_-, _Service_- und _Authentication_-Gateway werden in Bezug auf `isy-security` als jeweilig unterschiedliche Manadanten behandelt bzw. konfiguriert.

Das bedeutet, dass für jeden Mandanten sprich Authentifizierungsweg unterschiedliche Konfigurationen möglich bzw. notwendig sind.
...
Um dies zu ermöglichen bietet die _Spring-Konfiguration_ unter dem Pfad `spring.security.oauth2.resourceserver.jwt` die beiden Properties `issuer-uri` und `jwk-set-uri`.
...
Wird jetzt allerdings, wie beim _Portal_-Gateway für die `issuer-uri` eine URL benötigt, die von einer _externen Anwendung_ erreichbar ist, dann wird diese URL mit dem Token zusammen bei der Token-Validierung wiederverwendet.
Nur dann kann bei der späteren Token-Validierung in der  _internen ServiceBean_ der Authentifizierungsserver des IAM nicht über diese _externe URL_ erreicht werden.

Um diesen Konflikt zu lösen wird das zweite Property `jwk-set-uri` verwendet.
Sobald also beide Properties für einen Mandant gesetzt sind, dann wird der Wert von `jwk-set-uri` für den Zugriff auf den Authentifizierungsserver des IAM verwendet und der Wert von `issuer-uri` wird nur bei der Token-Validierung herangezogen.
