...
Der Baustein isy-security basiert auf Spring Security und bietet querschnittliche Funktionalität zur Authentifizierung und Autorisierung (Berechtigungsprüfung) von Anfragen.
Der Zugriff auf Spring Security findet über eine weitere gekapselte Schicht, die die bestehende Funktionalität um spezifische IsyFact Funktionalität erweitert, direkt über die Annotation @secured statt.
Außerdem wird sichergestellt, dass die Authentifizierung und Autorisierung über einheitliche Schnittstellen verlaufen, um die Migration von einer älteren IsyFact Version zu erleichtern und zu vereinfachen.
Die Authentifizierung und Autorisierung an der Benutzeroberfläche findet im Zuge der Einführung von SPAs nicht mehr über das Backend oder dem Baustein isy-security statt.

Hierzu stellt der Baustein über das Interface `Security` mehrere Manager bereit:

* `Authentifizierungsmanager`
* `Berechtigungsmanager`

Das Interface `Security` hat Zugriff auf eine Liste von allen im System hinterlegten Rollen.
Batches und Tasks greifen darüber auf den `Authentifizierungsmanager` und den `Berechtigungsmanager` zu.

Der `Authentifizierungsmanager` ist für die Kommunikation mit dem IAM-Service über den `OAuth2AuthorizedClientProvider` zuständig. Die Authentifizierung (Anmeldung) eines menschlichen Benutzers verläuft über diesen Aufruf. Nach einer erfolgreichen Authentifizierung erfolgt der Erhalt des Access Tokens und der dem Benutzer zugeordneten Rollen. Diese Informationen werden anschließend im `SecurityContext` abgelegt.
...
Mit der Annotation `@Nutzerauthentifizierung` werden Services abgesichert die von technischen Nutzern aufgerufen werden.
Der `NutzerAuthentifizierunginterceptor` ist für die Authentifizierung von technischen (nicht menschlichen) Nutzern zuständig.
Die Authentifizierungsdaten werden aus den Konfigurationen der Anwendung ausgelesen und zur Authentifizierung bereitgestellt.

Spring Security bietet eine eigene Annotation, `@secured`, mit der angebotene Services der Anwendung abgesichert werden.
...
Parallel zur bestehenden Annotation `@NutzerAuthentifikation` wird die neue Annotation `@ClientAuthentification` in Betrieb genommen.
Mit dieser Annotation wird im Zusammenspiel mit dem `SecurityContext` eine spontane Authentifizierung ermöglicht.
Dadurch wird Anwendungen ohne die Migration auf IsyFact 3 die Möglichkeit geboten mit IsyFact 3 zu laufen, da sie noch die alten Annotationen verwenden können.
Der `ClientAuthenticationInterceptor` ist eine Spezialisierung des `SecurityContext` und wird für eine interne Authentifizierung am IAM-Service mit den von der IsyFact 3 eingeführten Lösungen verwendet.
Das Ergebnis einer Authentifizierung über die Annotation `@ClientAuthentification` ist ein voll initialisierter `SecurityContext` mit einem validen AccessToken.
Die Annotation muss zusammen mit der von IsyFact 3 eingeführten `@secured` Annotation verwendet werden.

Der Baustein Security bietet folgende Funktionen:

* Für Service-Aufrufe werden Interceptoren von Spring Security angeboten, welche über Spring AOP eine deklarative Berechtigungsprüfung ermöglichen.
* Für den Kontext der Anfrage stellt der Baustein einen Berechtigungsmanager zur Verfügung, der die Rechte und Rollen des anfragenden Anwenders kennt.
Die Informationen zum anfragenden Anwender werden – falls vorhanden – aus dem `SecurityContext` entnommen.
Die Fachkomponenten (Anwendungskomponente) eines IT-Systems nutzen den Berechtigungsmanager für spezielle Berechtigungsprüfungen, die nicht deklarativ über Annotationen erfolgen können.
* Anwender können anhand der übergebenen Benutzerkennung (und Passwort) authentifiziert werden.
Dazu wird der IAM-Service bzw. die lokale Authentifizierung angesprochen.
Die gewonnenen Informationen werden im `SecurityContext` (Access Token) hinterlegt.
* Das Interface `Authentifizierungsmanager` ist eine Erweiterung des `OAuth2AuthorizedClientProvider` da es das BHKNZ mitberücksichtigt und an dem IAM-Service überträgt. Dieser `OAuth2AuthorizedClientProvider` bildet anschließend die Verbindung zum IAM-Service (Authentifizierungssystem in obiger Abbildung).
