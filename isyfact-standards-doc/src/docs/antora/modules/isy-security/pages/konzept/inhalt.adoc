...
Mit der Methode `authentifiziereClient` wird die OAuth2.0-konforme Authentifizierung eines Clients mittels Client-ID u. Client-Secret unterstützt und mit der Methode `authentifiziereSystem` der alternative Weg mittels (techn.) Nutzername und Passwort.
...
Access-Token besitzen eine zeitlich begrenzte Gültigkeit, die im Ermessen der Anwendungslandschaft definiert werden, kann durch die begrenzte zeitliche Gültigkeit ist sichergestellt, dass das Access-Token kein 'ewiger Universalschlüssel' ist, sondern regelmäßig erneuert werden muss.
