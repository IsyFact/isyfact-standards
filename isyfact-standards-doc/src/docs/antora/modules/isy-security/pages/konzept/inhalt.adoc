...
Das Interface `Security` ist der zentrale Einstiegspunkt in den Baustein Security.
...
Außerdem kann die Fachanwendung über die Methode `getAlleRollen` eine eine Liste von allen im System hinterlegten Rollen erhalten.

Der `Authentifizierungsmanager` übernimmt die Authentifizierung von technischen Nutzern am IAM-System.
Mit der Methode `authentifiziereClient` wird die OAuth-konforme Authentifizierung eines Client mittels Id u. secret uunterstützt mit der Methode `authentifiziereSystem` der (veraltete) Weg mittels (techn.) Nutzername u. Passwort.
Bei erfolgreicher Authentifizierung werden die vom IAM-System zurückgegebenen Daten (Access-Token, Rollen des Anwenders, ...) im Spring Security Context abgelegt, zusammen mit den aus den Rollen resultierenden Rechten des Anwenders.
