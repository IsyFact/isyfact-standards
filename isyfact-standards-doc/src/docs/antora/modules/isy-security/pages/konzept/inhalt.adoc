...
* Die feingranulare Autorisierung geschieht in der Nutzungsschicht und im Anwendungskern der IT-Systeme über den IsyFact-Baustein Security.
...
Die Integration von Spring Security und des Bausteins Security wird im xref:blaupausen:detailkonzept-komponente-web-gui/master.adoc[Detailkonzept Komponente WebGUI]) beschrieben.

[[aussensicht-der-komponente-security]]
=== Schnittstelle des Bausteins Security

Im Folgenden wird die Schnittstelle des Bausteins Security beschrieben.

.Schnittstelle des Bausteins Security
[id="image-schnittstelle-security",reftext="{figure-caption} {counter:figures}"]
image::konzept/security-schnittstellen_mit-Benutzermanager.dn.svg[align="center"]

Bei Instanziierung des Bausteins 'Security' wird die anwendungsspezifische Rollen-Rechte-Datei eingelesen, sowie der `Authentifizierungsmanager`, der `Berechtigungsmanager` und  der `Benutzermanager` damit erzeugt.
Die Manager sowie eine Liste aller für diese Anwendung spezifischen Rollen werden nach Außen angeboten.

Der `Authentifizierungsmanager` übernimmt die Authentifizierung am IAM-System.
Untersützt wird sowohl die OAuth-konforme Authentifizierung eines Client mittels Id u. secret als auch die (veraltete) Methode mittels (techn.) Nutzername u. Passwort.
Bei erfolgreicher Authentifizierung werden die vom IAM-System zurückgegebenen Daten (Access-Token, Daten u. Rollen des Anwenders, ...) im Spring Security Context abgelegt, zusammen mit den aus den Rollen resultierenden Rechten des Anwenders.
Eine nicht erfolgreiche Authentifizierung (fachliche Ablehnung oder technisches Problem) löst eine entsprechende Exception aus.
...
Eine Liste der dem Anwender zugewiesenen Rollen bzw. daran geknüpften Rechte können hier erfragt werden und es kann geprüft werden, ob der Anwender ein bestimmtes Recht hat.

Im `Benutzermanager` finden sich die persönlichen Daten des Benutzers aus dem IAM-System:

- das Behördenkennzeichen des Benutzers,
- die interne, unveränderliche Kennung des Benutzers,
- die Kennung des Benutzers,
- das Passwort des Benutzers und
- der Name des Sachbearbeiters (Endnutzer) (Vorname + Nachname)
...
Die einzelnen Schnittstellen werden durch Rechte abgesichert (siehe Dokument xref:nutzungsvorgaben/master.adoc#einleitung[Security - Nutzungsvorgaben] ).
...
Die einzelnen angebotenen Services werden über Rechte abgesichert (siehe Dokument xref:nutzungsvorgaben/master.adoc#einleitung[Security - Nutzungsvorgaben]).
...
. Konfiguration des IT-Systems mit den erstellten technischen Rollen und Rechten (siehe Dokument xref:nutzungsvorgaben/master.adoc#einleitung[Security - Nutzungsvorgaben]).
