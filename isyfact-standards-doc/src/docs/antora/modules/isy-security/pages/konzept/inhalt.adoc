...
Das heißt in der Batch-Konfiguration ist ein Property konfiguriert, das dahingehend aufgelöst wird, ob der Batch nun mit seinem _TechnischenNutzer_ und den zugehörigen Authentifizierungsdaten `Benutzer` und `Passwort` über den bisherigen <<resource-owner-password-credential-flow>> oder über den neuen <<client-credential-flow>> mit dem migrierten _Client_-Nutzer mit `Client-ID` und `Client-Secret` authentifiziert und autorisiert wird.
