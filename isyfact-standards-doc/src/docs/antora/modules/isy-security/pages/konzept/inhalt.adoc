...
Abgesehen von dem Sonderfall, dass ein Access Token kein Ablaufdatum hat - also ewig gültig ist, läuft ein Token irgendwann ab und muss erneuert werden.
...
Solange Nutzer die Anwendung innerhalb der Ablaufzeit des Refresh Token verwendet, bleibt er angemeldet und autorisiert.
Die Ablaufzeit des Refresh Tokens bestimmt hier die maximale Zeitspanne zwischen zwei Requests an die Anwendung, ohne dass eine erneute Authentifizierung des Benutzers notwendig wird.

* *fixed*: Das neue Refresh Token hat dasselbe Enddatum, wie das Alte. +
Der Nutzer muss sich in regelmäßigen Abständen neu authentifizieren (bestimmt durch die Ablaufzeit des Refresh Token).
Mit der Authentifizierung und der Ausstellung des ersten Refresh Tokens läuft die Zeit bis die nächste Authentifizierung fällig wird.
Diese Variante kommt bei Anwendungen zum Einsatz, die nur selten verwendet werden, der Benutzer sich aber nicht jedes Mal neu anmelden soll.
...
Die Validierung des Tokens scheitert deshalb und der Aufruf wird mit dem HTTP-Code 401 (und Fehlertext 'invalid_token') beantwortet.
...
Jetzt sollte in jedem Fall ein Access-Token mit ausreichender Lebenszeit vorliegen, das für den Service-Aufruf verwendet werden kann. +
Für Web-Anwendungen, die über einen Portalserver mit dem von OpenID zertifizierten Authentifizierungs- und Authorisierungsmodul _mod_auth_openidc_ (version 2.3.10 oder höher) laufen, muss dafür nur der Konfigurationsparameter _OIDCRefreshAccessTokenBeforeExpiry_ gesetzt werden.
...
Ein Request-Interceptor stellt sicher, dass ein Access-Token mit ausreichender Lebenszeit verwendet wird und falls trotzdem das Access Token als ungültig abgelehnt wird, wird das in der Antwort erkannt und der Aufruf mit einem Neuen wiederholt.
