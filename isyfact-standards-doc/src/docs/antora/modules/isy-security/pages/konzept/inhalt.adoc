...
Gemäß der IsyFact-Systemlandschaft - siehe xref:blaupausen:detailkonzept-komponente-batch/master.adoc#grobe-architektur-des-batchrahmens[Grobe Architektur des Batchrahmens] - ist ein Batch teil eines Anwendungssystems und setzt sich - siehe xref:image-AwLogBat[] - aus den Komponenten (1) Batch-Logik und -Steuerung sowie den dabei eingesetzten (2) Fachkomponenten zusammen.
...
Die Abbildung in xref:image-sequenzdiagramm-absicherung-batch[] stellt den logischen Ablauf zur Absicherung eines Batch skizzenhaft dar.
...
Liegt der Batch - wie in xref:image-AwLogBat[] dargestellt - in der Nutzungsschicht und damit außerhalb des Anwendungskerns, dann muss der Authentifizierungsaufruf zum IAM - wie in Kapitel xref:aufruf-service-gateway[] beschrieben - über das SergiveGateway geroutet werden.
...
In xref:image-sequenzdiagramm-absicherung-task[] wird der logische Ablauf zur Absicherung einer _Aufgabe_ mit dem neuen Security-Baustein `isy-security` im Zusammenspiel mit der Bibliothek `isy-task` skizziert.
