...
Genau hier erfolgt frühzeitig die Unterscheidung welcher _Credential_-Flow (xref:resource-owner-password-credentials-flow[] oder xref:client-credential-flow[]) zur Authentifizierung und Autorisierung des Batch verwendet werden soll.
...
Über die Batch-Konfiguration wird gesteuert, ob der Batch mit einem _Technischen Nutzer_ und den zugehörigen Authentifizierungsdaten `Benutzer` und `Passwort` über den xref:resource-owner-password-credentials-flow[] authentifiziert und autorisiert wird oder mit einem _Client_ und den zugehörigen Authentifizierungsdaten `Client-ID` und `Client-Secret` über den xref:client-credential-flow[].
...
Mit Einführung des Bausteins `isy-security` werden auch für die Absicherung von _Aufgaben_ beide _Credential_-Flows xref:client-credential-flow[] und xref:resource-owner-password-credentials-flow[] unterstützt.
...
Zur Unterstützung einer weichen Migration wird mit `IsyFact-3` der xref:resource-owner-password-credentials-flow[] mit `Benutzer` und `Passwort` weiterhin möglich sein.
...
[id="image-resourceowner-password-credentials-flow",reftext="{figure-caption} {counter:figures}"]
...
[id="image-clientcredentials-flow",reftext="{figure-caption} {counter:figures}"]
