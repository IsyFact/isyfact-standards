...
bisherige Authentifizierungslogik - _ohne Authentication-Gateway_ - beibehalten.
...
Dabei werden die Aufrufe mit Authentifizierungsdaten von Aufrufen mit Nutzungsdaten getrennt. Die
Authentifizierungsaufrufe werden über das neue _Authentication-Gateways_ geleitet, wohingegen die Aufrufe mit den
Nutzdaten über das _ServiceGateways (SGW)_ geleitet werden.
...
_IdentityAccessManagement-Service (IAM)_ authentifiziert. Mit der Einführung des _Authentication-Gateways_ werden somit
alle Anfragen mit Authentifizierungsdaten über einen separaten Kommunikationskanal geleitet. Damit kommt keine mit der
Authentifizierung involvierte Komponente mit späteren Anwendungsanfragen in Kontakt und umgekehrt.

Die externe Anwendung sendet ihre Authentifizierungsdaten an das _Authentication-Gateway_. Dort erfolgt eine Filterung
bzw. Validierung auf korrekt übergebene Credentials. Gemäß den Filterregeln im _Authentication-Gateway_ wird dann der Authentifizierungsrequest mit validen User-Credentials an den
IAM-Service weitergeleitet.

Im IAM-Service erfolgt dann mit den übergebenen User-Credentials die eigentliche Authentifizierung. Dabei wird neben der
neuen Authentifizierungslogik des _Client-Credential-Flows_ auch die bisherige Authentifizierungslogik des
_Resource-Owner-Password-Credentials-Flows_ unterstützt. Der _Client-Credential-Flow_ dient dabei der
Authentifizierung eines _Technischen Nutzer_ und über den _Resource-Owner-Password-Credentials-Flow_ erfolgt die
Authentifizierung eines _Natürlichen Nutzers_. Mit validen User-Credentials und erfolgreicher Authentifizierung liefert
der IAM-Service dann ein valides, neu erstelltes Access-Token an die externe Anwendung zurück.

Mit Erhalt eines gültigen Access-Tokens kann nun die externe Anwendung ihre fachlichen Aufrufe zusammen mit dem
...
Das nachfolgende Sequenzdiagramm zeigt noch einmal schematisch den logischen Ablauf der Authentifizierung und
Autorisierung im Zusammenspiel der System-Komponenten und dem Authentication-Gateway:
...
Authentifizierung ohne _Authentication-Gateway_ zum Einsatz kommt. Diese Authentifzierungs- und Autorisierungs-Variante
wird aus Vereinfachungsgründen der Migration von externen Anwendungen auf die neue `IsyFact Version-3` zusätzlich
beibehalten, da hierdurch die Umstellungsaufwände reduziert werden.

So muss der verwendete User für diese externe Anwendung noch nicht gemäß _SpringSecurity (OAuth2.0)_ auf einen
_TechnischenUser (Client)_ migriert worden sein, sondern es kann nach wie vor ein _NatürlichenNutzer_ zur
Authentifizierung verwendet werden. Zusätzlich kann die externe Anwendung den gleichen Kommunikationskanal für die
Authentifizierung verwenden, wie für ihre fachlichen Requests.
...
Access-Token. Dieses Token muss er sich zuerst beschaffen, indem er sich beim _IdentityAccessManagement-Service (IAM)_
authentifiziert.

Eingehende Authentifizierungsanfragen werden hierbei im _Service-Gateway_ erkannt und mit den übergebenen
User-Credentials an den _IAM-Service_ weitergeleitet.
...
Das nachfolgende Sequenzdiagramm zeigt noch einmal schematisch den logischen Ablauf der Authentifizierung und
Autorisierung im Zusammenspiel der System-Komponenten mit dem _Service-Gateway_ aber ohne _Authentication-Gateway_:
...
Credentials sicherstellt oder wenn keine andere Art der Berechtigungszuteilung verfügbar ist. So wird die
Authentifizierung für einen _TechnischenNutzer_ über diesen Credential-Flow abgebildet.

Obwohl in diesem Verfahren die Clientanwendung direkten Zugang auf die User-Credentials hat,
so dürfen diese nur einmalig für eine Anfrage nach einem Access-Token verwendet und müssen nach dessen Erhalt sofort
verworfen werden. Somit wird dabei vermieden, dass die Clientanwendung die User-Credentials für zukünftige Zugriffe speichern müsste, da diese durch ein Access-Token
...
Im vorherigen Bild wird der logische Ablauf des _Resource-Owner-Password-Credential_-Flows dargestellt. Hierbei wird die
Konfiguration des _TechnischenNutzer_ als der _Resource-Owner_ dargestellt.

Einem Confidential Client wird die Konfiguration (1.) eines _TechnischenNutzers_ mit den User-Credentials `Username`
und `Passwort` zugeordnet. Die so konfigurierte (Confidential-Client)-Anwendung schickt (2.) dann diese Credentials an
das zuständige Identity-Access-Management (IAM). Im IAM erfolgt mit den so übergebenen Credentials die
Authentifizierung, die im Ergebnis ein gültiges Access-Token als Antwort (3.) zurückgeliefert. Die Client-Anwendung kann
mit dem so erhaltenen Access-Token dann ihre Fachanwendungen aufrufen. Das Access-Token enthält alle notwendigen Daten,
um zu verifizieren, ob die aufrufende Anwendung ordnungsgemäß authentifiziert und autorisiert ist.


[[client-credential-flow]]
====== Client-Credential Flow

Der _Client-Credential_-Flow wird und darf nur zur Berechtigungszuteilung eingesetzt werden, wenn die Clientanwendung
selbst auch der Besitzer der Credentials und damit als Confidential-Clientanwendung eingestuft ist. Es erfolgt hierbei
keine Interaktion durch einen Benutzer, der sein Passwort eingeben müsste, sondern die Schritte zur Authentifizierung
und Autorisierung der Clientanwendung laufen selbständig als Hintergrundprozess analog einer Batch-Anwendung ab. Die
dabei verwendeten Credentials `Client-ID` und `Client-Secret` müssen vor der Nutzung für die Clientanwendung definiert und
mit dem IAM-System abgestimmt sein.
...
Im vorherigen Bild wird der logische Ablauf des _Client-Credential_-Flows dargestellt. Hierbei kann die Clientanwendung
nach einem Access-Token beim IAM anfragen (1.), indem sie dabei ihre eigenen Client-Credentials oder anderweitig
unterstützte Authentifizierungskriterien verwendet. Die Authentifizierung mit diesen so übermittelten Credentials
erfolgt dann im IAM. Bei erfolgreicher Authentifizierung wird (2.) ein gültiges Access-Token als Antwort zurückgeliefert.
Mit dem so erhaltenen Access-Token kann die Client-Anwendung dann ihre Fachanwendungen aufrufen. Das Access-Token enthält alle
notwendigen Daten, um zu verifizieren, ob die aufrufende Anwendung ordnungsgemäß authentifiziert und autorisiert ist.
