...
Der Benutzer gibt hier seine Authentifizierungsdaten (z.B. User-Name u. Passwort) ein.
...
Der Benutzer ist nun eingeloggt.
Bei jeder weiteren Anfrage fügt der Portal-Webserver das Access Token aus der user session hinzu.
So kann der Benutzer weiter mit der Web-App arbeiten, bis er sich explizit ausloggt oder das Access Token abläuft.
