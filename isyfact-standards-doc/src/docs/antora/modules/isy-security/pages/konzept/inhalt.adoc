...
Gemäß der xref:isyfact-standards-doku:referenzarchitektur:fachlich.adoc#[IsyFact-Referenzarchitektur] wird bei der Authentifizierung nach vordefinierten xref:isyfact-standards-doku:referenzarchitektur:software-technisch.adoc#prinzipien-der-sicherheitsarchitektur[Prinzipien der Sicherheitsarchitektur] unterschieden, wie das nachfolgende Bild zeigt:
...
Dabei ergeben sich die unterschiedlichen Kommunikationskanäle _Portal_, _Service-Gateway_, _Authentication-Gateway_ oder _interne Anwendungs-Authentifizierung_ (Batch/Task/Kontextwechsel), über welche ein Authentifizierungs-Request an der Anwendungslandschaft ankommt bzw. verarbeitet wird.

Alle daran beteiligten Komponenten unterscheiden sich in ihrem Aufbau, ihrer Konfiguration und ihrer Funktionalität, so dass jede genau die speziellen Kommunikationskanal-Eigenheiten - siehe dazu Kapitel <<aufruf-weboberflaeche-portal>>, <<aufruf-service-gateway-ohne-authentication-gateway>>, <<aufruf-service-gateway-mit-authentication-gateway>> und <<ausfuehrung-batch-u-task>> bzw. <<aufruf-von-nachbarsystemen>> - am besten bedienen kann.
...
==== Authentifizierungswege als Mandanten
Die bisher aufgezeigten Authentifizierungswege werden in Bezug auf `isy-security` als jeweilig unterschiedliche Mandanten behandelt bzw. konfiguriert.
...
Um nun für jeden Mandanten eine eindeutige Zuordnung bzw. eine Mandanten spezifische Gruppierung der Properties herzustellen, muss der Property-Key im Pfad eine Kennung des Mandanten beinhalten:

`isy.security.oauth2.resourceserver.jwt.tenants.*<mandant-id>*`

Der Platzhalter `<mandant-id>` wird in der konkreten Konfigurationsausprägung auf einen aussagekräftigen bzw. eindeutigen Namen des Mandanten gesetzt.
So wäre beispielsweise für die drei Authentifizierungswege aus der Nutzungsschicht (in <<image-multi-tenancy-multi-iam>>) folgende Property-Pfade vorstellbar:

- `isy.security.oauth2.resourceserver.jwt.tenants.*portal*`
- `isy.security.oauth2.resourceserver.jwt.tenants.*sgw*`
- `isy.security.oauth2.resourceserver.jwt.tenants.*agw*`

Sollten für einen Authentifizierungskanal mehrere Instanzen notwendig werden, dann müssen sich deren Namen natürlich auch unterscheiden, wie beispielsweise:

- `isy.security.oauth2.resourceserver.jwt.tenants.*sgw-1*`
- `isy.security.oauth2.resourceserver.jwt.tenants.*sgw-2*`


===== Mandanten spezifische Konfiguration am Beispiel Portal
So bedarf beispielsweise die Authentifizierung einer externen WebClient-Anwendung über das _Portal_ eine Konfiguration des IAM-Service, die berücksichtigt, dass der Aufruf des IAM-Service hier eine Unterscheidung zwischen _internem_ und _externem_ Aufruf benötigt. Wie und wo diese Aufruf-Unterscheidung zum Tragen kommt, zeigt nun das nachfolgende Bild die Kommunikationswege auf, die ein _WebClient_ über ein _Portal_ zur Authentifizierung und Autorisierung durchläuft.
...
- (2) Das _Portal_ (Apache tomcat mit _openid-connect_ Modul) erkennt die ausstehende Authentifizierung und
- (3) liefert an den externen WebClient einen Redirect „Login mit der *externen* Issuer-URI“, um (4) direkt im IAM/KeyCloak der AWL-1 den (5) LoginDialog zu öffnen.
- (6) In diesem Dialog gibt der User des WebClients sein _Username_ und _Passwort_ ein, aus dem im _IAM/KeyCloak_ bei valider Authentifizierung ein _OneTime AutorisationCode_ erzeugt und (7) als Redirect an den _WebClient_ zurückgegeben wird.
- (8) Den Redirect mit dem _OneTime AutorisationCode_ sendet der _WebClient_ wieder an das _Portal_, das (9) diesen über die *interne* Issuer-URI an den _IAM/KeyCloak_ in der AWL-1 weiterleitet.
- Der _IAM/KeyCloak_ der AWL-1 validiert den _OneTime AutorisationCode_ und (10) liefert dann ein _Access-Token_ mit der *internen* Issuer-URI im `iss-Claim` des Tokens an das Portal zurück, das das Token nun Session-bezogen verwaltet und (11) dem ursprünglichen Request des WebClients nun hinzufügt und an die Anwendung weiterleitet.
- Der _WebClient_ (12) sendet nun innerhalb dieser authentifizierten Session seine weiteren fachlichen Requests an das _Portal_, das (13) die ankommenden Requests mit dem Session-bezogenen Access-Token anreichert und an die entsprechende Anwendung in der AWL-1 weiterleitet.
- (14) In der Anwendung wird das Token nun validiert wozu ggf. über die *interne* Issuer-URI das Zertifikat aus dem IAM der AWL-1 vorher nachgeladen werden muss.
...
- `isy.security.oauth2.resourceserver.jwt.tenants.#portal#.issuer-uri`
- `isy.security.oauth2.resourceserver.jwt.tenants.#portal#.jwk-set-uri`

Sollte in der gleichen Anwendungslandschaft AWL z.B. eine weitere Instanz eines Portals, aber gegen eine andere IAM/KeyCloak-Instanz betrieben werden, dann bedarf es genau diese beiden Properties doppelt in der Konfiguration anzulegen und statt der Tenant-Id=`portal` eine andere eindeutige _Tenant-Id_ (`portal-2`) zu verwenden.
