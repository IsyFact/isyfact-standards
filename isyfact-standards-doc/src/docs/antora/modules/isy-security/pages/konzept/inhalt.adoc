...
Falls vor oder nach der Satz-Verarbeitungsschleife eine Datenverarbeitung stattfinden, die evtl. längere Zeit in Anspruch nimmt, dann ist im _Batchrahmen_ auch hierfür eine Möglichkeit der Überprüfung der Token-Gültigkeitsdauer mit ggf. der Anforderung für ein neues _AccessToken_ im Batch-Rahmen vorhanden.
...
Wird in einer Anwendung wie bisher aus der `isy-task`-Bibliothek die Klasse `IsyTaskAutoConfiguration` eingesetzt, so wird über deren Annotation `@EnableScheduling` das _SpringBoot-Scheduler_-Frameworks in die Anwendung mit eingebunden.
...
Zur Absicherung einer _Aufgabe_ mit dem neuen Security-Baustein `isy-security` ist eine weitere `AuthenticatorFactory`-Implementierung mit `IsySecurityAuthenticatorFactory` vorhanden, die als Weiche in eine der beiden _Credential_-Flows dient.
...
Sind als _Authentifizierungsdaten_ die Keys `Client-ID` und `Client-Secret` definiert, so wird als _Authenticator_ die neue Klasse `IsyClientCredentailAuthenticator` zurückgeliefert.
Liegen als _Authentifizierungsdaten_ die Keys `Benutzer` und `Passwort` vor, so wird als _Authenticator_ die neue Klasse `IsyResourceOwnerPasswordCredentailAuthenticator` zurückgeliefert.
Werden keine der genannten _AuthentifizierungsCredentials_ gefunden, dann wird der schon existierende _Authenticator_ `NoOpAuthenticator` zurückgegeben.

Die Implementierung des neuen `IsyResourceOwnerPasswordCredentailAuthenticator` startet dann intern die Authentifizierung über den _ResourceOwnerPasswordCredentail_-Flow im neuen Sicherheitsbaustein `isy-security`, der über das IAM dann einen initialisierten `SecurityContext` inklusive _AccessToken_ zurückliefert.

Die Implementierung des neuen `IsyClientCredentailAuthenticator` startet dann intern die Authentifizierung über den _ClientCredentail_-Flow im neuen Sicherheitsbaustein `isy-security`, der über das IAM dann einen initialisierten `SecurityContext` inklusive _AccessToken_ zurückliefert.
