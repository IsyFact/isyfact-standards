...
Der _Resource-Owner-Password-Credential_-Flow, der als Authentifizierungsdaten die Attribute `Username` und `Passwort` verwendet, sollte nur eingesetzt werden, wenn die involvierte Client-Anwendung ein hohes Vertrauen zur Wahrung der Credentials sicherstellt oder wenn keine andere Art der Berechtigungszuteilung verfügbar ist.
...
Einem Confidential Client wird die Konfiguration (1.) eines techn. Nutzers mit den Authentifizierungsdaten `Username` und `Passwort` zugeordnet.
Die so konfigurierte (Confidential-Client)-Anwendung schickt (2.) dann diese Credentials an den IAM-Service.
...
Die dabei verwendeten Credentials `Client-ID` und `Client-Secret` müssen vor der Nutzung für die Client-Anwendung definiert und mit dem IAM-System abgestimmt sein.
...
Hierbei kann die Client-Anwendung nach einem Access-Token beim IAM anfragen (1.), indem sie dabei ihre eigenen Client-Credentials oder anderweitig unterstützte Authentifizierungskriterien verwendet.
Die Authentifizierung mit diesen so übermittelten Credentials erfolgt dann im IAM-Service.
