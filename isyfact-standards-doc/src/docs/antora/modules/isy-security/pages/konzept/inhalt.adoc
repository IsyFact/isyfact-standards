...
Der `Authentifizierungsmanager` übernimmt die Authentifizierung und Autorisierung von technischen Nutzern am IAM-System.
Mit den aus der betrieblichen Konfiguration gelesenen Credentials des technischen Nutzers (über dessen RegistrationId: entweder Client-ID u. Client-Secret oder Nutzername u. Passwort) erfolgt seine OAuth2.0-konforme Authentifizierung und Autorisierung gegenüber dem IAM-System.
Für den Credential-Typ bzw. _Credential_-Flow xref:client-credential-flow[] ist die Methode `authentifiziereClient` zu verwenden, für den Credential-Typ bzw. _Credential_-Flow xref:resource-owner-password-credential-flow[] die Methode `authentifiziereSystem`. +
