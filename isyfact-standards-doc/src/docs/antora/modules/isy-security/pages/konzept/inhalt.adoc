...
Der techn. Nutzer und seine ihm zugeteilten Rollen werden im an das IAM-System angeschlossene Benutzerverzeichnis verwaltet.
...
Dieser techn. Nutzer muss mit seinen Rollen im Benutzerverzeichnis vorhanden und gepflegt sein.
...
Im IAM erfolgt mit den so übergebenen Authentifizierungsdaten die Authentifizierung sowohl des _Confidential Client_, als auch des techn. Nutzers als _Resource-Owner_, die im Ergebnis ein gültiges Access-Token mit den Rollen des techn. Nutzers als Antwort (3.) zurückgeliefert.

Die Client-Anwendung kann dann, mit dem so erhaltenen Access-Token und in den Rollen des techn. Nutzers, ihre Fachanwendungen aufrufen.
