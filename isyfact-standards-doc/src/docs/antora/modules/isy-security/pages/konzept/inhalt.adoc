...
Damit ein menschlicher Nutzer eine geschützte Anwendung über deren Web-Oberfläche (im Web-Browser) verwenden kann, muss er über den IAM-Service authentifiziert und dazu autorisiert sein.
In `IsyFact 3` ist dafür ein Apache Webserver (Portal Apache) zwischen den Web-Client (Web-Browser) und der Web-Anwendung geschaltet.
Dieser tritt sowohl als Reverse-Proxy auf, sowie als 'Relying Party' für das IAM-System.
Wenn für einen User (noch) keine gültige Session vorhanden ist, leitet der Portal-Apache auf die Login-Seite des IAM-Systems.
Nach erfolgtem Login wird die Autorisierung an die User-Session gebunden und an die Anwendung weitergeleitet.
Die folgende Abbildung zeigt die System-Architektur mit dem Portal-Apache in der Informations- u. Dienstezone vor der Anwendung und dem IAM-Service in der Logik- u. Verarbeitungszone.
...
Ein menschlicher Nutzer, der eine Anwendung über deren Web-Oberfläche (im Web-Browser) bedienen möchte, ist im Besitz von Authentifizierungsdaten (Nutzername u. Passwort, biometrische Daten, ...) mit denen er sich gegenüber dem IAM-System ausweisen kann.
Diese Daten sollten nur zum IAM-Service gelangen und nicht an oder durch andere Systeme kommen.
Deshalb wird der Anwender auf ein Anmeldeformular des IAM-Service umgeleitet, um nach erfolgreicher Authentifizierung & Autorisierung auf die Web-Anwendung weitergeleitet zu werden.
Das nachfolgende Sequenzdiagramm zeigt noch einmal schematisch den logischen und zeitlichen Ablauf sowie das Zusammenspiel der System-Komponenten im Authorization Code Flow mit Confidential Client:
...
Der Benutzer ruft die Web-Anwendung auf, er ist nicht angemeldet bzw. eine eventuelle vorherige Anmeldung ist abgelaufen.
Über die ihm zugeordneten Rolle(n) hat er das Recht, die Web-Anwendung und bestimmte Ressourcen zu verwenden.
Der Aufruf wird im Portal-Webserver von dem mod_auth_openidc Modul abgefangen.

. Umleitung zur Benutzer-Authentifizierung im IAM-Service:
Das Webservermodul erkennt (am fehlenden bzw. ungültigen Access-Token), dass noch keine Authentifizierung erfolgt ist und leitet um zum IAM-Service zur Authentifizierung des Benutzers (mit Client-ID u. redirect_uri für Callback der Web-Anwendung).

. Anmeldeformular mit dem Benutzer:
Der IAM-Service präsentiert dem Benutzer ein (an die Web-Anwendung angepassten) Anmeldeformular.
Der Benutzer gibt hier seine Authentifizierungsdaten (z.B. Nutzername u. Passwort) ein.

. Verifizierung der Authentifizierungsdaten:
Die eingegebenen Authentifizierungsdaten werden vom IAM-Service mit den bei ihm für den Ressourcenbesitzer hinterlegten verglichen.
Falls die Authentifizierung fehlschlägt, wird der Vorgang abgebrochen (und eine entsprechende Fehlerseite angezeigt).

. Weiterleitung zum mod_auth_openidc Modul mit Authorization Code:
Bei positiver Authentifizierung antwortet der IAM-Service auf den Authentifizierungsaufruf mit einer Umleitung zurück zum mod_auth_openidc Modul und einem Authorization Code.
Beim Authorization Code handelt es sich um ein einmalig verwendbares und kurzlebiges Token.
Er dient als Vor-Authorisierung und ist an das mod_auth_openidc Modul über die Client-ID gebunden.
...
Das ist notwendig, um sicherzustellen, dass das Access Token an die richtige Web-Anwendung ausgeliefert wird und nicht z.B. eine Redirection URI Manipulation stattgefunden hat.

. Austausch Authorization Code gegen Access Token:
Das mod_auth_openidc Modul schickt den erhaltenen Authorization Code und ihre Legitimation an den IAM-Service zur Autorisierung des Benutzers.
Nach Prüfung der Legitimation des Portal-Webservers und der Gültigkeit des Authorization Codes antwortet der IAM-Service mit einem dauerhafteren Access Token zu den Ressourcen des Ressourcenbesitzers.

. Aufruf der Web-Anwendung mit Access Token:
Das mod_auth_openidc Modul speichert das erhaltene Access Token innerhalb der user session und ruft damit die Web-Anwendung auf.

. Anzeige der Web-App:
Mit Erhalt des Access Token bekommt die Web-App die Autorisierung, Aktionen im Namen des Ressourcenbesitzers durchführen und geschützte Ressourcen verwenden.
Mit den Ergebnissen der Aktionen bzw. den Inhalten der geschützten Ressourcen wird die Ansichtsseite der Web-Anwendung erstellt und im Web-Client zur Anzeige gebracht.

. Weitere Interaktionen mit der Web-Anwendung:
Der Benutzer ist nun angemeldet.
Bei jeder weiteren Anfrage fügt der Portal-Webserver das Access Token aus der user session hinzu.
So kann der Benutzer weiter mit der Web-Anwendung arbeiten, bis er sich explizit ausloggt oder das Access Token abläuft.
Dann muss entweder die Anwendung mithilfe des Refresh Token ein neues Access Token beim IAM-Service anfordern oder der Benutzer muss sich erneut anmelden.
