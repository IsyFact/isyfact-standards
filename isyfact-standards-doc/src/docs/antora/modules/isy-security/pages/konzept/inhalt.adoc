...
Die rufende Anwendung (bzw. das SGW als Mittelsmann) legt anschließend dieses Access-Token im `SecurityContext` ab und präsentiert es der aufgerufenen Service-Schnittstelle der IsyFact-Anwendung.
...
Interne Anwendungen sind als Confidential Client eingestuft und verwahren ihre Authentifizierungsdaten (entweder Client-ID u. Client-Secret oder Nutzername u. Passwort) in ihrer Konfiguration (mit einer ApplicationID als Schlüssel).
Die Methode `authentifiziere(applicationID)` im Authentifizierungsmanager liest diese Authentifizierungsdaten aus der betrieblichen Konfiguration und führt damit eine OAuth2.0-konforme Authentifizierung und Autorisierung der Anwendung gegenüber dem IAM-System durch.
Dabei wird innerhalb der Methode `authentifiziere(applicationID)` anhand der property `authorization-grant-type` der passende Authentication-Flow ausgewählt:
Der Wert 'client-credentials' bedeutet, den _Credential_-Flow xref:client-credential-flow[] und damit die Methode `authentifiziereClient` zu verwenden, der Wert 'password'  steht für den _Credential_-Flow xref:resource-owner-password-credential-flow[] für den die Methode `authentifiziereSystem` zu verwenden ist - jeweils mit der ApplicationID und den Authentifizierungsdaten der internen Anwendung. +
...
- extrahiert die Authentifizierungsdaten und prüft sie auf Vollständigkeit und Validität,

- wählt anhand des Typs der Authentifizierungsdaten den passenden Authentication-Flow:
Für den Credential-Typ bzw. _Credential_-Flow xref:client-credential-flow[] ist die Methode `authentifiziereClient` zu verwenden, für den Credential-Typ bzw. _Credential_-Flow xref:resource-owner-password-credential-flow[] die Methode `authentifiziereSystem` - jeweils mit der ApplicationID des SGW und den Authentifizierungsdaten der externen Anwendung,
...
Die Methode `authentifiziereClient(applicationID, clientID, clientSecret, bhknz)` im Authentifizierungsmanager liest über die ApplicationID die URL des zu verwendenden IAM aus der betrieblichen Konfiguration und führt damit und den übergebenen Authentifizierungsdaten der anfordernden Anwendung den xref:client-credential-flow[] gegenüber dem IAM-System durch.
...
Die Methode `authentifiziereSystem(applicationID, username, password, bhknz)` im Authentifizierungsmanager liest über die ApplicationID die Authentifizierungsdaten des vermittelnden confidential client (SGW) und die URL des zu verwendenden IAM aus der betrieblichen Konfiguration und führt damit und den übergebenen Authentifizierungsdaten der anfordernden Anwendung den xref:resource-owner-password-credential-flow[] gegenüber dem IAM-System durch.
