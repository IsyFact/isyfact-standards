...
Ein menschlicher Nutzer, der eine Anwendung über deren Web-Oberfläche (im Web-Browser) bedienen möchte, ist im Besitz von Authentifizierungsdaten (Nutzername u. Passwort, biometrische Daten, ...) mit denen er sich gegenüber dem IAM-System ausweisen kann. Diese Daten sollten nur zum IAM-Service gelangen und nicht an oder durch andere Systeme kommen. Deshalb wird der Anwender auf ein Anwenderformular des IAM-Service umgeleitet, um nach erfolgreicher Authentifizierung & Autorisierung auf die Web-Anwendung weitergeleitet zu werden.
