...
Im vorherigen Bild wird der logische Ablauf des _Resource-Owner-Password_-Flows dargestellt. Als _Resource-Owner_ wird
hier der Inhaber bzw. Wissensträger der User-Credentials `Username` und `Passwort` bezeichnet, was für
Confidential-Clients dann im allgemeinen ein Admin, DevOp oder eine Person ist, die die (1.) Konfiguration der
Anwendungen mit den User-Credentials in der Produktionsumgebung einrichtet bzw. vornimmt.

Die so konfigurierte (Confidential-Client)-Anwendung schickt (2.) dann zur Authentifizierung und Autorisierung diese
Passwort-Credentials an das zuständige Identity-Access-Management-System, um die Credentials dort überprüfen zu lassen
und bei validen Credentails als Antwort (3.) ein gültiges Access-Token zu erhalten. Mit diesem Access-Token kann die
Client-Anwendung dann andere Fachanwendungen aufrufen, da das Access-Token alle notwendigen Daten enthält, damit  die
Fachanwendung verifizieren, ob die aufrufende Anwendung authentifiziert und autorisiert ist, dies zu tun.
