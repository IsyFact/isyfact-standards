...
image::isy-security:konzept/authorization-code-flow.dn.svg[align="center"]
...
Die Web-App erkennt (am fehlenden Access-Token), dass noch keine Authentifizierung erfolgt ist und leitet um zum IAM-Service zur Authentifizierung des Benutzers (mit Client-ID u. redirect_uri für Callback der Web-App).
...
Weiterhin soll dem Ressourcenbesitzer die Möglichkeit gegeben werden, den anfragenden Client, als auch die angefragten Rechte zu überprüfen (Zustimmung).
Die in IsyFact verwendete Authentifizierungsmethode ist ein Login-Dialog, in dem der Ressourcenbesitzer durch Eingabe seiner Authentifizierungsinformationen (User-Name u. Passwort) die Zustimmung erteilt.
Andere Methoden wären z.B. eine Zwei-Faktor-Authentifizierung oder die Verwendung biometrischer Merkmale des Ressourcenbesitzers.
Die eingegebenen Authentisierungsdaten werden vom IAM-Service mit den bei ihm für den Ressourcenbesitzer hinterlegten verglichen.
Falls die Authentifizierung fehlschlägt, wird der Vorgang abgebrochen (und eine entsprechende Fehlerseite angezeigt).

. Umleitung zu Web-App mit Authorization Code:
Bei positiver Authentifizierung antwortet der IAM-Service auf den Authentifizierungsaufruf mit einer Umleitung zurück zur Web-App und einem Authorization Code.
...
Tausch gegen eine andere (dauerhaftere) Legitimation in einem späteren Teil des Authorization Code Grants.
Das ist notwendig, um sicherzustellen dass das Zugangstoken an die richtige Web-App ausgeliefert wird und nicht z.B. eine Redirection URI Manipulation stattgefunden hat.

. Austausch Authorization Code gegen Zugangstoken:
die Web-App schickt den erhaltenen Authorization Code und ihre Legitimation an den IAM-Service zur Autorisierung.
Dieser prüft die Anfrage und antwortet mit einem dauerhafteren Zugangstoken (Access Token) für die Web-App zu den Ressourcen des Ressourcenbesitzers.
Da die Web-App als Confidential Client angesehen werden kann, wird für ihre Authentifizierung der OAuth 2.0 Grant-Type 'Client Credentials' (Client-ID u. Client-Secret) verwendet.
Im IsyFact Standard ist vorgesehen, dass dieses Zugangstoken eine begrenzte Lebensdauer hat und die Rollen des Ressourcenbesitzers enthält. +
Die Web-App speichert das erhaltene Zugangstoken innerhalb der user session.
Mit Erhalt des Zugangstokens bekommt sie die Autorisierung im Namen des Ressourcenbesitzers eine Aktion durchzuführen, bzw. mehrere - im Rahmen der seiner Rolle(n) zugeordneten Rechte, bis das Zugangstoken abgelaufen ist.
...
Der Ressourcen-Server verifiziert das Zugangstoken (ausstellender IAM-Service, Lebensdauer), wandelt die darin mitgegebenen Rollen des Benutzers in lokale Rechte und gibt die geschützte Ressource frei - falls der Benutzer das Recht dazu hat.
...
Die Web-App kann so weitere Aktionen im Namen des Ressourcenbesitzers durchführen und weitere geschützte Ressourcen verwenden, bis entweder der Benutzer die Verwendung der Web-App beendet (durch Logout) oder das Zugangstoken seine Lebensende erreicht hat. Dann wird ein neues Zugangstoken benötigt.

Im IsyFact Standard ist vorgesehen, dass die in den Schritten 2 (Umleitung zu Benutzer-Authentifizierung) und 5 (Austausch Authorization Code gegen Access Token) beschriebene Kommunikation der Web-App mit dem IAM-Service durch im Portal-Webserver vorgeschaltete Module erfolgt:
Durch ein Reverse-Proxy und dem Modul mod_auth_openidc wird der Webserver zu einer 'Relying Party', das die Umleitung an den IAM-Service zur Benutzer-Authentifizierung macht, sowie den Austausch des Authorization Code gegen das Zugangstoken und es in der user session der Anwendung übergibt.
Dadurch hat die Anwendung selbst keine Verbindung zum IAM-Service und ist automatisch vor unbefugtem Zugriff geschützt und erhält auch automatisch das Zugangstoken innerhalb der user session.
...
image::isy-security:konzept/authorization-code-connections.dn.svg[align="center"]
