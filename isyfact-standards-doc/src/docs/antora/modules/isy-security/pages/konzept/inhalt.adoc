...
Das Webservermodul erkennt (am fehlenden Access-Token), dass noch keine Authentifizierung erfolgt ist und leitet um zum IAM-Service zur Authentifizierung des Benutzers (mit Client-ID u. redirect_uri für Callback der Web-App).

. Authentifizierungs-Dialog mit dem Benutzer:
Dem Benutzer wird ein Login-Dialog präsentiert, in den er seine Authentifizierungsinformationen (User-Name u. Passwort) eingibt.
...
Das OAuth-Modul schickt den erhaltenen Authorization Code und ihre Legitimation an den IAM-Service zur Autorisierung.
Dieser prüft die Anfrage und antwortet mit einem dauerhafteren Zugangstoken (Access Token) zu den Ressourcen des Ressourcenbesitzers.

. Das OAuth-Modul speichert das erhaltene Zugangstoken innerhalb der user session und ruft damit die Web-App auf.
Mit Erhalt des Zugangstokens bekommt die Web-App die Autorisierung, Aktionen im Namen des Ressourcenbesitzers durchführen und geschützte Ressourcen verwenden.
...
image::isy-security:konzept/rollen-beziehungen.png[align="center",pdfwidth=70%,width=70%]
