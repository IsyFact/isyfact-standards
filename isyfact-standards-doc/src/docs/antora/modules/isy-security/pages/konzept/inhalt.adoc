...
image::konzept/security-schnittstellen.png[align="center"]

Bei Instanziierung des Bausteins 'Security' wird die anwendungsspezifische Rollen-Rechte-Datei eingelesen, sowie der `Berechtigungsmanager` und der `Authentifizierungsmanager` damit erzeugt.
Beide Manager sowie eine Liste aller für diese Anwendung spezifischen Rollen werden nach Außen angeboten.
...
Bei erfolgreicher Authentifizierung werden die vom IAM-System zurückgegebenen Daten (Access-Token, Rollen des Anwenders, ...) im Spring Secutity Context abgelegt, zusammen mit den aus den Rollen resultierenden Rechten des Anwenders.
