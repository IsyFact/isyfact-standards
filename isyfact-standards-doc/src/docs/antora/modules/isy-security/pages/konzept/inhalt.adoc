...
Dabei ergeben sich die unterschiedlichen Kommunikationskanäle _Portal_, _Service-Gateway_ oder _Authentication-Gateway_, über welche ein Authentifizierungs-Request an der Anwendungslandschaft ankommt bzw. verarbeitet wird.

Alle drei Komponenten unterscheiden sich in ihrem Aufbau, ihrer Konfiguration und ihrer Funktionalität, so dass jede genau die speziellen Kommunikationskanal-Eigenheiten - siehe dazu Kapitel <<aufruf-weboberflaeche-portal>>, <<aufruf-service-gateway-ohne-authentication-gateway>> und <<aufruf-service-gateway-mit-authentication-gateway>> - am besten bedienen kann.
...
==== Authentifizierungswege als Mandanten
Die bisher aufgezeigten Authentifizierungswege über _Portal_-, _Service_- und _Authentication-Gateway_ werden in Bezug auf `isy-security` als jeweilig unterschiedliche Mandanten behandelt bzw. konfiguriert.
...
`spring.security.oauth2.resourceserver.jwt.tenants.<realm>`

Der Platzhalter `<realm>` wird in der konkreten Konfigurationsausprägung auf den Namen des Mandanten gesetzt.
So wäre beispielsweise für die drei Authentifizierungswege folgende Property-Pfade vorstellbar:
...
Sollten für einen der drei Authentifizierungskanäle mehrere Instanzen notwendig werden, dann müssen sich deren Namen natürlich auch unterscheiden, wie beispielsweise:
...
Sollte in der gleichen Anwendungslandschaft AWL eine weitere Instanz eines Portals, aber gegen eine andere IAM/KeyCloak-Instanz betrieben werden, dann bedarf es genau diese beiden Properties doppelt in der Konfiguration anzulegen und statt der Tenand-Id=`portal` eine andere eindeutige _Tenand-Id_ (`portal-2`) zu verwenden.
