...
* Die Authentifizierung und grobgranulare Autorisierung erfolgt an den Außenschnittstellen der Systemlandschaft (Portal, Authentication-Gateway bzw. veraltet: Service-Gateway).
...
* Die feingranulare Autorisierung geschieht in der Nutzungsschicht und im Anwendungskern der IT-Systeme über den IsyFact-Baustein Security (bzw. veraltet: Service-Gateway).
...
Authentifikation: Prüfung der Identität eines Benutzers (oder Systems) als Zugangs- und Rechtekontrolle für ein System (durch eindeutige Eigenschaften (z.B. Biometrie) oder geheimes Wissen (z.B. Passwort)).
...
Nach der erfolgreichen Authentifizierung eines Anwenders leitet das IAM-System die Anfrage an das IT-System weiter - angereichert mit weiteren Informationen über den Anwender (speziell: zugeordnete Rollen).
