...
* Authentifizierungsinformationen, wie Benutzername und Passwort, werden nicht über IT-Systeme hinweg weitergegeben . Nur der Eigentümer der Authentifizierungsinformationen und der Authentifizierungsserver besitzen Kenntnis über diese Informationen.
...
Für die zentrale Authentifizierung wird in IsyFact ein OAuth 2.0 konformes IAM-System verwendet.
