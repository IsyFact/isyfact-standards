...
Aktuell erfolgt die Authentifizierung von Anfragen externer Anwendungen immer über das Service-Gateway.
Dafür müssen bei jeder Anfrage der externen Anwendung immer auch die Authentifizierungsdaten mitgeschickt werden.
Das Service-Gateway lässt die Authentifizierungsdaten durch den IAM-Service prüfen und leitet erst nach dessen OK die fachliche Anfrage an das Anwendungssystem weiter - jetzt zusammen mit dem Access-Token.

Die nachfolgende Abbildung zeigt die aktuell verwendete System-Architektur, in der das Service-Gateway mithilfe des IAM-Service die Authentifizierung durchführt.
...
. Eine externe Anwendung sendet fachliche Anfrage zusammen mit Authentifizierungsdaten an das Service-Gateway.
...
Die folgende Abbildung zeigt die zukünftig intendierte System-Architektur, die bei Aufrufen von externen Anwendungen zum Einsatz kommen soll.
...
. Das _Authentication-Gateway_ übergibt das Acess-Token der externen Anwendung.
...
. Für jede dieser fachlichen Anfragen wird das jeweils mitgelieferte Access-Token validiert. Das aufgerufene Anwendungssystem prüft hierzu dessen Signatur, dessen Gültigkeit und die im Access-Token hinterlegten Berechtigungen,
  die im Hinblick auf den jeweils aufgerufenen Service vollständig vorhanden sein müssen.

Die neue System-Architektur hat den Vorteil, dass sie das Service-Gateway von der Aufgabe befreit, 
neben dem Aufruf einer anderen Geschäftsanwendung auch einen entsprechenden Authentifizierungs-Prozess mit durchzuführen.
Der Authentifizierungsprozess wurde in das _Authentication-Gateway_ verlagert. 
Dies ermöglicht eine Wiederverwendung des Access-Tokens und führt letztlich zu einer starken Verringerung von Authentifizierungs-Anfragen an das IAM.
