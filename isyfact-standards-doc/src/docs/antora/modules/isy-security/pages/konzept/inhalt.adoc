...
Das Service-Gateway nimmt die fachliche Anfrage von Außen entgegen, prüft die Berechtigung und leitet die Anfrage weiter an den internen IsyFact-Service.

[[aufruf-service-gateway-ohne-authentication-gateway]]
===== Authentifizierung über das SGW am IAM-Service

Aktuell erfolgt die Authentifizierung von Anfragen externer Anwendungen direkt im Service-Gateway.
Dafür müssen bei jeder Anfrage auch die Authentifizierungsdaten mitgeschickt werden.
...
In einer zukünftigen Version von `IsyFact` wird eine neue Authentifizierungslogik (xref:aufruf-service-gateway-mit-authentication-gateway[]) für die Authentifizierung von Anwendungen, die außerhalb der Anwendungslandschaft betrieben werden, eingeführt.
