...
Dazu bietet die _Batch-Steuerung_ von `IsyFact-3` nach wie vor die bisherige API an, um den bisherigen xref:resource-owner-password-credential-flow[] zur Authentifizierung zu nutzen.
...
Das bedeutet der _isy-batchrahmen_ verfügt mit `IsyFact-3` innerhalb der Satz-Verarbeitungsschleife über einen Validierungsmechanismus, der bei jedem Durchlauf einer Datensatz-Verarbeitung vor den eigentlichen Aufrufen der Fachanwendungskomponenten die Gültigkeitsdauer des _Access-Tokens_ überprüft.
...
Mit `IsyFact-3` soll bevorzugt der xref:client-credential-flow[] und den Properties `Client-ID` und `Client-Secret` verwendet werden, da der xref:resource-owner-password-credential-flow[] und den Properties `Benutzer` und `Passwort` mit _SpringSecurity OAuth2.1_ aus der Spezifikation herausgenommen ist.

Zur Unterstützung einer weichen Migration wird mit `IsyFact-3` der xref:resource-owner-password-credential-flow[] mit `Benutzer` und `Passwort` weiterhin möglich sein, da zum Wechseln des Credential-Flows auch erst eine Migration des _NatürlichenNutzers_ auf einen _TechnischenNutzer_ im IAM abgeschlossen sein muss.
...
Mit `IsyFact-3` wird für die _Aufgaben_-Ausführung mit Authentifizierung und Autorisierung zusätzlich der neue xref:client-credential-flow[] eingeführt.
...
Dadurch können auch Aufgaben, deren Nutzer noch nicht zu einem _TechnischenNutzer_ mit `Client-ID` und `Client-Secret` in IAM migriert wurden, weiterhin mit `IsyFact-3` ausgeführt werden.
