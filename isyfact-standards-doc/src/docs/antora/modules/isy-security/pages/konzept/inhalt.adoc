...
Das OAuth 2.0 Autorisierungsframework ermöglicht Fremdanwendungen beschränkten Zugang zu HTTP-Services, entweder stellvertretend für den Ressourcenbesitzer - durch Organisation einer Erlaubnis-Interaktion zwischen dem Ressourcenbesitzer, der Fremdanwendung und dem HTTP-Service - oder indem - nach Prüfung - der Fremdanwendung der Zugang für sich selbst erlaubt wird.

Die direkte Authentifizierung des Clients mit dem Identitätsnachweis des Ressourcenbesitzers birgt einige Gefahren, Probleme und Einschränkungen, auf die hier nicht weiter eingegangen wird.
Das OAuth 2.0 Framework begegnet dem mit der Einführung einer Autorisierungs-Ebene und der Trennung von Client und Ressourcenbesitzer. +
In OAuth 2.0 authentisiert sich der Ressourcenbesitzer bei einem zentralen IAM-Service, der dem Client ein individuelles Access Token ausstellt.
Mit diesem Access Token verlangt der Client Zugang zu der gewünschten geschützten Ressource.
Der Ressourcen Server gewährt den Zugang nach Validierung des Access Tokens gegenüber dem IAM-Service.
So kommen weder der Client noch der Ressource-Server mit den Authentifizierungsinformationen in Berührung.
Zu Beachten ist hierbei, dass OAuth 2.0 mit dem Access Token nur eine Zugangserlaubnis - im Sinne eines Schlüssels zu einem Schloss - an den Client aushändigt, aber keinerlei Informationen über den Ressourcenbesitzer.
...
- Client: +
Eine Anwendung, die im Namen/für den Ressourcenbesitzer eine geschützte Ressource anfragt.
...
Eine Analogie wäre z.B. eine Schlüsselkarte, die der Gast (Client) vom Empfang (IAM-Service) nach dem Einchecken (Authentifizierung  u. Autorisierung) vom Empfang (IAM-Service) erhält.
Das Zimmerschloss (Ressourcen-Server) braucht den Namen des Gastes nicht, es muss nur die Gültigkeit der Schlüsselkarte prüfen.
Andere Zimmer können damit nicht geöffnet werden und nach Ablauf der Aufenthaltsdauer verliert die Schlüsselkarte automatisch ihre Gültigkeit und der Gast kann nicht mehr in sein Zimmer.
...
In der obigen Hotelanalogie wäre das z.B. die Verlängerung der Aufenthaltsdauer oder ein Zimmerwechsel, was an der Rezeption geklärt werden muss, aber ohne dass der Gast den Eincheck-Prozess erneut durchlaufen muss.
