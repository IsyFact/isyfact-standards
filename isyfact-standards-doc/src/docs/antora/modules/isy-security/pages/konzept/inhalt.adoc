...
Aktuell erfolgt die Authentifizierung von Anfragen über die Web-Oberfläche direkt im Service-Gateway.
Dafür müssen bei jeder Anfrage auch die Authentifizierungsdaten mitgeschickt werden.
Das Service-Gateway lässt die Authentifizierungsdaten durch den IAM-Service prüfen und leitet erst nach dessen OK die fachliche Anfrage an das Anwendungssystem weiter.
