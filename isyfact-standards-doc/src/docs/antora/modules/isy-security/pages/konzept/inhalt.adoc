...
Die bisher aufgezeigten Authentifizierungswege über Portal-, Service- und Authentication-Gateway werden in Bezug auf `isy-security` als jeweilig unterschiedliche Manadanten behandelt bzw. konfiguriert.

Das bedeutet, dass für jeden Mandanten unterschiedliche Konfigurationen möglich sind.
Ein Portal-Gateway bedarf beispielsweise einer anderen oder gar erweiterten Konfiguration gegenüber einem Service-Gateway.
...
So bedarf beispielsweise die Authentifizierung einer externen Webclient-Anwendung über das Portal-Gateway eine Konfiguration des IAM-Service, die berücksichtigt, dass der Aufruf des IAM-Service hier eine Unterscheidung zwischen _internem_ und _externem_ Aufruf benötigt.
