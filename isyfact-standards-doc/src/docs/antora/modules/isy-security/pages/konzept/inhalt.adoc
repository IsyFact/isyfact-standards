...
Wenn in einer Anwendungslandschaft das IAM über mehrere unterschiedliche URIs aufgerufen werden kann, dann benötigt eine Anwendung mindestens eine bis alle dieser URIs, um eine vollständige Authentifizierung inklusive darauffolgender Tokenvalidierung und Autorisierung mit dem IAM durchzuführen.
...
Die verwendete URI (Issuer-URI) zum Aufruf des IAM wird dort nach valider Authentifizierung im ausgestellten Access-Token hinterlegt und mit der in der Geschäftsanwendung konfigurierten URI im Rahmen der Tokenprüfung abgeglichen.
Die Prüfung schlägt oft fehl, da das Access-Token über die externe URI ausgestellt wurde, in der Geschäftsanwendung jedoch (nur) die interne URI konfiguriert ist.

Um nun in der Anwendung Access-Tokens erfolgreich zu validieren, die über eine in der Anwendung abweichend konfigurierte IAM-URI ausgestellt wurden, müssen alle genutzten URIs des IAM-Systems in der Anwendung hinterlegt werden.
