...
NOTE: Authentifikation: Verifikation der Identität eines Benutzers (menschl. xref:glossary:glossary:master.adoc#glossar-anwender[Anwender] oder techn. System) als Zugangskontrolle zu einem geschützten System.
Eine Authentisierung durch Präsentation eindeutiger Eigenschaften (z.B. Biometrie) oder geheimen Wissens (z.B. Passwort).
...
Das IAM-System verwaltet *Benutzer* (menschliche Anwender oder technische System) und ihre *Rollen* zentral für alle Komponenten der Anwendungs- und Systemlandschaft.
...
Im Regelfall werden diese Daten der originären Anfrage als aus dem Access-Token initialisierter `SecurityContext` übergeben.
...
Er bestätigt kann bei erhöhtem Sicherheitsbedarf (Revoked Token) auch gegenüber dem Ressourcen-Server die Gültigkeit des Access-Tokens bestätigen, das der Client dem Ressourcen-Server in der Zugangsanfrage mitgegeben hat (siehe Validierung der Access-Token).
...
Die folgende Abbildung zeigt die System-Architektur mit dem Webserver in der Informations- u. Dienstzone vor der Anwendung und dem IAM-Service in der Logik- u. Verarbeitungszone.
...
Somit ruft der Batch-Client die Komponente _Batch-Logik_ in der Nutzungsschicht der Anwendung auf, dass die notwendigen Initialisierungen vornimmt.
...
Diese Rechte werden fachliche Rollen zugeordnet, die dann wiederum den Anwendern der Anwendung zugeordnet werden können.
