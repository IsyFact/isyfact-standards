...
Um nun für jeden Mandanten eine eindeutige Zuordnung bzw. eine Mandanten spezifische Gruppierung der Properties herzustellen, muss der Property-Key im Pfad den Namen des Mandanten beinhalten:

`spring.security.oauth2.resourceserver.jwt.tenants.<mandant-id>`

Der Platzhalter `<mandant-id>` wird in der konkreten Konfigurationsausprägung auf den Namen des Mandanten gesetzt.
So wäre beispielsweise für die drei Authentifizierungswege aus der Nutzungsschicht folgende Property-Pfade vorstellbar:

- `spring.security.oauth2.resourceserver.jwt.tenants.*portal*`
- `spring.security.oauth2.resourceserver.jwt.tenants.*sgw*`
- `spring.security.oauth2.resourceserver.jwt.tenants.*agw*`
...
- `spring.security.oauth2.resourceserver.jwt.tenants.*sgw-1*`
- `spring.security.oauth2.resourceserver.jwt.tenants.*sgw-2*`


===== Mandanten spezifische Konfiguration am Beispiel Portal
So bedarf beispielsweise die Authentifizierung einer externen WebClient-Anwendung über das _Portal_-Gateway eine Konfiguration des IAM-Service, die berücksichtigt, dass der Aufruf des IAM-Service hier eine Unterscheidung zwischen _internem_ und _externem_ Aufruf benötigt. Wie und wo diese Aufruf-Unterscheidung zum Tragen kommt, zeigt nun das nachfolgende Bild die Kommunikationswege auf, die ein WebClient über ein Portal zur Authentifizierung und Autorisierung durchläuft.
...
- (8) Den Redirect mit dem _AutorisationCode_ sendet der _WebClient_ wieder an das _Portal_, das (9) diesen über die *interne* Issuer-URI an den _IAM/KeyCloak_ in der AWL-1 weiterleitet.
- Der _IAM/KeyCloak_ der AWL-1 validiert den _AutorisationCode_ und (10) liefert dann ein _Access-Token_ mit der *internen* Issuer-URI im `iss-Claim` des Tokens an das Portal zurück, das das Token nun Session-bezogen verwaltet und (11) dem ursprünglichen Request des WebClients nun hinzufügt und an die Anwendung weiterleitet.
...
- `spring.security.oauth2.resourceserver.jwt.tenants.#portal#.issuer-uri`
- `spring.security.oauth2.resourceserver.jwt.tenants.#portal#.jwk-set-uri`
