...
Der Aufruf wird im Webserver von dem OAuth-Modul abgefangen.
...
Er dient als Vor-Authorisierung und ist an das OAuth-Modul über die Client-ID gebunden.
...
Das OAuth-Modul schickt den erhaltenen Authorization Code und ihre Legitimation an den IAM-Service zur Autorisierung des Benutzers.
...
Das OAuth-Modul speichert das erhaltene Access Token innerhalb der user session und ruft damit die Web-App auf.
