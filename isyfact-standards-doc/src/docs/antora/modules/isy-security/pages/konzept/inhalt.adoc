...
Ein OAuth-Client kann die OpenID-Erweiterung aufrufen, indem er einfach den Scope-Wert `openid` zu einer Autorisierungsanfrage hinzufügt.
...
Der Authorization Server/IAM-Service stellt die OpenID Connect API zur Verfügung.
...
Der Baustein `Security` stellt hierfür keine Methoden bereit.
Hintergrund ist, dass Informationen zum Nutzer innerhalb des Access-Tokens geschrieben werden.
Momentan befinden sich die Applikationen in einer geschützten Umgebung und nicht in einem Third-Party Szenario und deshalb können persönliche, schützenswerte Informationen über den Benutzer auch in den Access Token geschrieben werden.
