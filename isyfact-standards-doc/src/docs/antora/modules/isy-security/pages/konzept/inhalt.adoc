...
_ServiceGateways (SGW)_ die Authentifizierungsdaten von den Nutzungsdaten getrennt werden müssen.
...
Anwendung bis zur ersten Infrastrukturkomponente erfolgt der Datenaustausch per HTTPS-Protokoll. Mit Eintritt in die
IsyFact-Infrastruktur erfolgt am _Loadbalancer_ dann zuerst die Terminierung der Transportverschlüsselung und
Weiterleitung zum _Authentication-Gateway_ per HTTP-Protokoll.
...
BSI-Vorgaben unverschlüsselte User-Credentials nur in der externen Anwendung und im IAM selbst bekannt sein bzw.
...
unverschlüsselten User-Credentials kann das IAM-System die eigentliche Authentifizierung durchführen. Das IAM muss dabei
unterscheiden, ob es sich um die User-Credentials eines _Natürlichen Nutzers_ mit `Username`und `Passwort` oder
ob es sich um einen _Technischen Nutzer_ mit `Client-ID` und `Client-Secret` handelt. Für einen _Technischen Nutzer_
liegen alle Daten zur Authentifizierung im IAM-System vor. Für einen _Natürlichen Nutzer_ muss das IAM-System sich noch
weitere Daten aus der _Benutzerverwaltung_ zu den User-Credentials hinzuholen, um dann letztendlich die Authentifizierung
abschließen zu können. Mit validen User-Credentials und erfolgreicher Authentifizierung liefert das IAM-System dann ein Access-Token aus, das im Response
...
image::isy-security:konzept/Squenzdiagramm_ServiceGateway_plus_AuthGateway.svg[align="center"]
...
Access-Token. Dieses Token muss er sich zuerst beschaffen, indem er sich beim _IdentityAccessManagement-System (IAM)_
authentifiziert.

Die Kommunikation ausserhalb der IsyFact-Infrastruktur ist grundsätzlich Transportverschlüsselt, dh. von einer externen
Anwendung bis zur ersten Infrastrukturkomponente erfolgt der Datenaustausch per HTTPS-Protokoll. Mit Eintritt in die
IsyFact-Infrastruktur erfolgt am _Loadbalancer_ dann zuerst die Terminierung der Transportverschlüsselung und
Weiterleitung zum _Service-Gateway_ per HTTP-Protokoll.
...
Im Isy-IAM-System erfolgt mit den User-Credentials die eigentliche Authentifizierung. Das IAM muss dabei
unterscheiden, ob es sich um die User-Credentials eines _Natürlichen Nutzers_ mit `Username`und `Passwort` oder
ob es sich um einen _Technischen Nutzer_ mit `Client-ID` und `Client-Secret` handelt. Für einen _Technischen Nutzer_
liegen alle Daten zur Authentifizierung im IAM-System vor. Für einen _Natürlichen Nutzer_ muss das IAM-System sich noch
weitere Daten aus der _Benutzerverwaltung_ zu den User-Credentials hinzuholen, um dann letztendlich die Authentifizierung
abschließen zu können. Bei validen User-Credentials und
...
image::isy-security:konzept/Squenzdiagramm_ServiceGateway_ohne_AuthGateway.svg[align="center"]
...
Obwohl in diesem Verfahren die Clientanwendung direkten Zugang auf die evtl. unverschlüsselten User-Credentials hat,
...
image::isy-security:konzept/Resource-Owner-Passwort-Credential-Flow.svg[align="center"]

Im vorherigen Bild wird der logische Ablauf des _Resource-Owner-Password-Credential_-Flows dargestellt. Als
_Resource-Owner_ wird hier der Inhaber bzw. Wissensträger der User-Credentials `Username` und `Passwort` bezeichnet,
was für Confidential-Clients dann im allgemeinen ein Admin, DevOp oder eine Person ist, die die (1.) Konfiguration der
Anwendungen mit den User-Credentials in der Produktionsumgebung einrichtet bzw. vornimmt.

Die so konfigurierte (Confidential-Client)-Anwendung schickt (2.) dann diese Passwort-Credentials an das zuständige
Identity-Access-Management (IAM). Diese Anfrage an das IAM muss dabei den `grant_type="password"` und den `username` mit
`password` enthalten. Optional kann über das Attribut `scope` der zu autorisierenden Anwendungsbereich mitgegeben werden, der vom
IAM geprüft, gegebenenfalls korrigiert und im Access-Token zurückgeliefert wird.

Im IAM erfolgt mit den so übergebenen Credentials die Authentifizierung und Autorisierung. Bei erfolgreicher
Authentifizierung und Autorisierung wird ein gültiges Access-Token als
Antwort (3.) zurückgeliefert. Die Client-Anwendung kann mit dem so erhaltenen Access-Token dann ihre Fachanwendungen
aufrufen. Das Access-Token enthält alle notwendigen Daten, um zu verifizieren, ob die aufrufende Anwendung ordnungsgemäß
...
image::isy-security:konzept/Client-Credential-Flow.svg[align="center"]

Im vorherigen Bild wird der logische Ablauf des _Client-Credential_-Flows dargestellt. Hierbei kann die Clientanwendung
nach einem Access-Token beim IAM anfragen (1.), indem sie dabei einfach ihre eigenen Client-Credentials oder anderweitig
unterstützte Authentifizierungskriterien verwendet. Die Anfrage an das IAM muss dabei den `grant_type="client_credentials"`
enthalten. Optional kann über das Attribut `scope` der zu autorisierenden Anwendungsbereich mitgegeben werden, der vom
IAM geprüft, gegebenenfalls korrigiert und im Access-Token zurückgeliefert wird.

Die Authentifizierung und Autorisierung mit diesen so übermittelten Credentials erfolgt dann im IAM. Bei erfolgreicher
Authentifizierung und Autorisierung wird (2.) ein gültiges Access-Token als Antwort zurückgeliefert. Mit dem so
erhaltenen Access-Token kann die Client-Anwendung dann ihre Fachanwendungen aufrufen. Das Access-Token enthält alle
