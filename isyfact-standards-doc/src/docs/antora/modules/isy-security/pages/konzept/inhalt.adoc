...
Mit der `IsyFact 3` wird eine neue Authentifizierungslogik mit zusätzlichem _Authentication-Gateway_ für die Authentifizierung von externen Anwendungen, die außerhalb der Anwendungslandschaft betrieben werden, eingeführt.
...
Der _Client-Credential-Flow_ dient dabei der Authentifizierung eines _Technischen Nutzer_ und über den _Resource-Owner-Password-Credentials-Flow_ erfolgt die Authentifizierung eines _Natürlichen Nutzers.
...
[id="image-sgw_ohne_auth_gw",reftext="{figure-caption} {counter:figures}"]
...
Das nachfolgende Sequenzdiagramm zeigt den schematisch den logischen Ablauf der Authentifizierung und Autorisierung im Zusammenspiel der System-Komponenten mit dem _Service-Gateway_ aber ohne _Authentication-Gateway_:
