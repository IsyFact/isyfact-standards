...
class PollingCluster {
    ...
    /** Default Domain-Name für die JMX-MBeans. */
    private static final String DEFAULT_DOMAIN_NAME = "de.bund.bva.pliscommon.polling";
     ...
     */
    private List<JMXConnectionParameter> jmxConnectionParameterListe = new ArrayList<JMXConnectionParameter>(5);

    /** MBean-Objekt-Name. Wird aus der Cluster-Id abgeleitet */
    private String mBeanObjektName;

    /** Zeitstempel der letzten durchgeführten Polling-Aktivität in Millisekunden. */
    ...
    PollingCluster(String jmxDomain, String clusterId, String clusterName, int wartezeit, 
        ...
        if (jmxDomain == null) {
            this.mBeanObjektName = DEFAULT_DOMAIN_NAME + 
                ":type=PollingStatus,name=\"Polling-Aktivitaet-" +
                clusterName + "\"";
        } else {
            this.mBeanObjektName = jmxDomain + 
                ":type=PollingStatus,name=\"Polling-Aktivitaet-" +
                clusterName + "\"";
        }
