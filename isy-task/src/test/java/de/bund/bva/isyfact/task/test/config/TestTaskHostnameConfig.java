...
@Configuration
public class TestTaskHostnameConfig {

    @Bean
    public GesichertTask hostnameTask1(@Qualifier("taskMonitor1") TaskMonitor monitor) {
    ...
    @Bean
    public GesichertTask hostnameTask2(@Qualifier("taskMonitor2") TaskMonitor taskMonitor) {
    ...
    public MBeanExporter mBeanExporter(@Qualifier("taskMonitor1") TaskMonitor monitor1, @Qualifier("taskMonitor2") TaskMonitor monitor2) {
        ...
        String key = "de.bund.bva.isyfact.taskRunner:type=TaskMonitor,name=\"HostnameTask1\"";

        mBeans.put(key, monitor1);

        key = "de.bund.bva.isyfact.taskRunner:type=TaskMonitor,name=\"HostnameTask2\"";
