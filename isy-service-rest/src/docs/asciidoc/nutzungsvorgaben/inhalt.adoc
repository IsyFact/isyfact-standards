...
Bei der Erstellung der Spezifikation sind die Vorgaben der IsyFact (siehe <<KonzeptInterneRestServices>>) zu beachten.
...
Der Pfadname muss den Vorgaben zur Ressourcenorientierung im <<KonzeptInterneRestServices>> entsprechen.
...
Vorgaben zur Wahl des Verbs finden sich im <<KonzeptInterneRestServices>>.
|===
...
Anforderung: Es muss immer eine externe Id mitgeschickt werden, wenn die Mitarbeitersuche aufgerufen wurde.
...
Bei Anfragen zum Lesen (GET) oder Löschen (DELETE) werden gemäß REST-Konzept (siehe <<KonzeptInterneRestServices>>) keine Informationen im Body übertragen.
