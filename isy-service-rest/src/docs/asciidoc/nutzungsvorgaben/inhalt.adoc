...
Weitere Informationen zu OpenApi 3.0 (Beschreibung und Vorgaben) finden Sie unter <<SwaggerSpezifikation>>).
Bei der Erstellung der Spezifikation sind die Vorgaben der IsyFact (siehe <<KonzeptRest>>) zu beachten.
...
Für das Erstellen dieser fachlichen Schnittstellenbeschreibung wird die IsyFact-Vorlage <<IsyFactVorlageSchnittstellendokumentation>> verwendet.
...
Der Pfadname muss den Vorgaben zur Ressourcenorientierung im <<KonzeptRest>> entsprechen.
...
Vorgaben zur Wahl des Verbs finden sich im <<KonzeptRest>>.
|===

Die für die Mapping-Tabelle genutzte Beispiel-Schnittstellenbeschreibung und das Ergebnis als OpenAPI-Spezifikation ist in <<zusammenfassung-beispiel>> zusammenfassend dargestellt.
...
//ToDo: Beispiel für Header-Metadaten (z.B. externeID)

[[fachliche_info_get_delete]]
=== Übertragung fachlicher Informationen bei GET- und DELETE-Anfragen

Bei Anfragen zum Lesen (GET) oder Löschen (DELETE) werden gemäß REST-Konzept (siehe <<KonzeptRest>>) keine Informationen im Body übertragen.
