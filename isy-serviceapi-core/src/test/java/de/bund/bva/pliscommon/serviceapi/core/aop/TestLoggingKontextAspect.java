...
import de.bund.bva.pliscommon.serviceapi.core.aop.test.LoggingKontextAspectService;
...
/**
 * Die Testklasse TestLoggingKontextAspect kann gelöscht werden, wenn die Klasse LoggingKontextAspect.java, welche als
 * @Deprecated annotiert ist, gelöscht wurde. (Anmerk. die Funktionalität der Klasse LoggingKontextAspect wird von der
 * Klasse StelltLoggingKontextBereit.java übernommen).
 * <p>
 * Die in dieser Klasse vorhandenen Testmethoden sind in die neu erstellte Testdatei
 * StelltLoggingKontextBereitAspectTest "gewandert", so dass durch Löschung von TestLoggingKontextAspect.java keine
 * QM-Einbuße entsteht.
 */
@Deprecated
public class TestLoggingKontextAspect {
    ...
    public void testAufrufMit2KorrelationsId() {
        ...
        to.setKorrelationsId(korrId1 + ";" + korrId2);
        service.aufrufMitAufrufKontext(to);
        verify(mockAppender, times(1)).doAppend(captorLoggingEvent.capture());
        assertEquals("Setzte Korrelations-ID aus AufrufKontext.",
                captorLoggingEvent.getAllValues().get(0).getMessage());
