...
/**
 * Tests for class PlisSicherheitUtil.
 */
public class PlisSicherheitUtilTest {

    @Test
    public void basicAuthUtilTest() {
        // Base 64 encoded "LoginId:Password"
        String base64EncodedSample1 = "Basic TG9naW5JZDpQYXNzd29yZA==";
        String[] loginCredentials = PlisSicherheitUtil.parseBasisAuthHeaderValue(base64EncodedSample1);
        ...
        loginCredentials = PlisSicherheitUtil.parseBasisAuthHeaderValue(base64EncodedSample2);
        assertEquals("LoginId-ÄÖÜ", loginCredentials[0]);
        assertEquals("Password-äöü", loginCredentials[1]);

        // "Basic" must is not case sensitive.
        loginCredentials = PlisSicherheitUtil.parseBasisAuthHeaderValue("BASIC TG9naW5JZDpQYXNzd29yZA==");
        assertEquals("LoginId", loginCredentials[0]);
        assertEquals("Password", loginCredentials[1]);

        // Header value must start with "Basic" (case insensitive)
        loginCredentials =
            PlisSicherheitUtil.parseBasisAuthHeaderValue("NOBASIC TG9naW5JZC3E1tw6UGFzc3dvcmQt5Pb8");
        assertNull(loginCredentials[0]);
        assertNull(loginCredentials[1]);

        // Insert of null results in no login credentials
        loginCredentials = PlisSicherheitUtil.parseBasisAuthHeaderValue(null);
        assertNull(loginCredentials[0]);
        assertNull(loginCredentials[1]);

        // Create Basic authentication header with normal characters
        String[] authHeader = PlisSicherheitUtil.createBasicAuthHeader("LoginId", "Password");
        ...
        authHeader = PlisSicherheitUtil.createBasicAuthHeader("LoginId-ÄÖÜ", "Password-äöü");
        assertEquals("Authorization", authHeader[0]);
        assertEquals(base64EncodedSample2, authHeader[1]);

        // Get the credentials from created header gives the input parameters.
        loginCredentials =
            PlisSicherheitUtil.parseBasisAuthHeaderValue(PlisSicherheitUtil.createBasicAuthHeader(
