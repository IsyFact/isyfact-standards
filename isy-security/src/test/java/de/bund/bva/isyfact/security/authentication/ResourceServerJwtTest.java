...
@SpringBootTest(classes = {
        IsySecurityTestApplication.class, SecurityConfig.class, ExampleRestController.class
...
public class ResourceServerJwtTest extends AbstractOidcProviderTest {
    ...
    public void shouldFetchJwksFromOAuth2Server() {
        ...
        String token = embeddedOidcProvider.getAccessTokenString("testclient", "testuser", Optional.empty(),
        ...
        verify(getRequestedFor(urlEqualTo(ISSUER_PATH + "/.well-known/openid-configuration")));
        // provider config contains the JWKS endpoint which contains the certs used to verify the token
        verify(getRequestedFor(urlEqualTo(ISSUER_PATH + "/protocol/openid-connect/certs")));
    }
